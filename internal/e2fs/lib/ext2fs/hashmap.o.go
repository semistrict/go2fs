...
func Xext2fs_djb2_hash(cgtls *iqlibc.ppTLS, aastr ppuintptr, aasize tnsize_t) (cgr tnuint32_t) {

	var aac ppint32
	var aahash tnuint32_t
	var aas, ccv2 ppuintptr
	var ccv1 tnsize_t
	pp_, pp_, pp_, pp_, pp_ = aac, aahash, aas, ccv1, ccv2
	aas = aastr
	aahash = ppuint32(5381)
	...
	for {
		...
		ccv2 = aas
		aas = aas + 1
		aac = ppint32(*(*ppint8)(iqunsafe.ppPointer(ccv2)))
		aahash = aahash<<iqlibc.ppInt32FromInt32(5) + aahash + iqlibc.ppUint32FromInt32(aac)

	}
	return aahash
}

type tn__ccgo_fp__Xext2fs_hashmap_create_0 = func(*iqlibc.ppTLS, ppuintptr, ppuint64) ppuint32

type tn__ccgo_fp__Xext2fs_hashmap_create_1 = func(*iqlibc.ppTLS, ppuintptr)

func Xext2fs_hashmap_create(cgtls *iqlibc.ppTLS, aa__ccgo_fp_hash_fct ppuintptr, aa__ccgo_fp_free_fct ppuintptr, aasize tnsize_t) (cgr ppuintptr) {

	var aah, ccv1 ppuintptr
	pp_, pp_ = aah, ccv1
	aah = Xcalloc(cgtls, ppuint64(1), ppuint64(40)+ppuint64(48)*aasize)
	if !(aah != 0) {
		return iqlibc.ppUintptrFromInt32(0)
	}

	(*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdsize = ppuint32(aasize)
	(*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdfree = aa__ccgo_fp_free_fct
	(*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdhash = aa__ccgo_fp_hash_fct
	ccv1 = iqlibc.ppUintptrFromInt32(0)
	(*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdlast = ccv1
	(*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdfirst = ccv1
	return aah
}

func Xext2fs_hashmap_add(cgtls *iqlibc.ppTLS, aah ppuintptr, aadata ppuintptr, aakey ppuintptr, aakey_len tnsize_t) (cgr ppint32) {

	var aae ppuintptr
	var aahash tnuint32_t
	pp_, pp_ = aae, aahash
	aahash = (*(*func(*iqlibc.ppTLS, ppuintptr, tnsize_t) tnuint32_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdhash})))(cgtls, aakey, aakey_len) % (*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdsize
	aae = Xmalloc(cgtls, ppuint64(48))

	if !(aae != 0) {
		return -ppint32(1)
	}

	(*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aae)).fddata = aadata
	(*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aae)).fdkey = aakey
	(*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aae)).fdkey_len = aakey_len
	(*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aae)).fdnext = *(*ppuintptr)(iqunsafe.ppPointer(aah + 40 + ppuintptr(aahash)*8))
	*(*ppuintptr)(iqunsafe.ppPointer(aah + 40 + ppuintptr(aahash)*8)) = aae

	(*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aae)).fdlist_prev = iqlibc.ppUintptrFromInt32(0)
	(*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aae)).fdlist_next = (*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdfirst
	if (*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdfirst != 0 {
		(*tsext2fs_hashmap_entry)(iqunsafe.ppPointer((*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdfirst)).fdlist_prev = aae
	}
	(*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdfirst = aae
	if !((*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdlast != 0) {
		(*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdlast = aae
	}

	return 0
}

func Xext2fs_hashmap_lookup(cgtls *iqlibc.ppTLS, aah ppuintptr, aakey ppuintptr, aakey_len tnsize_t) (cgr ppuintptr) {

	var aahash tnuint32_t
	var aaiter ppuintptr
	pp_, pp_ = aahash, aaiter
	aahash = (*(*func(*iqlibc.ppTLS, ppuintptr, tnsize_t) tnuint32_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdhash})))(cgtls, aakey, aakey_len) % (*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdsize

	aaiter = *(*ppuintptr)(iqunsafe.ppPointer(aah + 40 + ppuintptr(aahash)*8))
	for {
		if !(aaiter != 0) {
			break
		}
		if (*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aaiter)).fdkey_len == aakey_len && !(Xmemcmp(cgtls, (*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aaiter)).fdkey, aakey, aakey_len) != 0) {
			return (*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aaiter)).fddata
		}
		goto cg_1
	cg_1:
		;
		aaiter = (*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aaiter)).fdnext
	}
	return iqlibc.ppUintptrFromInt32(0)
}

func Xext2fs_hashmap_iter_in_order(cgtls *iqlibc.ppTLS, aah ppuintptr, aait ppuintptr) (cgr ppuintptr) {

	var ccv1, ccv2 ppuintptr
	pp_, pp_ = ccv1, ccv2
	if *(*ppuintptr)(iqunsafe.ppPointer(aait)) != 0 {
		ccv1 = (*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(aait)))).fdlist_next
	} else {
		ccv1 = (*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdfirst
	}
	*(*ppuintptr)(iqunsafe.ppPointer(aait)) = ccv1
	if *(*ppuintptr)(iqunsafe.ppPointer(aait)) != 0 {
		ccv1 = (*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(aait)))).fddata
	} else {
		ccv1 = iqlibc.ppUintptrFromInt32(0)
	}
	return ccv1
...
func Xext2fs_hashmap_free(cgtls *iqlibc.ppTLS, aah ppuintptr) {

	var aai tnsize_t
	var aait, aatmp ppuintptr
	pp_, pp_, pp_ = aai, aait, aatmp

	aai = ppuint64(0)
	for {
		if !(aai < ppuint64((*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdsize)) {
			break
		}
		aait = *(*ppuintptr)(iqunsafe.ppPointer(aah + 40 + ppuintptr(aai)*8))
		for aait != 0 {
			aatmp = (*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aait)).fdnext
			if (*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdfree != 0 {
				(*(*func(*iqlibc.ppTLS, ppuintptr))(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsext2fs_hashmap)(iqunsafe.ppPointer(aah)).fdfree})))(cgtls, (*tsext2fs_hashmap_entry)(iqunsafe.ppPointer(aait)).fddata)
			}
			Xfree(cgtls, aait)
			aait = aatmp
		}
