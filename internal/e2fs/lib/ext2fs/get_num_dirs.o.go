...
//	 * wrong!
//	 */
func Xext2fs_get_num_dirs(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaret_num_dirs ppuintptr) (cgr tnerrcode_t) {

	var aai tndgrp_t
	var aamax_dirs, aanum_dirs tnext2_ino_t
	pp_, pp_, pp_ = aai, aamax_dirs, aanum_dirs

	if (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdmagic != ppint64(2133571329) {
		return iqlibc.ppInt64FromInt64(2133571329)
	}

	aanum_dirs = ppuint32(0)
	aamax_dirs = (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_per_group
	aai = ppuint32(0)
	for {
		if !(aai < (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdgroup_desc_count) {
			break
		}
		if Xext2fs_bg_used_dirs_count(cgtls, aafs, aai) > aamax_dirs {
			aanum_dirs = aanum_dirs + aamax_dirs/ppuint32(8)
		} else {
			aanum_dirs = aanum_dirs + Xext2fs_bg_used_dirs_count(cgtls, aafs, aai)
		}
	...
	if aanum_dirs > (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_count {
		aanum_dirs = (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_count
	}

	*(*tnext2_ino_t)(iqunsafe.ppPointer(aaret_num_dirs)) = aanum_dirs
