...
//	 */
func Xext2fs_reserve_super_and_bgd(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aagroup tndgrp_t, aabmap tnext2fs_block_bitmap) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aanum_blocks, aaold_desc_blocks tnblk_t
	var ccv1 ppint32
	var pp_ /* new_desc_blk at bp+16 */ tnblk64_t
	var pp_ /* old_desc_blk at bp+8 */ tnblk64_t
	var pp_ /* super_blk at bp+0 */ tnblk64_t
	var pp_ /* used_blks at bp+24 */ tnblk_t
	pp_, pp_, pp_ = aanum_blocks, aaold_desc_blocks, ccv1
...
cg_2:
	if ccv1 != 0 {
		aaold_desc_blocks = (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_meta_bg
	} else {
		aaold_desc_blocks = ppuint32((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fddesc_blocks + ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_reserved_gdt_blocks))
	}

	if *(*tnblk64_t)(iqunsafe.ppPointer(cgbp)) != 0 || aagroup == ppuint32(0) {
		pp_ = Xext2fs_mark_generic_bmap(cgtls, aabmap, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp)))
		goto cg_3
	cg_3:
	}
	if aagroup == ppuint32(0) && (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize == ppuint32(1024) && ppint32(1)<<(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits > ppint32(1) {
		pp_ = Xext2fs_mark_generic_bmap(cgtls, aabmap, ppuint64(0))
		goto cg_4
	cg_4:
	}

	if *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8)) != 0 {
		aanum_blocks = aaold_desc_blocks
		if *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8))+ppuint64(aanum_blocks) >= Xext2fs_blocks_count(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper) {
			aanum_blocks = ppuint32(Xext2fs_blocks_count(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper) - *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8)))
		}
		Xext2fs_mark_block_bitmap_range2(cgtls, aabmap, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8)), aanum_blocks)
	}
	if *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) != 0 {
		pp_ = Xext2fs_mark_generic_bmap(cgtls, aabmap, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)))
		goto cg_5
	cg_5:
	}

	aanum_blocks = iqlibc.ppUint32FromInt32(Xext2fs_group_blocks_count(cgtls, aafs, aagroup))
	aanum_blocks = aanum_blocks - (ppuint32(2) + (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdinode_blocks_per_group + *(*tnblk_t)(iqunsafe.ppPointer(cgbp + 24)))

	return iqlibc.ppInt32FromUint32(aanum_blocks)
}
...
//	 */
func Xext2fs_reserve_super_and_bgd2(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aagroup tndgrp_t, aabmap tnext2fs_block_bitmap, aadesc_blocks ppuintptr) (cgr tnerrcode_t) {
	...
	Xext2fs_reserve_super_and_bgd(cgtls, aafs, aagroup, aabmap)

	aaretval = Xext2fs_count_used_blocks(cgtls, aafs, Xext2fs_group_first_block2(cgtls, aafs, aagroup), Xext2fs_group_last_block2(cgtls, aafs, aagroup), cgbp)
	if !(aaretval != 0) {
		*(*tnblk_t)(iqunsafe.ppPointer(aadesc_blocks)) = ppuint32(*(*tnblk64_t)(iqunsafe.ppPointer(cgbp)))
	}
...
func _ext2fs_count_used_blocks(*iqlibc.ppTLS, ppuintptr, ppuint64, ppuint64, ppuintptr) ppint64
...
func _ext2fs_super_and_bgd_loc2(*iqlibc.ppTLS, ppuintptr, ppuint32, ppuintptr, ppuintptr, ppuintptr, ppuintptr) ppint64
