...
const mvTEST_FLAG_DISCARD = 64
const mvTEST_FLAG_DUMP = 16
const mvTEST_FLAG_FLUSH = 8
const mvTEST_FLAG_READ = 1
const mvTEST_FLAG_READAHEAD = 128
const mvTEST_FLAG_SET_BLKSIZE = 4
const mvTEST_FLAG_SET_OPTION = 32
const mvTEST_FLAG_WRITE = 2
const mvTEST_FLAG_ZEROOUT = 256
const mvTIME_UTC = 1
...
type tstest_private_data = struct {
	fdmagic             ppint32
	fdreal1             tnio_channel
	fdflags             ppint32
	fdoutfile           ppuintptr
	fdblock             ppuint64
	fdread_abort_count  ppint32
	fdwrite_abort_count ppint32
	fdread_blk          ppuintptr
	fdwrite_blk         ppuintptr
	fdset_blksize       ppuintptr
	fdwrite_byte        ppuintptr
...
//	 */
var Xtest_io_backing_manager tnio_manager
var Xtest_io_cb_read_blk ppuintptr
var Xtest_io_cb_write_blk ppuintptr
var Xtest_io_cb_read_blk64 ppuintptr
var Xtest_io_cb_write_blk64 ppuintptr
var Xtest_io_cb_set_blksize ppuintptr
var Xtest_io_cb_write_byte ppuintptr

/*
 * Test flags
 */

func sitest_dump_block(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aadata ppuintptr, aablock ppuint64, aabuf ppuintptr) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aacksum ppuint64
	var aacp, aaf ppuintptr
	var aai, ccv3 ppint32
	pp_, pp_, pp_, pp_, pp_ = aacksum, aacp, aaf, aai, ccv3
	aaf = (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile
	aacksum = ppuint64(0)

	aai = 0
	aacp = aabuf
	for {
		if !(aai < (*tsstruct_io_channel)(iqunsafe.ppPointer(aachannel)).fdblock_size) {
			break
		}
		aacksum = aacksum + ppuint64(*(*ppuint8)(iqunsafe.ppPointer(aacp)))
	...
	Xfprintf(cgtls, aaf, "Contents of block %lu, checksum %08lu: \n\x00", iqlibc.ppVaList(cgbp+8, aablock, aacksum))
	aai = 0
	aacp = aabuf
	for {
		if !(aai < (*tsstruct_io_channel)(iqunsafe.ppPointer(aachannel)).fdblock_size) {
		...
		if aai%ppint32(16) == 0 {
			Xfprintf(cgtls, aaf, "%04x: \x00", iqlibc.ppVaList(cgbp+8, aai))
		}
		if aai%ppint32(16) == ppint32(15) {
			ccv3 = ppint32('\n')
		} else {
			ccv3 = ppint32(' ')
		}
		Xfprintf(cgtls, aaf, "%02x%c\x00", iqlibc.ppVaList(cgbp+8, iqlibc.ppInt32FromUint8(*(*ppuint8)(iqunsafe.ppPointer(aacp))), ccv3))
...
//	 */
func sitest_flush(cgtls *iqlibc.ppTLS, aachannel tnio_channel) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aadata, ccv1 ppuintptr
	...
	if ppint64((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdmagic) != ppint64(2133571339) {
		return iqlibc.ppInt64FromInt64(2133571339)
	}

	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 {
		aaretval = (*(*func(*iqlibc.ppTLS, tnio_channel) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsstruct_io_manager)(iqunsafe.ppPointer((*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdmanager)).fdflush})))(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)
	}

	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_FLUSH) != 0 {
		if aaretval != 0 {
			ccv1 = Xerror_message(cgtls, aaretval)
		} else {
			ccv1 = "OK\x00"
		}
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: flush() returned %s\n\x00", iqlibc.ppVaList(cgbp+8, ccv1))
	}

	return aaretval
}

func sitest_abort(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aablock ppuint64) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aadata, aaf ppuintptr
	pp_, pp_ = aadata, aaf

	aadata = (*tsstruct_io_channel)(iqunsafe.ppPointer(aachannel)).fdprivate_data
	aaf = (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile
	sitest_flush(cgtls, aachannel)

	Xfprintf(cgtls, aaf, "Aborting due to I/O to block %lu\n\x00", iqlibc.ppVaList(cgbp+8, aablock))
	Xfflush(cgtls, aaf)
	Xabort(cgtls)
}

func sitest_open(cgtls *iqlibc.ppTLS, aaname ppuintptr, aaflags ppint32, aachannel ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aaretval, ccv1, ccv3, ccv5 tnerrcode_t
	var aavalue, ccv10, ccv11, ccv12, ccv14, ccv16, ccv7, ccv8, ccv9 ppuintptr
	var pp_ /* data at bp+24 */ ppuintptr
	var pp_ /* io at bp+16 */ tnio_channel
	var pp_ /* p at bp+8 */ ppuintptr
	var pp_ /* pp at bp+0 */ ppuintptr
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aaretval, aavalue, ccv1, ccv10, ccv11, ccv12, ccv14, ccv16, ccv3, ccv5, ccv7, ccv8, ccv9
...
cg_2:
	...
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, ppuint64(96))
	if !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) != 0) {
		ccv1 = iqlibc.ppInt64FromInt64(2133571398)
		goto cg_4
	}
...
cg_4:
	...
	(*tsstruct_io_channel)(iqunsafe.ppPointer(*(*tnio_channel)(iqunsafe.ppPointer(cgbp + 16)))).fdmanager = Xtest_io_manager
...
cg_6:
	...
	Xmemset(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)), 0, ppuint64(96))
	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdmagic = ppint32(iqlibc.ppInt64FromInt64(2133571339))
	if Xtest_io_backing_manager != 0 {
		aaretval = (*(*func(*iqlibc.ppTLS, ppuintptr, ppint32, ppuintptr) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsstruct_io_manager)(iqunsafe.ppPointer(Xtest_io_backing_manager)).fdopen})))(cgtls, aaname, aaflags, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24))+8)
		if aaretval != 0 {
			goto ppcleanup
		}
	} else {
		(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdreal1 = ppuintptr(0)
	}
	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdread_blk = Xtest_io_cb_read_blk
	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdwrite_blk = Xtest_io_cb_write_blk
	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdset_blksize = Xtest_io_cb_set_blksize
	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdwrite_byte = Xtest_io_cb_write_byte
	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdread_blk64 = Xtest_io_cb_read_blk64
	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdwrite_blk64 = Xtest_io_cb_write_blk64

	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdoutfile = iqlibc.ppUintptrFromInt32(0)
	ccv7 = Xext2fs_safe_getenv(cgtls, "TEST_IO_LOGFILE\x00")
	aavalue = ccv7
	if ccv7 != iqlibc.ppUintptrFromInt32(0) {
		(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdoutfile = Xfopen(cgtls, aavalue, "w\x00")
	}
	if !((*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdoutfile != 0) {
		(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdoutfile = X__stderrp
	}

	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdflags = 0
	ccv7 = Xext2fs_safe_getenv(cgtls, "TEST_IO_FLAGS\x00")
	aavalue = ccv7
	if ccv7 != iqlibc.ppUintptrFromInt32(0) {
		(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdflags = iqlibc.ppInt32FromUint64(Xstrtoul(cgtls, aavalue, iqlibc.ppUintptrFromInt32(0), 0))
	}

	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdblock = ppuint64(0)
	ccv7 = Xext2fs_safe_getenv(cgtls, "TEST_IO_BLOCK\x00")
	aavalue = ccv7
	if ccv7 != iqlibc.ppUintptrFromInt32(0) {
		(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdblock = Xstrtoul(cgtls, aavalue, iqlibc.ppUintptrFromInt32(0), 0)
	}

	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdread_abort_count = 0
	ccv7 = Xext2fs_safe_getenv(cgtls, "TEST_IO_READ_ABORT\x00")
	aavalue = ccv7
	if ccv7 != iqlibc.ppUintptrFromInt32(0) {
		(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdread_abort_count = iqlibc.ppInt32FromUint64(Xstrtoul(cgtls, aavalue, iqlibc.ppUintptrFromInt32(0), 0))
	}

	(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdwrite_abort_count = 0
	ccv7 = Xext2fs_safe_getenv(cgtls, "TEST_IO_WRITE_ABORT\x00")
	aavalue = ccv7
	if ccv7 != iqlibc.ppUintptrFromInt32(0) {
		(*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdwrite_abort_count = iqlibc.ppInt32FromUint64(Xstrtoul(cgtls, aavalue, iqlibc.ppUintptrFromInt32(0), 0))
	}

	if (*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdreal1 != 0 {
		(*tsstruct_io_channel)(iqunsafe.ppPointer(*(*tnio_channel)(iqunsafe.ppPointer(cgbp + 16)))).fdalign = (*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdreal1)).fdalign
		if (*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))).fdreal1)).fdflags&ppuint32(mvCHANNEL_FLAGS_THREADS) != 0 {
...
func sitest_close(cgtls *iqlibc.ppTLS, cg_channel tnio_channel) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)
	...
	*(*ppint32)(iqunsafe.ppPointer(ccv2)) = *(*ppint32)(iqunsafe.ppPointer(ccv2)) - 1
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 {
		aaretval = (*(*func(*iqlibc.ppTLS, tnio_channel) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsstruct_io_manager)(iqunsafe.ppPointer((*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdmanager)).fdclose1})))(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)
	}

	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile != 0 && (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile != X__stderrp {
		Xfclose(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile)
	}
...
func sitest_set_blksize(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aablksize ppint32) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 {
		aaretval = (*(*func(*iqlibc.ppTLS, tnio_channel, ppint32) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsstruct_io_manager)(iqunsafe.ppPointer((*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdmanager)).fdset_blksize})))(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aablksize)
		(*tsstruct_io_channel)(iqunsafe.ppPointer(aachannel)).fdalign = (*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdalign
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdset_blksize != 0 {
		(*(*func(*iqlibc.ppTLS, ppint32, tnerrcode_t))(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdset_blksize})))(cgtls, aablksize, aaretval)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_SET_BLKSIZE) != 0 {
		if aaretval != 0 {
			ccv1 = Xerror_message(cgtls, aaretval)
		} else {
			ccv1 = "OK\x00"
		}
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: set_blksize(%d) returned %s\n\x00", iqlibc.ppVaList(cgbp+8, aablksize, ccv1))
	}
	(*tsstruct_io_channel)(iqunsafe.ppPointer(aachannel)).fdblock_size = aablksize
	return aaretval
}

func sitest_read_blk(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aablock ppuint64, aacount ppint32, aabuf ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aadata, ccv1, ccv3 ppuintptr
	var aaretval tnerrcode_t
	var ccv2 ppint32
	pp_, pp_, pp_, pp_, pp_ = aadata, aaretval, ccv1, ccv2, ccv3
	aaretval = 0
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 {
		aaretval = (*(*func(*iqlibc.ppTLS, tnio_channel, ppuint64, ppint32, ppuintptr) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsstruct_io_manager)(iqunsafe.ppPointer((*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdmanager)).fdread_blk})))(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aablock, aacount, aabuf)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdread_blk != 0 {
		(*(*func(*iqlibc.ppTLS, ppuint64, ppint32, tnerrcode_t))(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdread_blk})))(cgtls, aablock, aacount, aaretval)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_READ) != 0 {
		if aaretval != 0 {
			ccv1 = Xerror_message(cgtls, aaretval)
		} else {
			ccv1 = "OK\x00"
		}
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: read_blk(%lu, %d) returned %s\n\x00", iqlibc.ppVaList(cgbp+8, aablock, aacount, ccv1))
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdblock != 0 && (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdblock == aablock {
		if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_DUMP) != 0 {
			sitest_dump_block(cgtls, aachannel, aadata, aablock, aabuf)
		}
		ccv1 = aadata + 40
		*(*ppint32)(iqunsafe.ppPointer(ccv1)) = *(*ppint32)(iqunsafe.ppPointer(ccv1)) - 1
		ccv2 = *(*ppint32)(iqunsafe.ppPointer(ccv1))
		if ccv2 == 0 {
			sitest_abort(cgtls, aachannel, aablock)
		}
	}
	return aaretval
}

func sitest_write_blk(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aablock ppuint64, aacount ppint32, aabuf ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 {
		aaretval = (*(*func(*iqlibc.ppTLS, tnio_channel, ppuint64, ppint32, ppuintptr) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsstruct_io_manager)(iqunsafe.ppPointer((*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdmanager)).fdwrite_blk})))(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aablock, aacount, aabuf)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdwrite_blk != 0 {
		(*(*func(*iqlibc.ppTLS, ppuint64, ppint32, tnerrcode_t))(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdwrite_blk})))(cgtls, aablock, aacount, aaretval)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_WRITE) != 0 {
		if aaretval != 0 {
			ccv1 = Xerror_message(cgtls, aaretval)
		} else {
			ccv1 = "OK\x00"
		}
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: write_blk(%lu, %d) returned %s\n\x00", iqlibc.ppVaList(cgbp+8, aablock, aacount, ccv1))
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdblock != 0 && (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdblock == aablock {
		if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_DUMP) != 0 {
			sitest_dump_block(cgtls, aachannel, aadata, aablock, aabuf)
...
func sitest_read_blk64(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aablock ppuint64, aacount ppint32, aabuf ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 {
		aaretval = Xio_channel_read_blk64(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aablock, aacount, aabuf)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdread_blk64 != 0 {
		(*(*func(*iqlibc.ppTLS, ppuint64, ppint32, tnerrcode_t))(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdread_blk64})))(cgtls, aablock, aacount, aaretval)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_READ) != 0 {
		if aaretval != 0 {
			ccv1 = Xerror_message(cgtls, aaretval)
		...
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: read_blk64(%llu, %d) returned %s\n\x00", iqlibc.ppVaList(cgbp+8, aablock, aacount, ccv1))
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdblock != 0 && ppuint64((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdblock) == aablock {
		if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_DUMP) != 0 {
			sitest_dump_block(cgtls, aachannel, aadata, ppuint64(aablock), aabuf)
		}
		ccv1 = aadata + 40
		*(*ppint32)(iqunsafe.ppPointer(ccv1)) = *(*ppint32)(iqunsafe.ppPointer(ccv1)) - 1
		ccv2 = *(*ppint32)(iqunsafe.ppPointer(ccv1))
		if ccv2 == 0 {
			sitest_abort(cgtls, aachannel, ppuint64(aablock))
		}
	}
	return aaretval
}

func sitest_write_blk64(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aablock ppuint64, aacount ppint32, aabuf ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 {
		aaretval = Xio_channel_write_blk64(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aablock, aacount, aabuf)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdwrite_blk64 != 0 {
		(*(*func(*iqlibc.ppTLS, ppuint64, ppint32, tnerrcode_t))(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdwrite_blk64})))(cgtls, aablock, aacount, aaretval)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_WRITE) != 0 {
		if aaretval != 0 {
			ccv1 = Xerror_message(cgtls, aaretval)
		...
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: write_blk64(%llu, %d) returned %s\n\x00", iqlibc.ppVaList(cgbp+8, aablock, aacount, ccv1))
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdblock != 0 && ppuint64((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdblock) == aablock {
		if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_DUMP) != 0 {
			sitest_dump_block(cgtls, aachannel, aadata, ppuint64(aablock), aabuf)
...
func sitest_write_byte(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aaoffset ppuint64, aacount ppint32, aabuf ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 && (*tsstruct_io_manager)(iqunsafe.ppPointer((*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdmanager)).fdwrite_byte != 0 {
		aaretval = Xio_channel_write_byte(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aaoffset, aacount, aabuf)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdwrite_byte != 0 {
		(*(*func(*iqlibc.ppTLS, ppuint64, ppint32, tnerrcode_t))(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdwrite_byte})))(cgtls, aaoffset, aacount, aaretval)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_WRITE) != 0 {
		if aaretval != 0 {
			ccv1 = Xerror_message(cgtls, aaretval)
		...
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: write_byte(%lu, %d) returned %s\n\x00", iqlibc.ppVaList(cgbp+8, aaoffset, aacount, ccv1))
	}
	return aaretval
}

func sitest_set_option(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aaoption ppuintptr, aaarg ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_SET_OPTION) != 0 {
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: set_option(%s, %s) \x00", iqlibc.ppVaList(cgbp+8, aaoption, aaarg))
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 && (*tsstruct_io_manager)(iqunsafe.ppPointer((*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdmanager)).fdset_option != 0 {
		aaretval = (*(*func(*iqlibc.ppTLS, tnio_channel, ppuintptr, ppuintptr) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsstruct_io_manager)(iqunsafe.ppPointer((*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdmanager)).fdset_option})))(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aaoption, aaarg)
		if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_SET_OPTION) != 0 {
			if aaretval != 0 {
				ccv1 = Xerror_message(cgtls, aaretval)
			} else {
			...
			Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "returned %s\n\x00", iqlibc.ppVaList(cgbp+8, ccv1))
		}
	} else {
		if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_SET_OPTION) != 0 {
			Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "not implemented\n\x00", 0)
		}
	}
	return aaretval
}

func sitest_get_stats(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aastats ppuintptr) (cgr tnerrcode_t) {
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 && (*tsstruct_io_manager)(iqunsafe.ppPointer((*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdmanager)).fdget_stats != 0 {
		aaretval = (*(*func(*iqlibc.ppTLS, tnio_channel, ppuintptr) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsstruct_io_manager)(iqunsafe.ppPointer((*tsstruct_io_channel)(iqunsafe.ppPointer((*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1)).fdmanager)).fdget_stats})))(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aastats)
	}
	return aaretval
}

func sitest_discard(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aablock ppuint64, aacount ppuint64) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 {
		aaretval = Xio_channel_discard(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aablock, aacount)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_DISCARD) != 0 {
		if aaretval != 0 {
			ccv1 = Xerror_message(cgtls, aaretval)
		} else {
			ccv1 = "OK\x00"
		}
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: discard(%llu, %llu) returned %s\n\x00", iqlibc.ppVaList(cgbp+8, aablock, aacount, ccv1))
	}
	return aaretval
}

func sitest_cache_readahead(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aablock ppuint64, aacount ppuint64) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 {
		aaretval = Xio_channel_cache_readahead(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aablock, aacount)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_READAHEAD) != 0 {
		if aaretval != 0 {
			ccv1 = Xerror_message(cgtls, aaretval)
		} else {
			ccv1 = "OK\x00"
		}
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: readahead(%llu, %llu) returned %s\n\x00", iqlibc.ppVaList(cgbp+8, aablock, aacount, ccv1))
	}
	return aaretval
}

func sitest_zeroout(cgtls *iqlibc.ppTLS, aachannel tnio_channel, aablock ppuint64, aacount ppuint64) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
	...
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1 != 0 {
		aaretval = Xio_channel_zeroout(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdreal1, aablock, aacount)
	}
	if (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdflags&ppint32(mvTEST_FLAG_ZEROOUT) != 0 {
		if aaretval != 0 {
			ccv1 = Xerror_message(cgtls, aaretval)
		} else {
			ccv1 = "OK\x00"
		}
		Xfprintf(cgtls, (*tstest_private_data)(iqunsafe.ppPointer(aadata)).fdoutfile, "Test_io: zeroout(%llu, %llu) returned %s\n\x00", iqlibc.ppVaList(cgbp+8, aablock, aacount, ccv1))
	}
	return aaretval
}

var sistruct_test_manager = tsstruct_io_manager{
	fdmagic: iqlibc.ppInt64FromInt64(2133571335),
	fdname:  "Test I/O Manager\x00",
}

func init() {
	p := iqunsafe.ppPointer(&sistruct_test_manager)
	*(*uintptr)(iqunsafe.ppAdd(p, 16)) = pp__ccgo_fp(sitest_open)
	*(*uintptr)(iqunsafe.ppAdd(p, 24)) = pp__ccgo_fp(sitest_close)
	*(*uintptr)(iqunsafe.ppAdd(p, 32)) = pp__ccgo_fp(sitest_set_blksize)
	*(*uintptr)(iqunsafe.ppAdd(p, 40)) = pp__ccgo_fp(sitest_read_blk)
	*(*uintptr)(iqunsafe.ppAdd(p, 48)) = pp__ccgo_fp(sitest_write_blk)
	*(*uintptr)(iqunsafe.ppAdd(p, 56)) = pp__ccgo_fp(sitest_flush)
	*(*uintptr)(iqunsafe.ppAdd(p, 64)) = pp__ccgo_fp(sitest_write_byte)
	*(*uintptr)(iqunsafe.ppAdd(p, 72)) = pp__ccgo_fp(sitest_set_option)
	*(*uintptr)(iqunsafe.ppAdd(p, 80)) = pp__ccgo_fp(sitest_get_stats)
	*(*uintptr)(iqunsafe.ppAdd(p, 88)) = pp__ccgo_fp(sitest_read_blk64)
	*(*uintptr)(iqunsafe.ppAdd(p, 96)) = pp__ccgo_fp(sitest_write_blk64)
	*(*uintptr)(iqunsafe.ppAdd(p, 104)) = pp__ccgo_fp(sitest_discard)
	*(*uintptr)(iqunsafe.ppAdd(p, 112)) = pp__ccgo_fp(sitest_cache_readahead)
	*(*uintptr)(iqunsafe.ppAdd(p, 120)) = pp__ccgo_fp(sitest_zeroout)
}

var Xtest_io_manager = ppuintptr(iqunsafe.ppPointer(&sistruct_test_manager))
