...
const mvEXT2FS_LINK_APPEND = 16
const mvEXT2FS_LINK_EXPAND = 32
const mvEXT2FS_LINK_FT_MASK = 7
...
type tsdx_lookup_info = struct {
	fdname     ppuintptr
	fdnamelen  ppint32
	fdhash_alg ppint32
	fdhash     tn__u32
	fdlevels   ppuint32
	fdframes   [3]tsdx_frame
}

func sialloc_dx_frame(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaframe ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var ccv1 tnerrcode_t
	...
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, ppuint64((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize))
	...
	Xmemcpy(cgtls, aaframe, cgbp, ppuint64(8))
	ccv1 = 0
	goto cg_2
cg_2:
	return ccv1
...
func sidx_release(cgtls *iqlibc.ppTLS, aainfo ppuintptr) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aalevel ppuint32
	var ccv2 ppuintptr
	var pp_ /* p at bp+0 */ ppuintptr
	pp_, pp_ = aalevel, ccv2

	aalevel = ppuint32(0)
	for {
		if !(aalevel < (*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdlevels) {
			break
		}
		if (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aalevel)*40))).fdbuf == iqlibc.ppUintptrFromInt32(0) {
			break
		}
		ccv2 = aainfo + 24 + ppuintptr(aalevel)*40
	...
	(*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdlevels = ppuint32(0)
}

func sidx_search_entry(cgtls *iqlibc.ppTLS, aaframe ppuintptr, aacount ppint32, aahash tn__u32) {

	var aam, aap, aaq ppuintptr
	pp_, pp_, pp_ = aam, aap, aaq

	aap = (*tsdx_frame)(iqunsafe.ppPointer(aaframe)).fdentries + ppuintptr(1)*8
	aaq = (*tsdx_frame)(iqunsafe.ppPointer(aaframe)).fdentries + ppuintptr(aacount)*8 - ppuintptr(1)*8
	for aap <= aaq {
		aam = aap + ppuintptr((ppint64(aaq)-ppint64(aap))/8/ppint64(2))*8
		if (*tsext2_dx_entry)(iqunsafe.ppPointer(aam)).fdhash > aahash {
			aaq = aam - ppuintptr(1)*8
		} else {
			aap = aam + ppuintptr(1)*8
		}
	}
	(*tsdx_frame)(iqunsafe.ppPointer(aaframe)).fdat = aap - ppuintptr(1)*8
}

func siload_logical_dir_block(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t, aadiri ppuintptr, aablock tnblk64_t, aapblk ppuintptr, aabuf ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aaerrcode tnerrcode_t
	var pp_ /* ret_flags at bp+0 */ ppint32
	pp_ = aaerrcode

	aaerrcode = Xext2fs_bmap2(cgtls, aafs, aadir, aadiri, iqlibc.ppUintptrFromInt32(0), 0, aablock, cgbp, aapblk)
	if aaerrcode != 0 {
		return aaerrcode
	}
	if *(*ppint32)(iqunsafe.ppPointer(cgbp))&ppint32(mvBMAP_RET_UNINIT) != 0 {
		return iqlibc.ppInt64FromInt64(2133571363)
	}
	return Xext2fs_read_dir_block4(cgtls, aafs, *(*tnblk64_t)(iqunsafe.ppPointer(aapblk)), aabuf, 0, aadir)
}

func sidx_lookup(cgtls *iqlibc.ppTLS, aafs1 tnext2_filsys, aadir tnext2_ino_t, aadiri ppuintptr, aainfo ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aacount, aahash_alg, aahash_flags, aalevel, aalimit, ccv1 ppint32
	var aaerrcode tnerrcode_t
	var aaframe, aaroot ppuintptr
	var ccv3 ppuint32
	var pp_ /* minor_hash at bp+0 */ tn__u32
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aacount, aaerrcode, aaframe, aahash_alg, aahash_flags, aalevel, aalimit, aaroot, ccv1, ccv3
	aalevel = 0
	aahash_flags = iqlibc.ppInt32FromUint32((*tsext2_inode)(iqunsafe.ppPointer(aadiri)).fdi_flags & ppuint32(mvEXT4_CASEFOLD_FL))

	aaerrcode = sialloc_dx_frame(cgtls, aafs1, aainfo+24)
	if aaerrcode != 0 {
		return aaerrcode
	}
	(*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdlevels = ppuint32(1)

	aaerrcode = siload_logical_dir_block(cgtls, aafs1, aadir, aadiri, ppuint64(0), aainfo+24+8, (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24))).fdbuf)
	if aaerrcode != 0 {
		goto ppout_err
	}
	aaroot = (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24))).fdbuf + iqlibc.ppUintptrFromInt32(mvEXT2_DX_ROOT_OFF)
	aahash_alg = iqlibc.ppInt32FromUint8((*tsext2_dx_root_info)(iqunsafe.ppPointer(aaroot)).fdhash_version)
	if aahash_alg != ppint32(mvEXT2_HASH_TEA) && aahash_alg != ppint32(mvEXT2_HASH_HALF_MD4) && aahash_alg != mvEXT2_HASH_LEGACY {
		aaerrcode = iqlibc.ppInt64FromInt64(2133571417)
		goto ppout_err
	}
	if aahash_alg <= ppint32(mvEXT2_HASH_TEA) && (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdsuper)).fds_flags&ppuint32(mvEXT2_FLAGS_UNSIGNED_HASH) != 0 {
		aahash_alg = aahash_alg + ppint32(3)
	}
	ccv1 = iqlibc.ppBoolInt32((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdsuper)).fds_feature_incompat&ppuint32(mvEXT4_FEATURE_INCOMPAT_LARGEDIR) != ppuint32(0))
	goto cg_2
cg_2:
	if ccv1 != 0 {
		ccv3 = ppuint32(mvEXT4_HTREE_LEVEL)
		goto cg_4
	}
	ccv3 = ppuint32(mvEXT4_HTREE_LEVEL_COMPAT)
	goto cg_4
cg_4:
	if ppuint32((*tsext2_dx_root_info)(iqunsafe.ppPointer(aaroot)).fdindirect_levels) >= ccv3 {
		aaerrcode = iqlibc.ppInt64FromInt64(2133571363)
		goto ppout_err
	}
	(*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdhash_alg = aahash_alg

	aaerrcode = Xext2fs_dirhash2(cgtls, aahash_alg, (*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdname, (*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdnamelen, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdencoding, aahash_flags, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdsuper+236, aainfo+16, cgbp)
	if aaerrcode != 0 {
		goto ppout_err
	}
	...
	aalevel = 0
	for {
		if !(aalevel <= iqlibc.ppInt32FromUint8((*tsext2_dx_root_info)(iqunsafe.ppPointer(aaroot)).fdindirect_levels)) {
			break
		}
		aaframe = aainfo + 24 + ppuintptr(aalevel)*40
		if aalevel > 0 {
			aaerrcode = sialloc_dx_frame(cgtls, aafs1, aaframe)
			if aaerrcode != 0 {
				goto ppout_err
			}
			(*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdlevels = (*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdlevels + 1

			aaerrcode = siload_logical_dir_block(cgtls, aafs1, aadir, aadiri, ppuint64((*tsext2_dx_entry)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aalevel-ppint32(1))*40))).fdat)).fdblock&ppuint32(0x0fffffff)), aaframe+8, (*tsdx_frame)(iqunsafe.ppPointer(aaframe)).fdbuf)
			if aaerrcode != 0 {
				goto ppout_err
			}
		}
		aaerrcode = Xext2fs_get_dx_countlimit(cgtls, aafs1, (*tsdx_frame)(iqunsafe.ppPointer(aaframe)).fdbuf, aaframe+16, iqlibc.ppUintptrFromInt32(0))
		if aaerrcode != 0 {
			goto ppout_err
		}
		aacount = iqlibc.ppInt32FromUint16((*tsext2_dx_countlimit)(iqunsafe.ppPointer((*tsdx_frame)(iqunsafe.ppPointer(aaframe)).fdhead)).fdcount)
		aalimit = iqlibc.ppInt32FromUint16((*tsext2_dx_countlimit)(iqunsafe.ppPointer((*tsdx_frame)(iqunsafe.ppPointer(aaframe)).fdhead)).fdlimit)
		(*tsdx_frame)(iqunsafe.ppPointer(aaframe)).fdentries = (*tsdx_frame)(iqunsafe.ppPointer(aaframe)).fdhead
		if !(aacount != 0) || aacount > aalimit {
			aaerrcode = iqlibc.ppInt64FromInt64(2133571363)
			goto ppout_err
		}

		sidx_search_entry(cgtls, aaframe, aacount, (*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdhash)
...
ppout_err:
	;
	sidx_release(cgtls, aainfo)
	return aaerrcode
}

type tslink_struct = struct {
	fdfs        tnext2_filsys
	fdname      ppuintptr
	fdnamelen   ppint32
	fdinode     tnext2_ino_t
	fdflags     ppint32
	fddone      ppint32
	fdblocksize ppuint32
	fderr       tnerrcode_t
...
func silink_proc(cgtls *iqlibc.ppTLS, aadir tnext2_ino_t, aaentru ppint32, aadirent ppuintptr, aaoffset ppint32, aablocksize ppint32, aabuf ppuintptr, aapriv_data ppuintptr) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aacsum_size, aarec_len, aaret, ccv12, ccv3, ccv5 ppint32
	var aals, aanext, ccv10, ccv11, ccv14, ccv9 ppuintptr
	var aamin_rec_len, aarec_len1, ccv1, ccv7 ppuint32
	var pp_ /* curr_rec_len at bp+0 */ ppuint32
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aacsum_size, aals, aamin_rec_len, aanext, aarec_len, aarec_len1, aaret, ccv1, ccv10, ccv11, ccv12, ccv14, ccv3, ccv5, ccv7, ccv9
	aals = aapriv_data
	aaret = 0
	aacsum_size = 0

	if (*tslink_struct)(iqunsafe.ppPointer(aals)).fddone != 0 {
		return ppint32(mvDIRENT_ABORT)
	}

	aarec_len = iqlibc.ppInt32FromUint8(iqlibc.ppUint8FromInt32((*tslink_struct)(iqunsafe.ppPointer(aals)).fdnamelen)) + ppint32(mvEXT2_DIR_ENTRY_HEADER_LEN) + (iqlibc.ppInt32FromInt32(mvEXT2_DIR_PAD) - iqlibc.ppInt32FromInt32(1))
	aarec_len = aarec_len & ^(iqlibc.ppInt32FromInt32(mvEXT2_DIR_PAD) - iqlibc.ppInt32FromInt32(1))
	if 0 != 0 {
		aarec_len = aarec_len + ppint32(mvEXT2_DIR_ENTRY_HASH_LEN)
	}
	ccv1 = iqlibc.ppUint32FromInt32(aarec_len)
	goto cg_2
cg_2:
	aarec_len1 = ccv1

	(*tslink_struct)(iqunsafe.ppPointer(aals)).fderr = Xext2fs_get_rec_len(cgtls, (*tslink_struct)(iqunsafe.ppPointer(aals)).fdfs, aadirent, cgbp)
	if (*tslink_struct)(iqunsafe.ppPointer(aals)).fderr != 0 {
		return ppint32(mvDIRENT_ABORT)
	}

	ccv3 = iqlibc.ppBoolInt32((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer((*tslink_struct)(iqunsafe.ppPointer(aals)).fdfs)).fdsuper)).fds_feature_ro_compat&ppuint32(mvEXT4_FEATURE_RO_COMPAT_METADATA_CSUM) != ppuint32(0))
	goto cg_4
cg_4:
	if ccv3 != 0 {
		aacsum_size = ppint32(12)
	}
	/*
	 * See if the following directory entry (if any) is unused;
	 * if so, absorb it into this one.
	 */
	aanext = aabuf + ppuintptr(aaoffset) + ppuintptr(*(*ppuint32)(iqunsafe.ppPointer(cgbp)))
	if aaoffset+iqlibc.ppInt32FromUint32(*(*ppuint32)(iqunsafe.ppPointer(cgbp))) < aablocksize-(ppint32(8)+aacsum_size) && (*tsext2_dir_entry)(iqunsafe.ppPointer(aanext)).fdinode == ppuint32(0) && aaoffset+iqlibc.ppInt32FromUint32(*(*ppuint32)(iqunsafe.ppPointer(cgbp)))+iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(aanext)).fdrec_len) <= aablocksize {
		*(*ppuint32)(iqunsafe.ppPointer(cgbp)) = *(*ppuint32)(iqunsafe.ppPointer(cgbp)) + ppuint32((*tsext2_dir_entry)(iqunsafe.ppPointer(aanext)).fdrec_len)
		(*tslink_struct)(iqunsafe.ppPointer(aals)).fderr = Xext2fs_set_rec_len(cgtls, (*tslink_struct)(iqunsafe.ppPointer(aals)).fdfs, *(*ppuint32)(iqunsafe.ppPointer(cgbp)), aadirent)
		if (*tslink_struct)(iqunsafe.ppPointer(aals)).fderr != 0 {
			return ppint32(mvDIRENT_ABORT)
		}
		aaret = ppint32(mvDIRENT_CHANGED)
	}
	 ...
	 */
	if (*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdinode != 0 {
		ccv3 = iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdname_len) & ppint32(0xff)
		goto cg_6
	cg_6:
		aarec_len = iqlibc.ppInt32FromUint8(iqlibc.ppUint8FromInt32(ccv3)) + ppint32(mvEXT2_DIR_ENTRY_HEADER_LEN) + (iqlibc.ppInt32FromInt32(mvEXT2_DIR_PAD) - iqlibc.ppInt32FromInt32(1))
		aarec_len = aarec_len & ^(iqlibc.ppInt32FromInt32(mvEXT2_DIR_PAD) - iqlibc.ppInt32FromInt32(1))
		if 0 != 0 {
			aarec_len = aarec_len + ppint32(mvEXT2_DIR_ENTRY_HASH_LEN)
		}
	...
	cg_8:
		aamin_rec_len = ccv1
		if *(*ppuint32)(iqunsafe.ppPointer(cgbp)) < aamin_rec_len+aarec_len1 {
			return aaret
		}
		aarec_len1 = *(*ppuint32)(iqunsafe.ppPointer(cgbp)) - aamin_rec_len
		(*tslink_struct)(iqunsafe.ppPointer(aals)).fderr = Xext2fs_set_rec_len(cgtls, (*tslink_struct)(iqunsafe.ppPointer(aals)).fdfs, aamin_rec_len, aadirent)
		if (*tslink_struct)(iqunsafe.ppPointer(aals)).fderr != 0 {
			return ppint32(mvDIRENT_ABORT)
		}
		aanext = aabuf + ppuintptr(aaoffset) + ppuintptr((*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdrec_len)
		(*tsext2_dir_entry)(iqunsafe.ppPointer(aanext)).fdinode = ppuint32(0)
		ccv9 = aanext
		(*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len = iqlibc.ppUint16FromInt32(iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len)&ppint32(0xff00) | 0&ppint32(0xff))
		ccv9 = aanext
		(*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len = iqlibc.ppUint16FromInt32(iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len)&ppint32(0xff) | 0<<ppint32(8))
		(*tslink_struct)(iqunsafe.ppPointer(aals)).fderr = Xext2fs_set_rec_len(cgtls, (*tslink_struct)(iqunsafe.ppPointer(aals)).fdfs, aarec_len1, aanext)
		if (*tslink_struct)(iqunsafe.ppPointer(aals)).fderr != 0 {
			return ppint32(mvDIRENT_ABORT)
		}
		return ppint32(mvDIRENT_CHANGED)
	}
	 ...
	 */
	if *(*ppuint32)(iqunsafe.ppPointer(cgbp)) < aarec_len1 {
		return aaret
	}
	(*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdinode = (*tslink_struct)(iqunsafe.ppPointer(aals)).fdinode
	ccv9 = aadirent
	(*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len = iqlibc.ppUint16FromInt32(iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len)&ppint32(0xff00) | (*tslink_struct)(iqunsafe.ppPointer(aals)).fdnamelen&ppint32(0xff))
	Xstrncpy(cgtls, aadirent+8, (*tslink_struct)(iqunsafe.ppPointer(aals)).fdname, iqlibc.ppUint64FromInt32((*tslink_struct)(iqunsafe.ppPointer(aals)).fdnamelen))
	ccv3 = iqlibc.ppBoolInt32((*tsext2_super_block)(iqunsafe.ppPointer((*tslink_struct)(iqunsafe.ppPointer(aals)).fdsb)).fds_feature_incompat&ppuint32(mvEXT2_FEATURE_INCOMPAT_FILETYPE) != ppuint32(0))
	goto cg_13
cg_13:
	if ccv3 != 0 {
		ccv9 = aadirent
		(*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len = iqlibc.ppUint16FromInt32(iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len)&ppint32(0xff) | (*tslink_struct)(iqunsafe.ppPointer(aals)).fdflags&ppint32(mvEXT2FS_LINK_FT_MASK)<<ppint32(8))
	}

	(*tslink_struct)(iqunsafe.ppPointer(aals)).fddone = (*tslink_struct)(iqunsafe.ppPointer(aals)).fddone + 1
	return iqlibc.ppInt32FromInt32(mvDIRENT_ABORT) | iqlibc.ppInt32FromInt32(mvDIRENT_CHANGED)
}

func siadd_dirent_to_buf(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aablockcnt tne2_blkcnt_t, aabuf ppuintptr, aadir tnext2_ino_t, aadiri ppuintptr, aaname ppuintptr, aaino tnext2_ino_t, aaflags ppint32, aapblkp ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(112)
	defer cgtls.ppFree(112)

	var aaretval tnerrcode_t
	var pp_ /* ctx at bp+0 */ tsdir_context
	var pp_ /* ls at bp+48 */ tslink_struct
	pp_ = aaretval

	aaretval = siload_logical_dir_block(cgtls, aafs, aadir, aadiri, iqlibc.ppUint64FromInt64(aablockcnt), aapblkp, aabuf)
	if aaretval != 0 {
		return aaretval
	}
	(*(*tsdir_context)(iqunsafe.ppPointer(cgbp))).fderrcode = 0
	(*(*tsdir_context)(iqunsafe.ppPointer(cgbp))).fdfunc1 = pp__ccgo_fp(silink_proc)
	(*(*tsdir_context)(iqunsafe.ppPointer(cgbp))).fddir = aadir
	(*(*tsdir_context)(iqunsafe.ppPointer(cgbp))).fdflags = ppint32(mvDIRENT_FLAG_INCLUDE_EMPTY)
	(*(*tsdir_context)(iqunsafe.ppPointer(cgbp))).fdbuf = aabuf
	(*(*tsdir_context)(iqunsafe.ppPointer(cgbp))).fdpriv_data = cgbp + 48

	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fdfs = aafs
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fdname = aaname
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fdnamelen = iqlibc.ppInt32FromUint64(Xstrlen(cgtls, aaname))
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fdinode = aaino
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fdflags = aaflags
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fddone = 0
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fdsb = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fdblocksize = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fderr = 0

	Xext2fs_process_dir_block(cgtls, aafs, aapblkp, aablockcnt, ppuint64(0), 0, cgbp)
	if (*(*tsdir_context)(iqunsafe.ppPointer(cgbp))).fderrcode != 0 {
		return (*(*tsdir_context)(iqunsafe.ppPointer(cgbp))).fderrcode
	}
	if (*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fderr != 0 {
		return (*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fderr
	}
	if !((*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 48))).fddone != 0) {
		return iqlibc.ppInt64FromInt64(2133571366)
	}
	return 0
}

type tsdx_hash_map = struct {
	fdhash tn__u32
	fdsize ppint32
	fdoff  ppint32
}

func sidx_hash_map_cmp(cgtls *iqlibc.ppTLS, aaap ppuintptr, aabp ppuintptr) (cgr ppint32) {

	var aaa, aab ppuintptr
	pp_, pp_ = aaa, aab
	aaa = aaap
	aab = aabp

	if (*tsdx_hash_map)(iqunsafe.ppPointer(aaa)).fdhash < (*tsdx_hash_map)(iqunsafe.ppPointer(aab)).fdhash {
		return -ppint32(1)
	}
	if (*tsdx_hash_map)(iqunsafe.ppPointer(aaa)).fdhash > (*tsdx_hash_map)(iqunsafe.ppPointer(aab)).fdhash {
		return ppint32(1)
...
func sidx_move_dirents(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aamap ppuintptr, aacount ppint32, aafrom ppuintptr, aato ppuintptr) (cgr tnerrcode_t) {

	var aabase, aade ppuintptr
	var aacsum_size, aai, aarec_len, aarec_len1, ccv1, ccv4 ppint32
	var aaretval tnerrcode_t
	var ccv6 ppuint32
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aabase, aacsum_size, aade, aai, aarec_len, aarec_len1, aaretval, ccv1, ccv4, ccv6
	aarec_len1 = 0
	aacsum_size = 0
	aabase = aato
...
cg_2:
	...
	for {
		...
		aade = aafrom + ppuintptr((*(*tsdx_hash_map)(iqunsafe.ppPointer(aamap + ppuintptr(aai)*12))).fdoff)
		ccv1 = iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(aade)).fdname_len) & ppint32(0xff)
		goto cg_5
	cg_5:
		aarec_len = iqlibc.ppInt32FromUint8(iqlibc.ppUint8FromInt32(ccv1)) + ppint32(mvEXT2_DIR_ENTRY_HEADER_LEN) + (iqlibc.ppInt32FromInt32(mvEXT2_DIR_PAD) - iqlibc.ppInt32FromInt32(1))
		aarec_len = aarec_len & ^(iqlibc.ppInt32FromInt32(mvEXT2_DIR_PAD) - iqlibc.ppInt32FromInt32(1))
		if 0 != 0 {
			aarec_len = aarec_len + ppint32(mvEXT2_DIR_ENTRY_HASH_LEN)
		}
		ccv6 = iqlibc.ppUint32FromInt32(aarec_len)
		goto cg_7
	cg_7:
		aarec_len1 = iqlibc.ppInt32FromUint32(ccv6)
		Xmemcpy(cgtls, aato, aade, iqlibc.ppUint64FromInt32(aarec_len1))
		aaretval = Xext2fs_set_rec_len(cgtls, aafs, iqlibc.ppUint32FromInt32(aarec_len1), aato)
		if aaretval != 0 {
			return aaretval
		}
		aato = aato + ppuintptr(aarec_len1)
	 ...
	 */
	aato = aato - ppuintptr(aarec_len1)
	aarec_len1 = ppint32(iqlibc.ppInt64FromUint32((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize) - (ppint64(aato) - ppint64(aabase)) - ppint64(aacsum_size))
	aaretval = Xext2fs_set_rec_len(cgtls, aafs, iqlibc.ppUint32FromInt32(aarec_len1), aato)
	if aaretval != 0 {
		return aaretval
	}
	if aacsum_size != 0 {
		Xext2fs_initialize_dirent_tail(cgtls, aafs, aabase+ppuintptr((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize)-iqlibc.ppUintptrFromInt64(12))
	}
	return 0
}

func sidx_insert_entry(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t, aainfo ppuintptr, aalevel ppint32, aahash tn__u32, aalblk tnblk64_t) (cgr tnerrcode_t) {

	var aanew, aatop ppuintptr
	var aapcount ppint32
	pp_, pp_, pp_ = aanew, aapcount, aatop

	aapcount = iqlibc.ppInt32FromUint16((*tsext2_dx_countlimit)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aalevel)*40))).fdhead)).fdcount)
	aatop = (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aalevel)*40))).fdentries + ppuintptr(aapcount)*8
	aanew = (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aalevel)*40))).fdat + ppuintptr(1)*8
	Xmemmove(cgtls, aanew+ppuintptr(1)*8, aanew, iqlibc.ppUint64FromInt64(ppint64(aatop)-ppint64(aanew)))
	(*tsext2_dx_entry)(iqunsafe.ppPointer(aanew)).fdhash = aahash
	(*tsext2_dx_entry)(iqunsafe.ppPointer(aanew)).fdblock = ppuint32(aalblk)
	(*tsext2_dx_countlimit)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aalevel)*40))).fdhead)).fdcount = iqlibc.ppUint16FromInt32(aapcount + iqlibc.ppInt32FromInt32(1))
	return Xext2fs_write_dir_block4(cgtls, aafs, (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aalevel)*40))).fdpblock, (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aalevel)*40))).fdbuf, 0, aadir)
}

func sidx_split_leaf(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t, aadiri ppuintptr, aainfo ppuintptr, aabuf ppuintptr, aaleaf_pblk tnblk64_t, aanew_lblk tnblk64_t, aanew_pblk tnblk64_t) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(48)
	defer cgtls.ppFree(48)

	var aacontinued, aacount1, aahash_flags, aai, ccv12, ccv14 ppint32
	var aade, ccv17, ccv19, ccv9 ppuintptr
	var aamove_size, aaoffset ppuint32
	var aaretval, ccv1, ccv5, ccv7 tnerrcode_t
	var ccv3, ccv4 ppuint64
	var pp_ /* buf2 at bp+16 */ ppuintptr
	var pp_ /* map at bp+32 */ ppuintptr
	var pp_ /* minor_hash at bp+40 */ tn__u32
	var pp_ /* p at bp+8 */ ppuintptr
	var pp_ /* pp at bp+0 */ ppuintptr
	var pp_ /* rec_len at bp+24 */ ppuint32
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aacontinued, aacount1, aade, aahash_flags, aai, aamove_size, aaoffset, aaretval, ccv1, ccv12, ccv14, ccv17, ccv19, ccv3, ccv4, ccv5, ccv7, ccv9
	aahash_flags = iqlibc.ppInt32FromUint32((*tsext2_inode)(iqunsafe.ppPointer(aadiri)).fdi_flags & ppuint32(mvEXT4_CASEFOLD_FL))
	aaretval = 0
	aacount1 = 0
...
cg_2:
	...
	ccv3 = ppuint64((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize / ppuint32(12))
	ccv4 = ppuint64(12)
	if ccv3 != 0 && ^iqlibc.ppUint64FromUint64(0)/ccv3 < ccv4 {
		ccv1 = iqlibc.ppInt64FromInt64(2133571398)
		goto cg_6
	}
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, ccv3*ccv4)
	if !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) != 0) {
...
cg_6:
	...
	aaoffset = ppuint32(0)
	for {
		if !(aaoffset < (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize) {
			break
		}
		aade = aabuf + ppuintptr(aaoffset)
		aaretval = Xext2fs_get_rec_len(cgtls, aafs, aade, cgbp+24)
		if aaretval != 0 {
			goto ppout
		}
		ccv12 = iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(aade)).fdname_len) & ppint32(0xff)
		goto cg_13
	cg_13:
		;
		if ccv12 > 0 && (*tsext2_dir_entry)(iqunsafe.ppPointer(aade)).fdinode != 0 {
			(*(*tsdx_hash_map)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32)) + ppuintptr(aacount1)*12))).fdoff = iqlibc.ppInt32FromUint32(aaoffset)
			(*(*tsdx_hash_map)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32)) + ppuintptr(aacount1)*12))).fdsize = iqlibc.ppInt32FromUint32(*(*ppuint32)(iqunsafe.ppPointer(cgbp + 24)))
			ccv12 = iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(aade)).fdname_len) & ppint32(0xff)
			goto cg_15
		cg_15:
			aaretval = Xext2fs_dirhash2(cgtls, (*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdhash_alg, aade+8, ccv12, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdencoding, aahash_flags, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper+236, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32))+ppuintptr(aacount1)*12, cgbp+40)
			if aaretval != 0 {
				goto ppout
			}
			aacount1 = aacount1 + 1
	...
	cg_11:
		;
		aaoffset = aaoffset + *(*ppuint32)(iqunsafe.ppPointer(cgbp + 24))
	}
	Xqsort(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32)), iqlibc.ppUint64FromInt32(aacount1), ppuint64(12), pp__ccgo_fp(sidx_hash_map_cmp))
	aamove_size = ppuint32(0)
	/* Find place to split block */
	aai = aacount1 - ppint32(1)
	for {
		if !(aai >= 0) {
			break
		}
		if aamove_size+iqlibc.ppUint32FromInt32((*(*tsdx_hash_map)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32)) + ppuintptr(aai)*12))).fdsize/ppint32(2)) > (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize/ppuint32(2) {
			break
		}
		aamove_size = aamove_size + iqlibc.ppUint32FromInt32((*(*tsdx_hash_map)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32)) + ppuintptr(aai)*12))).fdsize)
	...
	aaretval = sidx_move_dirents(cgtls, aafs, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32))+ppuintptr(aai)*12, aacount1-aai, aabuf, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)))
	if aaretval != 0 {
		goto ppout
	}
	aaretval = Xext2fs_write_dir_block4(cgtls, aafs, aanew_pblk, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)), 0, aadir)
	if aaretval != 0 {
		goto ppout
	}
	/* Repack remaining entries in the old block */
	aaretval = sidx_move_dirents(cgtls, aafs, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32)), aai, aabuf, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)))
	if aaretval != 0 {
		goto ppout
	}
	aaretval = Xext2fs_write_dir_block4(cgtls, aafs, aaleaf_pblk, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)), 0, aadir)
	if aaretval != 0 {
		goto ppout
	}
	/* Update parent node */
	aacontinued = iqlibc.ppBoolInt32((*(*tsdx_hash_map)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32)) + ppuintptr(aai)*12))).fdhash == (*(*tsdx_hash_map)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32)) + ppuintptr(aai-ppint32(1))*12))).fdhash)
	aaretval = sidx_insert_entry(cgtls, aafs, aadir, aainfo, iqlibc.ppInt32FromUint32((*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdlevels-ppuint32(1)), (*(*tsdx_hash_map)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 32)) + ppuintptr(aai)*12))).fdhash+iqlibc.ppUint32FromInt32(aacontinued), aanew_lblk)
	goto ppout
ppout:
	;
	ccv9 = cgbp + 16
...
func sidx_grow_tree(cgtls *iqlibc.ppTLS, aafs1 tnext2_filsys, aadir tnext2_ino_t, aadiri ppuintptr, aainfo ppuintptr, aabuf ppuintptr, aaleaf_pblk tnblk64_t) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aacount, aacount1, aacount2, aacsum_size, aai, ccv1, ccv4 ppint32
	var aade, aahead, aaroot, ccv10, ccv9 ppuintptr
	var aalblk tnblk64_t
	var aaretval tnerrcode_t
	var aasize tnext2_off64_t
	var aasplit_hash tn__u32
	var ccv6 ppuint32
	var ccv8 ppbool
	var pp_ /* pblk at bp+0 */ tnblk64_t
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aacount, aacount1, aacount2, aacsum_size, aade, aahead, aai, aalblk, aaretval, aaroot, aasize, aasplit_hash, ccv1, ccv10, ccv4, ccv6, ccv8, ccv9
	aasize = iqlibc.ppInt64FromUint64(ppuint64((*tsext2_inode)(iqunsafe.ppPointer(aadiri)).fdi_size) | ppuint64((*tsext2_inode)(iqunsafe.ppPointer(aadiri)).fdi_size_high)<<iqlibc.ppInt32FromInt32(32))
	aacsum_size = 0

	ccv1 = iqlibc.ppBoolInt32((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdsuper)).fds_feature_ro_compat&ppuint32(mvEXT4_FEATURE_RO_COMPAT_METADATA_CSUM) != ppuint32(0))
	goto cg_2
cg_2:
	if ccv1 != 0 {
		aacsum_size = ppint32(8)
	}

	/* Find level which can accommodate new child */
	aai = iqlibc.ppInt32FromUint32((*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdlevels - ppuint32(1))
	for {
		if !(aai >= 0) {
			break
		}
		if iqlibc.ppInt32FromUint16((*tsext2_dx_countlimit)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aai)*40))).fdhead)).fdcount) < iqlibc.ppInt32FromUint16((*tsext2_dx_countlimit)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aai)*40))).fdhead)).fdlimit) {
			break
		}
		goto cg_3
	cg_3:
	...
	cg_5:
		if ccv1 != 0 {
			ccv6 = ppuint32(mvEXT4_HTREE_LEVEL)
			goto cg_7
		}
		ccv6 = ppuint32(mvEXT4_HTREE_LEVEL_COMPAT)
		goto cg_7
	cg_7:
	}
	if ccv8 && (*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdlevels >= ccv6 {
		return iqlibc.ppInt64FromInt64(2133571366)
	}
	aalblk = iqlibc.ppUint64FromInt64(aasize / iqlibc.ppInt64FromUint32((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdblocksize))
	aasize = aasize + iqlibc.ppInt64FromUint32((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdblocksize)
	aaretval = Xext2fs_inode_size_set(cgtls, aafs1, aadiri, aasize)
	if aaretval != 0 {
		return aaretval
	}
	aaretval = Xext2fs_fallocate(cgtls, aafs1, iqlibc.ppInt32FromInt32(mvEXT2_FALLOCATE_FORCE_INIT)|iqlibc.ppInt32FromInt32(mvEXT2_FALLOCATE_ZERO_BLOCKS), aadir, aadiri, ppuint64(0), aalblk, ppuint64(1))
	if aaretval != 0 {
		return aaretval
	}
	aaretval = Xext2fs_write_inode(cgtls, aafs1, aadir, aadiri)
	if aaretval != 0 {
		return aaretval
	}
	aaretval = Xext2fs_bmap2(cgtls, aafs1, aadir, aadiri, iqlibc.ppUintptrFromInt32(0), 0, aalblk, iqlibc.ppUintptrFromInt32(0), cgbp)
	if aaretval != 0 {
		return aaretval
	}
	/* Only leaf addition needed? */
	if aai == iqlibc.ppInt32FromUint32((*tsdx_lookup_info)(iqunsafe.ppPointer(aainfo)).fdlevels)-ppint32(1) {
		return sidx_split_leaf(cgtls, aafs1, aadir, aadiri, aainfo, aabuf, aaleaf_pblk, aalblk, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp)))
	}

	aade = aabuf
	(*tsext2_dir_entry)(iqunsafe.ppPointer(aade)).fdinode = ppuint32(0)
	ccv9 = aade
	(*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len = iqlibc.ppUint16FromInt32(iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len)&ppint32(0xff00) | 0&ppint32(0xff))
	ccv9 = aade
	(*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len = iqlibc.ppUint16FromInt32(iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(ccv9)).fdname_len)&ppint32(0xff) | 0<<ppint32(8))
	aaretval = Xext2fs_set_rec_len(cgtls, aafs1, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdblocksize, aade)
	if aaretval != 0 {
		return aaretval
	}
	aahead = aabuf + iqlibc.ppUintptrFromInt32(8)
	aacount = iqlibc.ppInt32FromUint16((*tsext2_dx_countlimit)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aai+ppint32(1))*40))).fdhead)).fdcount)
	/* Growing tree depth? */
	if aai < 0 {

		Xmemcpy(cgtls, aahead, (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24))).fdentries, iqlibc.ppUint64FromInt32(aacount)*ppuint64(8))
		(*tsext2_dx_countlimit)(iqunsafe.ppPointer(aahead)).fdlimit = ppuint16(ppuint64((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdblocksize-iqlibc.ppUint32FromInt32(iqlibc.ppInt32FromInt32(8)+aacsum_size)) / iqlibc.ppUint64FromInt64(8))
		/* head->count gets set by memcpy above to correct value */

		/* Now update tree root */
		(*tsext2_dx_countlimit)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24))).fdhead)).fdcount = iqlibc.ppUint16FromInt32(iqlibc.ppInt32FromInt32(1))
		(*(*tsext2_dx_entry)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24))).fdentries))).fdblock = ppuint32(aalblk)
		aaroot = (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24))).fdbuf + iqlibc.ppUintptrFromInt32(mvEXT2_DX_ROOT_OFF)
		(*tsext2_dx_root_info)(iqunsafe.ppPointer(aaroot)).fdindirect_levels = (*tsext2_dx_root_info)(iqunsafe.ppPointer(aaroot)).fdindirect_levels + 1
	} else {
		/* Splitting internal node in two */
		aacount1 = aacount / ppint32(2)
		aacount2 = aacount - aacount1
		aasplit_hash = (*(*tsext2_dx_entry)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aai+ppint32(1))*40))).fdentries + ppuintptr(aacount1)*8))).fdhash

		Xmemcpy(cgtls, aahead, (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aai+ppint32(1))*40))).fdentries+ppuintptr(aacount1)*8, iqlibc.ppUint64FromInt32(aacount2)*ppuint64(8))
		(*tsext2_dx_countlimit)(iqunsafe.ppPointer(aahead)).fdcount = iqlibc.ppUint16FromInt32(aacount2)
		(*tsext2_dx_countlimit)(iqunsafe.ppPointer(aahead)).fdlimit = ppuint16(ppuint64((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdblocksize-iqlibc.ppUint32FromInt32(iqlibc.ppInt32FromInt32(8)+aacsum_size)) / iqlibc.ppUint64FromInt64(8))
		(*tsext2_dx_countlimit)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aai+ppint32(1))*40))).fdhead)).fdcount = iqlibc.ppUint16FromInt32(aacount1)

		/* Update parent node */
		aaretval = sidx_insert_entry(cgtls, aafs1, aadir, aainfo, aai, aasplit_hash, aalblk)
		if aaretval != 0 {
			return aaretval
		}
	...
	aaretval = Xext2fs_write_dir_block4(cgtls, aafs1, (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aai+ppint32(1))*40))).fdpblock, (*(*tsdx_frame)(iqunsafe.ppPointer(aainfo + 24 + ppuintptr(aai+ppint32(1))*40))).fdbuf, 0, aadir)
	if aaretval != 0 {
		return aaretval
	}
	/* Writeout new tree block */
	aaretval = Xext2fs_write_dir_block4(cgtls, aafs1, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp)), aabuf, 0, aadir)
	if aaretval != 0 {
		return aaretval
	}
	return 0
...
func sidx_link(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t, aadiri ppuintptr, aaname ppuintptr, aaino tnext2_ino_t, aaflags ppint32) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(176)
	defer cgtls.ppFree(176)

	var aarestart ppuint32
	var aaretval, ccv1 tnerrcode_t
	var ccv3 ppuintptr
	var pp_ /* blockbuf at bp+160 */ ppuintptr
	var pp_ /* dx_info at bp+16 */ tsdx_lookup_info
	var pp_ /* leaf_pblk at bp+168 */ tnblk64_t
	var pp_ /* p at bp+8 */ ppuintptr
	var pp_ /* pp at bp+0 */ ppuintptr
	pp_, pp_, pp_, pp_ = aarestart, aaretval, ccv1, ccv3
	aarestart = ppuint32(0)
...
cg_2:
	...
	(*(*tsdx_lookup_info)(iqunsafe.ppPointer(cgbp + 16))).fdname = aaname
	(*(*tsdx_lookup_info)(iqunsafe.ppPointer(cgbp + 16))).fdnamelen = iqlibc.ppInt32FromUint64(Xstrlen(cgtls, aaname))
	goto ppagain
ppagain:
	;
	aaretval = sidx_lookup(cgtls, aafs, aadir, aadiri, cgbp+16)
	if aaretval != 0 {
		goto ppfree_buf
	}

	aaretval = siadd_dirent_to_buf(cgtls, aafs, iqlibc.ppInt64FromUint32((*tsext2_dx_entry)(iqunsafe.ppPointer((*(*tsdx_frame)(iqunsafe.ppPointer(cgbp + 16 + 24 + ppuintptr((*(*tsdx_lookup_info)(iqunsafe.ppPointer(cgbp + 16))).fdlevels-ppuint32(1))*40))).fdat)).fdblock&ppuint32(0x0fffffff)), *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 160)), aadir, aadiri, aaname, aaino, aaflags, cgbp+168)
	/*
	 ...
	 * new leaf block.
	 */
	if aarestart >= (*(*tsdx_lookup_info)(iqunsafe.ppPointer(cgbp + 16))).fdlevels || aaretval != ppint64(2133571366) {
		goto ppfree_frames
	}
	aaretval = sidx_grow_tree(cgtls, aafs, aadir, aadiri, cgbp+16, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 160)), *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 168)))
	if aaretval != 0 {
		goto ppfree_frames
	}
	/* Restart everything now that the tree is larger */
	aarestart = aarestart + 1
	sidx_release(cgtls, cgbp+16)
	goto ppagain
	goto ppfree_frames
ppfree_frames:
	;
...
//	 * entry filetype.
//	 */
func Xext2fs_link(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t, aaname ppuintptr, aaino tnext2_ino_t, aaflags ppint32) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(256)
	defer cgtls.ppFree(256)

	var aalblk tnblk64_t
	var aaretval, ccv1, ccv3 tnerrcode_t
	var ccv2 ppuint64
	var ccv5 ppuintptr
	var pp_ /* ctx at bp+208 */ tsdir_context
	var pp_ /* inode at bp+72 */ tsext2_inode
	var pp_ /* ls at bp+16 */ tslink_struct
	var pp_ /* p at bp+8 */ ppuintptr
	var pp_ /* pblk at bp+200 */ tnblk64_t
	var pp_ /* pp at bp+0 */ ppuintptr
	pp_, pp_, pp_, pp_, pp_, pp_ = aalblk, aaretval, ccv1, ccv2, ccv3, ccv5
...
ppretry:
	;
	ccv1 = Xext2fs_read_inode(cgtls, aafs, aadir, cgbp+72)
	aaretval = ccv1
	if ccv1 != 0 {
		return aaretval
	}

	if (*(*tsext2_inode)(iqunsafe.ppPointer(cgbp + 72))).fdi_flags&ppuint32(mvEXT2_INDEX_FL) != 0 {
		return sidx_link(cgtls, aafs, aadir, cgbp+72, aaname, aaino, aaflags)
	}

	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fdfs = aafs
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fdname = aaname
	if aaname != 0 {
		ccv2 = Xstrlen(cgtls, aaname)
	} else {
		ccv2 = ppuint64(0)
	}
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fdnamelen = iqlibc.ppInt32FromUint64(ccv2)
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fdinode = aaino
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fdflags = aaflags
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fddone = 0
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fdsb = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fdblocksize = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fderr = 0

	if aaflags&ppint32(mvEXT2FS_LINK_APPEND) != 0 && !((*(*tsext2_inode)(iqunsafe.ppPointer(cgbp + 72))).fdi_flags&iqlibc.ppUint32FromInt32(mvEXT4_INLINE_DATA_FL) != 0) {
		*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 200)) = ppuint64(0)

		aalblk = ppuint64((*(*tsext2_inode)(iqunsafe.ppPointer(cgbp + 72))).fdi_size/(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize - ppuint32(1))
		aaretval = Xext2fs_bmap2(cgtls, aafs, aadir, cgbp+72, iqlibc.ppUintptrFromInt32(0), 0, aalblk, iqlibc.ppUintptrFromInt32(0), cgbp+200)
		if aaretval != 0 {
			return aaretval
		}

		(*(*tsdir_context)(iqunsafe.ppPointer(cgbp + 208))).fddir = aadir
		(*(*tsdir_context)(iqunsafe.ppPointer(cgbp + 208))).fdflags = ppint32(mvDIRENT_FLAG_INCLUDE_EMPTY)
		(*(*tsdir_context)(iqunsafe.ppPointer(cgbp + 208))).fdfunc1 = pp__ccgo_fp(silink_proc)
		(*(*tsdir_context)(iqunsafe.ppPointer(cgbp + 208))).fdpriv_data = cgbp + 16
		(*(*tsdir_context)(iqunsafe.ppPointer(cgbp + 208))).fderrcode = 0
		*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, ppuint64((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize))
		if !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) != 0) {
			ccv1 = iqlibc.ppInt64FromInt64(2133571398)
			goto cg_4
		}
		Xmemcpy(cgtls, cgbp+208+8, cgbp, ppuint64(8))
		ccv1 = 0
		goto cg_4
	cg_4:
		aaretval = ccv1
		...
		Xext2fs_process_dir_block(cgtls, aafs, cgbp+200, iqlibc.ppInt64FromUint64(aalblk), ppuint64(0), 0, cgbp+208)
		aaretval = (*(*tsdir_context)(iqunsafe.ppPointer(cgbp + 208))).fderrcode
		ccv5 = cgbp + 208 + 8
	...
	cg_6:
		...
		aaretval = Xext2fs_dir_iterate2(cgtls, aafs, aadir, ppint32(mvDIRENT_FLAG_INCLUDE_EMPTY), iqlibc.ppUintptrFromInt32(0), pp__ccgo_fp(silink_proc), cgbp+16)
	}
	if aaretval != 0 {
		return aaretval
	}
	if (*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fderr != 0 {
		return (*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fderr
	}

	if !((*(*tslink_struct)(iqunsafe.ppPointer(cgbp + 16))).fddone != 0) {
		if !(aaflags&iqlibc.ppInt32FromInt32(mvEXT2FS_LINK_EXPAND) != 0) {
			return iqlibc.ppInt64FromInt64(2133571366)
		}
		aaretval = Xext2fs_expand_dir(cgtls, aafs, aadir)
		if aaretval != 0 {
			return aaretval
		}
		goto ppretry
...
func _ext2fs_dirhash2(*iqlibc.ppTLS, ppint32, ppuintptr, ppint32, ppuintptr, ppint32, ppuintptr, ppuintptr, ppuintptr) ppint64
...
func _ext2fs_get_dx_countlimit(*iqlibc.ppTLS, ppuintptr, ppuintptr, ppuintptr, ppuintptr) ppint64

func _ext2fs_get_rec_len(*iqlibc.ppTLS, ppuintptr, ppuintptr, ppuintptr) ppint64

func _ext2fs_initialize_dirent_tail(*iqlibc.ppTLS, ppuintptr, ppuintptr)

func _ext2fs_inode_size_set(*iqlibc.ppTLS, ppuintptr, ppuintptr, ppint64) ppint64

func _ext2fs_process_dir_block(*iqlibc.ppTLS, ppuintptr, ppuintptr, ppint64, ppuint64, ppint32, ppuintptr) ppint32

func _ext2fs_read_dir_block4(*iqlibc.ppTLS, ppuintptr, ppuint64, ppuintptr, ppint32, ppuint32) ppint64
...
func _ext2fs_write_dir_block4(*iqlibc.ppTLS, ppuintptr, ppuint64, ppuintptr, ppint32, ppuint32) ppint64
