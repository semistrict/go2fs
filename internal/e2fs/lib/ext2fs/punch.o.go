...
//	 */
func sicheck_zero_block(cgtls *iqlibc.ppTLS, aabuf ppuintptr, aablocksize ppint32) (cgr ppint32) {

	var aacp, ccv1 ppuintptr
	var aaleft ppint32
	pp_, pp_, pp_ = aacp, aaleft, ccv1
	aacp = aabuf
	aaleft = aablocksize
	...
	for aaleft > 0 {
		ccv1 = aacp
		aacp = aacp + 1
		if *(*ppint8)(iqunsafe.ppPointer(ccv1)) != 0 {
			return 0
		}
		aaleft = aaleft - 1
	}
...
//	 */
func siind_punch(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aainode ppuintptr, aablock_buf ppuintptr, aap ppuintptr, aalevel ppint32, aastart tnblk64_t, aacount tnblk64_t, aamax ppint32) (cgr tnerrcode_t) {

	var aab, aacount2, aastart2 tnblk_t
	var aaend, aaincr, aaoffset tnblk64_t
	var aafreed, aai ppint32
	var aaretval tnerrcode_t
	var ccv2, ccv3 ppuint64
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aab, aacount2, aaend, aafreed, aai, aaincr, aaoffset, aaretval, aastart2, ccv2, ccv3
	aaend = aastart + aacount
	aafreed = 0

	aaincr = ppuint64(1) << (((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_log_block_size + ppuint32(10) - ppuint32(2)) * iqlibc.ppUint32FromInt32(aalevel))
	aai = 0
	aaoffset = iqlibc.ppUint64FromInt32(0)
	for {
		if !(aai < aamax) {
			break
		}
		...
		if *(*tnblk_t)(iqunsafe.ppPointer(aap)) == ppuint32(0) || aaoffset+aaincr <= aastart {
			goto cg_1
		}
		aab = *(*tnblk_t)(iqunsafe.ppPointer(aap))
		if aalevel > 0 {
			aaretval = Xext2fs_read_ind_block(cgtls, aafs, aab, aablock_buf)
			if aaretval != 0 {
				return aaretval
			}
			if aastart > aaoffset {
				ccv2 = aastart - aaoffset
			} else {
				ccv2 = ppuint64(0)
			}
			aastart2 = ppuint32(ccv2)
			if aastart > aaoffset {
				ccv2 = aastart
			} else {
				ccv2 = aaoffset
			}
			aacount2 = ppuint32(aaend - ccv2)
			aaretval = siind_punch(cgtls, aafs, aainode, aablock_buf+ppuintptr((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize), aablock_buf, aalevel-ppint32(1), ppuint64(aastart2), ppuint64(aacount2), iqlibc.ppInt32FromUint32((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize>>ppint32(2)))
			if aaretval != 0 {
				return aaretval
			}
			aaretval = Xext2fs_write_ind_block(cgtls, aafs, aab, aablock_buf)
			if aaretval != 0 {
				return aaretval
			}
			if !(sicheck_zero_block(cgtls, aablock_buf, iqlibc.ppInt32FromUint32((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize)) != 0) {
				goto cg_1
			}
		}
		Xext2fs_block_alloc_stats(cgtls, aafs, aab, -ppint32(1))
		*(*tnblk_t)(iqunsafe.ppPointer(aap)) = ppuint32(0)
		aafreed = aafreed + 1
	...
	cg_1:
		...
		aai = aai + 1
		aap += 4
		aaoffset = aaoffset + aaincr
	}
	return Xext2fs_iblk_sub_blocks(cgtls, aafs, aainode, iqlibc.ppUint64FromInt32(aafreed))
}

func siext2fs_punch_ind(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aainode ppuintptr, aablock_buf ppuintptr, aastart tnblk64_t, aaend tnblk64_t) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aaaddr_per_block, aacount1 tnblk_t
	var aabp, ccv8 ppuintptr
	var aalevel, aanum ppint32
	var aamax tnblk64_t
	...
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aaaddr_per_block, aabp, aacount1, aalevel, aamax, aanum, aaretval, ccv1, ccv2, ccv3, ccv5, ccv8
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)) = ppuintptr(0)
	aanum = ppint32(mvEXT2_NDIR_BLOCKS)
	aabp = aainode + 40
	aamax = ppuint64(mvEXT2_NDIR_BLOCKS)

	/* Check start/end don't overflow the 2^32-1 indirect block limit */
	if aastart > ppuint64(ppuint32(^iqlibc.ppUint64FromUint64(0))) {
		return 0
	}
	if aaend >= ppuint64(ppuint32(^iqlibc.ppUint64FromUint64(0))) || aaend-aastart+ppuint64(1) >= ppuint64(ppuint32(^iqlibc.ppUint64FromUint64(0))) {
		aacount1 = ppuint32(ppuint64(ppuint32(^iqlibc.ppUint64FromUint64(0))) - aastart)
	} else {
		aacount1 = ppuint32(aaend - aastart + ppuint64(1))
	}
	...
	aalevel = 0
	for {
		if !(aalevel < ppint32(4)) {
			break
		}
		if aastart < aamax {
			aaretval = siind_punch(cgtls, aafs, aainode, aablock_buf, aabp, aalevel, aastart, ppuint64(aacount1), aanum)
			if aaretval != 0 {
				goto pperrout
			}
			if ppuint64(aacount1) > aamax {

				aacount1 = ppuint32(ppuint64(aacount1) - (aamax - aastart))
			} else {
				break
			}
			aastart = ppuint64(0)
		} else {
			aastart = aastart - aamax
		}
		aabp = aabp + ppuintptr(aanum)*4
		if aalevel == 0 {
			aanum = ppint32(1)
			aamax = ppuint64(1)
		}
	...
	cg_7:
		;
		aalevel = aalevel + 1
		aamax = aamax * ppuint64(aaaddr_per_block)
	}
	aaretval = 0
	goto pperrout
pperrout:
...
func sipunch_extent_blocks(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaino tnext2_ino_t, aainode ppuintptr, aalfree_start tnblk64_t, aafree_start tnblk64_t, aafree_count tn__u32, aafreed ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aacluster_freed, aafreed_now, ccv1 tn__u32
	var aaretval tnerrcode_t
	var ccv2 tnblk64_t
	var pp_ /* pblk at bp+0 */ tnblk64_t
	pp_, pp_, pp_, pp_, pp_ = aacluster_freed, aafreed_now, aaretval, ccv1, ccv2
	aafreed_now = ppuint32(0)
	aaretval = 0

	if aafree_start < ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block) || aafree_start+ppuint64(aafree_count) > Xext2fs_blocks_count(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper) {
		return iqlibc.ppInt64FromInt64(2133571370)
	}

	/* No bigalloc?  Just free each block. */
	if ppint32(1)<<(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits == ppint32(1) {

		*(*tnblk64_t)(iqunsafe.ppPointer(aafreed)) += ppuint64(aafree_count)
		for {
			ccv1 = aafree_count
			aafree_count = aafree_count - 1
			if !(ccv1 > ppuint32(0)) {
				break
			}
			ccv2 = aafree_start
			aafree_start = aafree_start + 1
			Xext2fs_block_alloc_stats2(cgtls, aafs, ccv2, -ppint32(1))
		}
		return aaretval
	}
	 ...
	 */
	if aafree_start&iqlibc.ppUint64FromInt32(iqlibc.ppInt32FromInt32(1)<<(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits-iqlibc.ppInt32FromInt32(1)) != 0 {
		aaretval = Xext2fs_map_cluster_block(cgtls, aafs, aaino, aainode, aalfree_start, cgbp)
		if aaretval != 0 {
			goto pperrout
		}
		if !(*(*tnblk64_t)(iqunsafe.ppPointer(cgbp)) != 0) {
			Xext2fs_block_alloc_stats2(cgtls, aafs, aafree_start, -ppint32(1))
			aafreed_now = aafreed_now + 1
		}
		aacluster_freed = ppuint32(iqlibc.ppUint64FromInt32(iqlibc.ppInt32FromInt32(1)<<(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits) - aafree_start&iqlibc.ppUint64FromInt32(iqlibc.ppInt32FromInt32(1)<<(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits-iqlibc.ppInt32FromInt32(1)))
		if aacluster_freed > aafree_count {
			aacluster_freed = aafree_count
		}
		aafree_count = aafree_count - aacluster_freed
		aafree_start = aafree_start + ppuint64(aacluster_freed)
		aalfree_start = aalfree_start + ppuint64(aacluster_freed)
	}

	/* Free whole clusters from the middle of the range. */
	for aafree_count > ppuint32(0) && aafree_count >= iqlibc.ppUint32FromInt32(iqlibc.ppInt32FromInt32(1)<<(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits) {
		Xext2fs_block_alloc_stats2(cgtls, aafs, aafree_start, -ppint32(1))
		aafreed_now = aafreed_now + 1
		aacluster_freed = iqlibc.ppUint32FromInt32(iqlibc.ppInt32FromInt32(1) << (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits)
		aafree_count = aafree_count - aacluster_freed
		aafree_start = aafree_start + ppuint64(aacluster_freed)
		aalfree_start = aalfree_start + ppuint64(aacluster_freed)
	}
...
pperrout:
	...
	*(*tnblk64_t)(iqunsafe.ppPointer(aafreed)) += ppuint64(aafreed_now)
	return aaretval
}

func siext2fs_punch_extent(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaino tnext2_ino_t, aainode ppuintptr, aastart tnblk64_t, aaend tnblk64_t) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(96)
	defer cgtls.ppFree(96)

	var aafree_count, aanewlen tn__u32
	var aafree_start, aalfree_start, aanext, aanext_lblk, aaold_lblk tnblk64_t
	var aaop ppint32
	var aaretval tnerrcode_t
	var pp_ /* extent at bp+8 */ tsext2fs_extent
	var pp_ /* freed at bp+32 */ tnblk64_t
	var pp_ /* handle at bp+0 */ tnext2_extent_handle_t
	var pp_ /* newex at bp+40 */ tsext2fs_extent
	var pp_ /* newex at bp+64 */ tsext2fs_extent
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aafree_count, aafree_start, aalfree_start, aanewlen, aanext, aanext_lblk, aaold_lblk, aaop, aaretval
	*(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)) = ppuintptr(0)
	*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 32)) = ppuint64(0)
	 ...
	 * done.
	 */
	Xext2fs_extent_goto(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), aastart)
	aaretval = Xext2fs_extent_get(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), mvEXT2_EXTENT_CURRENT, cgbp+8)
	...
	for ppint32(1) != 0 {
		aaop = ppint32(mvEXT2_EXTENT_NEXT_LEAF)

		aanext = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk + ppuint64((*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_len)

		if aastart <= (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk {
			/*
			 * Have we iterated past the end of the punch region?
			 * If so, we can stop.
			 */
			if aaend < (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk {
				break
			}

			/* Start of deleted region before extent;
			   adjust beginning of extent */
			aafree_start = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_pblk
			aalfree_start = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk
			if aanext > aaend {
				aafree_count = ppuint32(aaend - (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk + ppuint64(1))
			} else {
				aafree_count = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_len
			}

			(*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_len -= aafree_count

			(*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk += ppuint64(aafree_count)

			(*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_pblk += ppuint64(aafree_count)
		} else {
			if aaend >= aanext-ppuint64(1) {
				/*
				...
				aanewlen = ppuint32(aastart - (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk)
				aafree_start = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_pblk + ppuint64(aanewlen)
				aalfree_start = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk + ppuint64(aanewlen)
				aafree_count = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_len - aanewlen
				(*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_len = aanewlen
			} else {

				/* The hard case; we need to split the extent */
				(*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 40))).fde_pblk = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_pblk + (aaend + ppuint64(1) - (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk)
				(*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 40))).fde_lblk = aaend + ppuint64(1)
				(*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 40))).fde_len = ppuint32(aanext - aaend - ppuint64(1))
				(*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 40))).fde_flags = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_flags

				(*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_len = ppuint32(aastart - (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk)
				aafree_start = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_pblk + ppuint64((*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_len)
				aalfree_start = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk + ppuint64((*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_len)
				aafree_count = ppuint32(aaend - aastart + ppuint64(1))

				aaretval = Xext2fs_extent_insert(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), ppint32(mvEXT2_EXTENT_INSERT_AFTER), cgbp+40)
				if aaretval != 0 {
					goto pperrout
				}
				aaretval = Xext2fs_extent_fix_parents(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)))
				if aaretval != 0 {
					goto pperrout
				}
				/*
				 ...
				 */
				aaretval = Xext2fs_extent_goto(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_lblk)
				if aaretval != 0 {
					goto pperrout
				}
			}
		}
		if (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 8))).fde_len != 0 {

			aaretval = Xext2fs_extent_replace(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), 0, cgbp+8)
			if aaretval != 0 {
				goto pperrout
			}
			aaretval = Xext2fs_extent_fix_parents(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)))
			 ...
			 */
			aaretval = Xext2fs_extent_get(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), mvEXT2_EXTENT_CURRENT, cgbp+64)
			if aaretval != 0 {
				goto pperrout
			}
			aaold_lblk = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 64))).fde_lblk

			aaretval = Xext2fs_extent_get(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), ppint32(mvEXT2_EXTENT_NEXT_LEAF), cgbp+64)
			if aaretval == ppint64(2133571452) {
				aanext_lblk = aaold_lblk
			} else {
				if aaretval != 0 {
					goto pperrout
				} else {
					aanext_lblk = (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp + 64))).fde_lblk
				}
			}

			aaretval = Xext2fs_extent_goto(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), aaold_lblk)
			if aaretval != 0 {
				goto pperrout
			}

			/* Now delete the extent. */
			aaretval = Xext2fs_extent_delete(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), 0)
			if aaretval != 0 {
				goto pperrout
			}

			aaretval = Xext2fs_extent_fix_parents(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)))
			if aaretval != 0 && aaretval != ppint64(2133571456) {
				goto pperrout
			}
			aaretval = 0
			 ...
			 * capture them for us.
			 */
			Xext2fs_extent_goto(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), aanext_lblk)
		...
		aaretval = sipunch_extent_blocks(cgtls, aafs, aaino, aainode, aalfree_start, aafree_start, aafree_count, cgbp+32)
		if aaretval != 0 {
			goto pperrout
		}
		goto ppnext_extent
	ppnext_extent:
		;
		aaretval = Xext2fs_extent_get(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp)), aaop, cgbp+8)
		if aaretval == ppint64(2133571452) || aaretval == ppint64(2133571456) {
			break
		}
		if aaretval != 0 {
			goto pperrout
	...
	aaretval = Xext2fs_iblk_sub_blocks(cgtls, aafs, aainode, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 32)))
	goto pperrout
pperrout:
	;
...
func siext2fs_punch_inline_data(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaino tnext2_ino_t, aainode ppuintptr, aastart tnblk64_t, aaend tnblk64_t) (cgr tnerrcode_t) {
	 ...
	 * now.
	 */
	if aastart > ppuint64(0) {
		return 0
	}

	Xmemset(cgtls, aainode+40, 0, iqlibc.ppUint64FromInt64(4)*iqlibc.ppUint64FromInt32(iqlibc.ppInt32FromInt32(mvEXT2_NDIR_BLOCKS)+iqlibc.ppInt32FromInt32(1)+iqlibc.ppInt32FromInt32(1)+iqlibc.ppInt32FromInt32(1)))
	...
	return Xext2fs_inline_data_ea_remove(cgtls, aafs, aaino)
}
...
//	 */
func Xext2fs_punch(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaino tnext2_ino_t, aainode ppuintptr, aablock_buf ppuintptr, aastart tnblk64_t, aaend tnblk64_t) (cgr tnerrcode_t) {
	...
	if (*tsext2_inode)(iqunsafe.ppPointer(aainode)).fdi_flags&ppuint32(mvEXT4_INLINE_DATA_FL) != 0 {
		return siext2fs_punch_inline_data(cgtls, aafs, aaino, aainode, aastart, aaend)
	} else {
		if (*tsext2_inode)(iqunsafe.ppPointer(aainode)).fdi_flags&ppuint32(mvEXT4_EXTENTS_FL) != 0 {
			aaretval = siext2fs_punch_extent(cgtls, aafs, aaino, aainode, aastart, aaend)
		} else {
			aaretval = siext2fs_punch_ind(cgtls, aafs, aainode, aablock_buf, aastart, aaend)
		}
	}
	if aaretval != 0 {
		return aaretval
	...
	return Xext2fs_write_inode(cgtls, aafs, aaino, aainode)
}
...
func _ext2fs_inline_data_ea_remove(*iqlibc.ppTLS, ppuintptr, ppuint32) ppint64
