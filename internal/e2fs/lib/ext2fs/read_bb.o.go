...
type tsread_bb_record = struct {
	fdbb_list tnext2_badblocks_list
	fderr     tnerrcode_t
}
...
//	 */
func simark_bad_block(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aablock_nr ppuintptr, aablockcnt tne2_blkcnt_t, aaref_block tnblk_t, aaref_offset ppint32, aapriv_data ppuintptr) (cgr ppint32) {
	...
	if *(*tnblk_t)(iqunsafe.ppPointer(aablock_nr)) < (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block || ppuint64(*(*tnblk_t)(iqunsafe.ppPointer(aablock_nr))) >= Xext2fs_blocks_count(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper) {
		return 0
	} /* Ignore illegal blocks */

	(*tsread_bb_record)(iqunsafe.ppPointer(aarb)).fderr = Xext2fs_badblocks_list_add(cgtls, (*tsread_bb_record)(iqunsafe.ppPointer(aarb)).fdbb_list, *(*tnblk_t)(iqunsafe.ppPointer(aablock_nr)))
	if (*tsread_bb_record)(iqunsafe.ppPointer(aarb)).fderr != 0 {
...
//	 */
func Xext2fs_read_bb_inode(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aabb_list ppuintptr) (cgr tnerrcode_t) {
	...
	var pp_ /* rb at bp+0 */ tsread_bb_record
	pp_, pp_, pp_ = aanumblocks, aaretval, ccv1

	if (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdmagic != ppint64(2133571329) {
		return iqlibc.ppInt64FromInt64(2133571329)
	}

	if !(*(*tnext2_badblocks_list)(iqunsafe.ppPointer(aabb_list)) != 0) {
		aaretval = Xext2fs_read_inode(cgtls, aafs, ppuint32(mvEXT2_BAD_INO), cgbp+16)
		if aaretval != 0 {
			return aaretval
		}
		aanumblocks = (*(*tsext2_inode)(iqunsafe.ppPointer(cgbp + 16))).fdi_blocks
	...
	cg_2:
		;
		if !(ccv1 != 0 && (*(*tsext2_inode)(iqunsafe.ppPointer(cgbp + 16))).fdi_flags&ppuint32(mvEXT4_HUGE_FILE_FL) != 0) {
			aanumblocks = aanumblocks / ((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize / ppuint32(512))
		}
		aanumblocks = aanumblocks + ppuint32(20)
		if aanumblocks < ppuint32(50) {
			aanumblocks = ppuint32(50)
		}
		if aanumblocks > ppuint32(50000) {
			aanumblocks = ppuint32(500)
		}
		aaretval = Xext2fs_badblocks_list_create(cgtls, aabb_list, iqlibc.ppInt32FromUint32(aanumblocks))
		if aaretval != 0 {
			return aaretval
		}
	}

	(*(*tsread_bb_record)(iqunsafe.ppPointer(cgbp))).fdbb_list = *(*tnext2_badblocks_list)(iqunsafe.ppPointer(aabb_list))
	(*(*tsread_bb_record)(iqunsafe.ppPointer(cgbp))).fderr = 0
	aaretval = Xext2fs_block_iterate2(cgtls, aafs, ppuint32(mvEXT2_BAD_INO), ppint32(mvBLOCK_FLAG_READ_ONLY), ppuintptr(0), pp__ccgo_fp(simark_bad_block), cgbp)
	if aaretval != 0 {
		return aaretval
	}

	return (*(*tsread_bb_record)(iqunsafe.ppPointer(cgbp))).fderr
}

func _ext2fs_badblocks_list_add(*iqlibc.ppTLS, ppuintptr, ppuint32) ppint64

func _ext2fs_badblocks_list_create(*iqlibc.ppTLS, ppuintptr, ppint32) ppint64
