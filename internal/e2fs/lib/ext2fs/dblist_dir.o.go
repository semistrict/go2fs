...
type tn__ccgo_fp__Xext2fs_dblist_dir_iterate_3 = func(*iqlibc.ppTLS, ppuint32, ppint32, ppuintptr, ppint32, ppint32, ppuintptr, ppuintptr) ppint32

func Xext2fs_dblist_dir_iterate(cgtls *iqlibc.ppTLS, aadblist tnext2_dblist, aaflags ppint32, aablock_buf ppuintptr, aa__ccgo_fp_func ppuintptr, aapriv_data ppuintptr) (cgr tnerrcode_t) {
	...
	(*(*tsdir_context)(iqunsafe.ppPointer(cgbp + 16))).fddir = ppuint32(0)
	(*(*tsdir_context)(iqunsafe.ppPointer(cgbp + 16))).fdflags = aaflags
	if aablock_buf != 0 {
		(*(*tsdir_context)(iqunsafe.ppPointer(cgbp + 16))).fdbuf = aablock_buf
	} else {
		*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, ppuint64((*tsstruct_ext2_filsys)(iqunsafe.ppPointer((*tsext2_struct_dblist)(iqunsafe.ppPointer(aadblist)).fdfs)).fdblocksize))
	...
	aaretval = Xext2fs_dblist_iterate2(cgtls, aadblist, pp__ccgo_fp(sidb_dir_proc), cgbp+16)
...
func sidb_dir_proc(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadb_info ppuintptr, aapriv_data ppuintptr) (cgr ppint32) {
	...
	aactx = aapriv_data
	(*tsdir_context)(iqunsafe.ppPointer(aactx)).fddir = (*tsext2_db_entry2)(iqunsafe.ppPointer(aadb_info)).fdino
	(*tsdir_context)(iqunsafe.ppPointer(aactx)).fderrcode = 0

	(*tsdir_context)(iqunsafe.ppPointer(aactx)).fderrcode = Xext2fs_read_inode(cgtls, aafs, (*tsdir_context)(iqunsafe.ppPointer(aactx)).fddir, cgbp)
	if (*tsdir_context)(iqunsafe.ppPointer(aactx)).fderrcode != 0 {
		return ppint32(mvDBLIST_ABORT)
	}
	if (*(*tsext2_inode)(iqunsafe.ppPointer(cgbp))).fdi_flags&ppuint32(mvEXT4_INLINE_DATA_FL) != 0 {
		aaret = Xext2fs_inline_data_dir_iterate(cgtls, aafs, (*tsdir_context)(iqunsafe.ppPointer(aactx)).fddir, aactx)
	} else {
		aaret = Xext2fs_process_dir_block(cgtls, aafs, aadb_info+8, (*tsext2_db_entry2)(iqunsafe.ppPointer(aadb_info)).fdblockcnt, ppuint64(0), 0, aapriv_data)
	}
	if aaret&ppint32(mvBLOCK_ABORT) != 0 && !((*tsdir_context)(iqunsafe.ppPointer(aactx)).fderrcode != 0) {
		return ppint32(mvDBLIST_ABORT)
	}
	return 0
}

func _ext2fs_dblist_iterate2(*iqlibc.ppTLS, ppuintptr, ppuintptr, ppuintptr) ppint64
