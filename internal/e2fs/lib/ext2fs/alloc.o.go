...
const mvEXT2_ALLOCRANGE_ALL_FLAGS = 3
const mvEXT2_ALLOCRANGE_FIXED_GOAL = 1
const mvEXT2_ALLOCRANGE_ZERO_BLOCKS = 2
...
const mvEXT2_NEWRANGE_ALL_FLAGS = 3
...
//	 */
func Xext2fs_clear_block_uninit(cgtls *iqlibc.ppTLS, aafs3 tnext2_filsys, aagroup tndgrp_t) {

	var ccv1 tnext2_filsys
	var ccv2, ccv4, ccv6 ppint32
	var ccv8, ccv9 ppbool
	pp_, pp_, pp_, pp_, pp_, pp_ = ccv1, ccv2, ccv4, ccv6, ccv8, ccv9

	if ccv9 = aagroup >= (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs3)).fdgroup_desc_count; !ccv9 {
		ccv1 = aafs3
	...
	if ccv9 || !(ccv2 != 0) || !(Xext2fs_bg_flags_test(cgtls, aafs3, aagroup, ppuint16(mvEXT2_BG_BLOCK_UNINIT)) != 0) {
		return
	}

	/* uninit block bitmaps are now initialized in read_bitmaps() */

	Xext2fs_bg_flags_clear(cgtls, aafs3, aagroup, ppuint16(mvEXT2_BG_BLOCK_UNINIT))
	Xext2fs_group_desc_csum_set(cgtls, aafs3, aagroup)

	*(*ppint32)(iqunsafe.ppPointer(aafs3 + 16)) |= iqlibc.ppInt32FromInt32(mvEXT2_FLAG_DIRTY) | iqlibc.ppInt32FromInt32(mvEXT2_FLAG_CHANGED)

	*(*ppint32)(iqunsafe.ppPointer(aafs3 + 16)) |= iqlibc.ppInt32FromInt32(mvEXT2_FLAG_BB_DIRTY) | iqlibc.ppInt32FromInt32(mvEXT2_FLAG_CHANGED)
}
...
//	 */
func sicheck_inode_uninit(cgtls *iqlibc.ppTLS, aafs3 tnext2_filsys, aamap tnext2fs_inode_bitmap, aagroup tndgrp_t) {

	var aai, aaino tnext2_ino_t
	var ccv1 tnext2_filsys
	var ccv2, ccv4, ccv6 ppint32
	var ccv8, ccv9 ppbool
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aai, aaino, ccv1, ccv2, ccv4, ccv6, ccv8, ccv9
	...
	if ccv9 || !(ccv2 != 0) || !(Xext2fs_bg_flags_test(cgtls, aafs3, aagroup, ppuint16(mvEXT2_BG_INODE_UNINIT)) != 0) {
		return
	}

	aaino = aagroup*(*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs3)).fdsuper)).fds_inodes_per_group + ppuint32(1)
	aai = ppuint32(0)
	for {
		if !(aai < (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs3)).fdsuper)).fds_inodes_per_group) {
			break
		}
		Xext2fs_unmark_generic_bmap(cgtls, aamap, ppuint64(aaino))
		goto cg_10
	cg_10:
		;
		aai = aai + 1
	...
	Xext2fs_bg_flags_clear(cgtls, aafs3, aagroup, ppuint16(mvEXT2_BG_INODE_UNINIT))
	/* Mimics what the kernel does */
	Xext2fs_bg_flags_clear(cgtls, aafs3, aagroup, ppuint16(mvEXT2_BG_BLOCK_UNINIT))
	Xext2fs_group_desc_csum_set(cgtls, aafs3, aagroup)

	*(*ppint32)(iqunsafe.ppPointer(aafs3 + 16)) |= iqlibc.ppInt32FromInt32(mvEXT2_FLAG_IB_DIRTY) | iqlibc.ppInt32FromInt32(mvEXT2_FLAG_CHANGED)
...
//	 */
func Xext2fs_new_inode(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t, aamode ppint32, aamap tnext2fs_inode_bitmap, aaret ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aagroup tndgrp_t
	var aai, aaino_in_group, aastart_inode, aaupto tnext2_ino_t
	var aaretval, aarv, ccv3 tnerrcode_t
	var ccv1, ccv2, ccv5 ppuint32
	var ccv6 ppint32
	var pp_ /* first_zero at bp+8 */ tnext2_ino_t
	var pp_ /* o at bp+0 */ tn__u64
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aagroup, aai, aaino_in_group, aaretval, aarv, aastart_inode, aaupto, ccv1, ccv2, ccv3, ccv5, ccv6
	aastart_inode = ppuint32(0)
	...
	if !(aamap != 0) {
		aamap = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdinode_map
	}
	if !(aamap != 0) {
	...
	if aadir > ppuint32(0) {
		aagroup = (aadir - ppuint32(1)) / (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_per_group
		aastart_inode = aagroup*(*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_per_group + ppuint32(1)
	}
	if (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_rev_level == ppuint32(mvEXT2_GOOD_OLD_REV) {
		ccv1 = ppuint32(mvEXT2_GOOD_OLD_FIRST_INO)
	} else {
		ccv1 = (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_ino
	}
	if aastart_inode < ccv1 {
		if (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_rev_level == ppuint32(mvEXT2_GOOD_OLD_REV) {
			ccv2 = ppuint32(mvEXT2_GOOD_OLD_FIRST_INO)
		} else {
			ccv2 = (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_ino
		}
		aastart_inode = ccv2
	}
	if aastart_inode > (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_count {
		return iqlibc.ppInt64FromInt64(2133571401)
	}
	aai = aastart_inode
	for cgcond := true; cgcond; cgcond = aai != aastart_inode {
		aaino_in_group = (aai - ppuint32(1)) % (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_per_group
		aagroup = (aai - ppuint32(1)) / (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_per_group

		sicheck_inode_uninit(cgtls, aafs, aamap, aagroup)
		aaupto = aai + ((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_per_group - aaino_in_group)
		if aai < aastart_inode && aaupto >= aastart_inode {
			aaupto = aastart_inode - ppuint32(1)
		}
		if aaupto > (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_count {
			aaupto = (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_count
		}

		aarv = Xext2fs_find_first_zero_generic_bmap(cgtls, aamap, ppuint64(aai), ppuint64(aaupto), cgbp)
		if !(aarv != 0) {
			*(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp + 8)) = ppuint32(*(*tn__u64)(iqunsafe.ppPointer(cgbp)))
		}
		ccv3 = aarv
		goto cg_4
	cg_4:
		aaretval = ccv3
		if aaretval == 0 {
			aai = *(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp + 8))
			break
		}
		if aaretval != ppint64(mvENOENT) {
			return iqlibc.ppInt64FromInt64(2133571401)
		}
		aai = aaupto + ppuint32(1)
		if aai > (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_inodes_count {
			if (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_rev_level == ppuint32(mvEXT2_GOOD_OLD_REV) {
				ccv1 = ppuint32(mvEXT2_GOOD_OLD_FIRST_INO)
			} else {
				ccv1 = (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_ino
	...
	ccv6 = Xext2fs_test_generic_bmap(cgtls, aamap, ppuint64(aai))
	goto cg_7
cg_7:
	if ccv6 != 0 {
		return iqlibc.ppInt64FromInt64(2133571401)
	}
	*(*tnext2_ino_t)(iqunsafe.ppPointer(aaret)) = aai
	return 0
}
...
//	 */
func Xext2fs_new_block3(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aagoal tnblk64_t, aamap tnext2fs_block_bitmap, aaret ppuintptr, aactx ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aagab, aagab2 ppuintptr
	var aaretval, aarv, ccv1, ccv3 tnerrcode_t
	var pp_ /* b at bp+8 */ tnblk64_t
	var pp_ /* o at bp+0 */ tn__u64
	pp_, pp_, pp_, pp_, pp_, pp_ = aagab, aagab2, aaretval, aarv, ccv1, ccv3
	*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8)) = ppuint64(0)
	...
	if !(aamap != 0) {
		 ...
		 */
		if (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block2 != 0 {
			aagab2 = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block2
			(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block2 = iqlibc.ppUintptrFromInt32(0)
			aaretval = (*(*func(*iqlibc.ppTLS, tnext2_filsys, tnblk64_t, ppuintptr, ppuintptr) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{aagab2})))(cgtls, aafs, aagoal, cgbp+8, aactx)
			(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block2 = aagab2
			goto ppallocated
		} else {
			if (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block != 0 {
				aagab = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block
				(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block = iqlibc.ppUintptrFromInt32(0)
				aaretval = (*(*func(*iqlibc.ppTLS, tnext2_filsys, tnblk64_t, ppuintptr) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{aagab})))(cgtls, aafs, aagoal, cgbp+8)
				(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block = aagab
				goto ppallocated
			}
		}
	}
	if !(aamap != 0) {
		aamap = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblock_map
	}
	if !(aamap != 0) {
		return iqlibc.ppInt64FromInt64(2133571368)
	}
	if !(aagoal != 0) || aagoal >= Xext2fs_blocks_count(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper) {
		aagoal = ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block)
	}
	aagoal = aagoal & iqlibc.ppUint64FromInt32(^(iqlibc.ppInt32FromInt32(1)<<(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits - iqlibc.ppInt32FromInt32(1)))

	aarv = Xext2fs_find_first_zero_generic_bmap(cgtls, aamap, aagoal, Xext2fs_blocks_count(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)-ppuint64(1), cgbp)
	if !(aarv != 0) {
...
cg_2:
	aaretval = ccv1
	if aaretval == ppint64(mvENOENT) && aagoal != ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block) {
		aarv = Xext2fs_find_first_zero_generic_bmap(cgtls, aamap, ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block), aagoal-ppuint64(1), cgbp)
		if !(aarv != 0) {
			*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8)) = *(*tn__u64)(iqunsafe.ppPointer(cgbp))
		}
		ccv1 = aarv
...
ppallocated:
	...
	Xext2fs_clear_block_uninit(cgtls, aafs, Xext2fs_group_of_blk2(cgtls, aafs, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8))))
	*(*tnblk64_t)(iqunsafe.ppPointer(aaret)) = *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8))
	return 0
}

func Xext2fs_new_block2(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aagoal tnblk64_t, aamap tnext2fs_block_bitmap, aaret ppuintptr) (cgr tnerrcode_t) {

	return Xext2fs_new_block3(cgtls, aafs, aagoal, aamap, aaret, iqlibc.ppUintptrFromInt32(0))
}

func Xext2fs_new_block(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aagoal tnblk_t, aamap tnext2fs_block_bitmap, aaret ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aaretval tnerrcode_t
	var pp_ /* val at bp+0 */ tnblk64_t
	pp_ = aaretval
	aaretval = Xext2fs_new_block2(cgtls, aafs, ppuint64(aagoal), aamap, cgbp)
	if !(aaretval != 0) {
		*(*tnblk_t)(iqunsafe.ppPointer(aaret)) = ppuint32(*(*tnblk64_t)(iqunsafe.ppPointer(cgbp)))
	}
	return aaretval
}
...
//	 */
func Xext2fs_alloc_block3(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aagoal tnblk64_t, aablock_buf ppuintptr, aaret ppuintptr, aactx ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aaretval tnerrcode_t
	var pp_ /* block at bp+0 */ tnblk64_t
	pp_ = aaretval

	if (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block2 != 0 {
		aaretval = (*(*func(*iqlibc.ppTLS, tnext2_filsys, tnblk64_t, ppuintptr, ppuintptr) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block2})))(cgtls, aafs, aagoal, cgbp, aactx)
		if aaretval != 0 {
			goto ppfail
		}
	} else {
		if (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block != 0 {
			aaretval = (*(*func(*iqlibc.ppTLS, tnext2_filsys, tnblk64_t, ppuintptr) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block})))(cgtls, aafs, aagoal, cgbp)
			if aaretval != 0 {
				goto ppfail
			}
		} else {
			if !((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblock_map != 0) {
				aaretval = Xext2fs_read_block_bitmap(cgtls, aafs)
				if aaretval != 0 {
					goto ppfail
				}
			}

			aaretval = Xext2fs_new_block3(cgtls, aafs, aagoal, ppuintptr(0), cgbp, aactx)
			if aaretval != 0 {
				goto ppfail
			}
		}
	...
	if aablock_buf != 0 {
		Xmemset(cgtls, aablock_buf, 0, ppuint64((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize))
		aaretval = Xio_channel_write_blk64(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdio, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp)), ppint32(1), aablock_buf)
	} else {
		aaretval = Xext2fs_zero_blocks2(cgtls, aafs, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp)), ppint32(1), iqlibc.ppUintptrFromInt32(0), iqlibc.ppUintptrFromInt32(0))
	}
	if aaretval != 0 {
		goto ppfail
	}

	Xext2fs_block_alloc_stats2(cgtls, aafs, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp)), +iqlibc.ppInt32FromInt32(1))
	*(*tnblk64_t)(iqunsafe.ppPointer(aaret)) = *(*tnblk64_t)(iqunsafe.ppPointer(cgbp))
...
func Xext2fs_alloc_block2(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aagoal tnblk64_t, aablock_buf ppuintptr, aaret ppuintptr) (cgr tnerrcode_t) {

	return Xext2fs_alloc_block3(cgtls, aafs, aagoal, aablock_buf, aaret, iqlibc.ppUintptrFromInt32(0))
}

func Xext2fs_alloc_block(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aagoal tnblk_t, aablock_buf ppuintptr, aaret ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aagoal64 tnblk64_t
	var aaretval tnerrcode_t
	var pp_ /* ret64 at bp+0 */ tnblk64_t
	pp_, pp_ = aagoal64, aaretval
	aagoal64 = ppuint64(aagoal)
	aaretval = Xext2fs_alloc_block3(cgtls, aafs, aagoal64, aablock_buf, cgbp, iqlibc.ppUintptrFromInt32(0))
	if !(aaretval != 0) {
		*(*tnblk_t)(iqunsafe.ppPointer(aaret)) = ppuint32(*(*tnblk64_t)(iqunsafe.ppPointer(cgbp)))
	}
	return aaretval
...
func Xext2fs_get_free_blocks2(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aastart tnblk64_t, aafinish tnblk64_t, aanum1 ppint32, aamap tnext2fs_block_bitmap, aaret ppuintptr) (cgr tnerrcode_t) {

	var aab tnblk64_t
	var aac_ratio, ccv1 ppint32
	pp_, pp_, pp_ = aab, aac_ratio, ccv1
	aab = aastart
	...
	if !(aab != 0) {
		aab = ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block)
	}
	if !(aafinish != 0) {
		aafinish = aastart
	}
	...
	aac_ratio = ppint32(1) << Xext2fs_get_bitmap_granularity(cgtls, aamap)
	aab = aab & iqlibc.ppUint64FromInt32(^(aac_ratio - iqlibc.ppInt32FromInt32(1)))
	aafinish = aafinish & iqlibc.ppUint64FromInt32(^(aac_ratio - iqlibc.ppInt32FromInt32(1)))
	for cgcond := true; cgcond; cgcond = aab != aafinish {
		if aab+iqlibc.ppUint64FromInt32(aanum1)-ppuint64(1) >= Xext2fs_blocks_count(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper) {
			if aafinish > aastart {
				return iqlibc.ppInt64FromInt64(2133571400)
			}
			aab = ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block)
		}
		ccv1 = Xext2fs_test_block_bitmap_range2(cgtls, aamap, aab, iqlibc.ppUint32FromInt32(aanum1))
		goto cg_2
	cg_2:
		if ccv1 != 0 {
			*(*tnblk64_t)(iqunsafe.ppPointer(aaret)) = aab
			return 0
		}
		aab = aab + iqlibc.ppUint64FromInt32(aac_ratio)
	}
	return iqlibc.ppInt64FromInt64(2133571400)
}

func Xext2fs_get_free_blocks(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aastart tnblk_t, aafinish tnblk_t, aanum ppint32, aamap tnext2fs_block_bitmap, aaret ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)
	...
	var pp_ /* val at bp+0 */ tnblk64_t
	pp_ = aaretval
	aaretval = Xext2fs_get_free_blocks2(cgtls, aafs, ppuint64(aastart), ppuint64(aafinish), aanum, aamap, cgbp)
	if !(aaretval != 0) {
		*(*tnblk_t)(iqunsafe.ppPointer(aaret)) = ppuint32(*(*tnblk64_t)(iqunsafe.ppPointer(cgbp)))
	}
	return aaretval
...
type tn__ccgo_fp__Xext2fs_set_alloc_block_callback_1 = func(*iqlibc.ppTLS, ppuintptr, ppuint64, ppuintptr) ppint64

type tn__ccgo_fp__Xext2fs_set_alloc_block_callback_2 = func(*iqlibc.ppTLS, ppuintptr, ppuint64, ppuintptr) ppint64

func Xext2fs_set_alloc_block_callback(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aa__ccgo_fp_func ppuintptr, aa__ccgo_fp_old ppuintptr) {

	if !(aafs != 0) || (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdmagic != ppint64(2133571329) {
		return
	}

	if aa__ccgo_fp_old != 0 {
		*(*ppuintptr)(iqunsafe.ppPointer(aa__ccgo_fp_old)) = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block
	}

	(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdget_alloc_block = aa__ccgo_fp_func
}

func Xext2fs_find_inode_goal(cgtls *iqlibc.ppTLS, aafs1 tnext2_filsys, aaino1 tnext2_ino_t, aainode ppuintptr, aalblk tnblk64_t) (cgr tnblk64_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aaerr tnerrcode_t
	var aagroup, ccv1 tndgrp_t
	var aalog_flex tn__u8
	var pp_ /* extent at bp+0 */ tsext2fs_extent
	var pp_ /* handle at bp+24 */ tnext2_extent_handle_t
	pp_, pp_, pp_, pp_ = aaerr, aagroup, aalog_flex, ccv1
	*(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp + 24)) = iqlibc.ppUintptrFromInt32(0)

	/* Make sure data stored in inode->i_block is neither fast symlink nor
	 * inline data.
	 */
	if aainode == iqlibc.ppUintptrFromInt32(0) || Xext2fs_is_fast_symlink(cgtls, aainode) != 0 || (*tsext2_inode)(iqunsafe.ppPointer(aainode)).fdi_flags&ppuint32(mvEXT4_INLINE_DATA_FL) != 0 {
		goto ppno_blocks
	}

	if (*tsext2_inode)(iqunsafe.ppPointer(aainode)).fdi_flags&ppuint32(mvEXT4_EXTENTS_FL) != 0 {
		aaerr = Xext2fs_extent_open2(cgtls, aafs1, aaino1, aainode, cgbp+24)
		if aaerr != 0 {
			goto ppno_blocks
		}
		aaerr = Xext2fs_extent_goto2(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp + 24)), 0, aalblk)
		if aaerr != 0 {
			goto ppno_blocks
		}
		aaerr = Xext2fs_extent_get(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp + 24)), mvEXT2_EXTENT_CURRENT, cgbp)
		if aaerr != 0 {
			goto ppno_blocks
		}
		Xext2fs_extent_free(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp + 24)))
		return (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp))).fde_pblk + (aalblk - (*(*tsext2fs_extent)(iqunsafe.ppPointer(cgbp))).fde_lblk)
	}

	/* block mapped file; see if block zero is mapped? */
	if *(*tn__u32)(iqunsafe.ppPointer(aainode + 40)) != 0 {
		return ppuint64(*(*tn__u32)(iqunsafe.ppPointer(aainode + 40)))
	}
...
ppno_blocks:
	;
	Xext2fs_extent_free(cgtls, *(*tnext2_extent_handle_t)(iqunsafe.ppPointer(cgbp + 24)))
	aalog_flex = (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdsuper)).fds_log_groups_per_flex
	ccv1 = (aaino1 - ppuint32(1)) / (*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs1)).fdsuper)).fds_inodes_per_group
	goto cg_2
cg_2:
	aagroup = ccv1
	if aalog_flex != 0 {
		aagroup = aagroup & iqlibc.ppUint32FromInt32(^(iqlibc.ppInt32FromInt32(1)<<aalog_flex - iqlibc.ppInt32FromInt32(1)))
	}
	return Xext2fs_group_first_block2(cgtls, aafs1, aagroup)
}
...
//	 */
func Xext2fs_new_range(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaflags ppint32, aagoal tnblk64_t, aalen tnblk64_t, aamap tnext2fs_block_bitmap, aapblk ppuintptr, aaplen ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aab, aamax_blocks tnblk64_t
	var aalooped ppint32
	var aanrf ppuintptr
	var aaretval, aarv, aarv1, ccv3, ccv6 tnerrcode_t
	var ccv5 ppuint64
	var pp_ /* end at bp+24 */ tnblk64_t
	var pp_ /* o at bp+0 */ tn__u64
	var pp_ /* o at bp+8 */ tn__u64
	var pp_ /* start at bp+16 */ tnblk64_t
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aab, aalooped, aamax_blocks, aanrf, aaretval, aarv, aarv1, ccv3, ccv5, ccv6
	aalooped = 0
	...
	if aalen == ppuint64(0) || aaflags & ^iqlibc.ppInt32FromInt32(mvEXT2_NEWRANGE_ALL_FLAGS) != 0 {
		return iqlibc.ppInt64FromInt64(2133571399)
	}

	if !(aamap != 0) && (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdnew_range != 0 {
		/*
		 ...
		 * end up in an infinite loop.
		 */
		aanrf = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdnew_range
		(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdnew_range = iqlibc.ppUintptrFromInt32(0)
		aaretval = (*(*func(*iqlibc.ppTLS, tnext2_filsys, ppint32, tnblk64_t, tnblk64_t, ppuintptr, ppuintptr) tnerrcode_t)(iqunsafe.ppPointer(&struct{ ppuintptr }{aanrf})))(cgtls, aafs, aaflags, aagoal, aalen, aapblk, aaplen)
		(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdnew_range = aanrf
		if aaretval != 0 {
			return aaretval
		}
		*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) = *(*tnblk64_t)(iqunsafe.ppPointer(aapblk))
		*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 24)) = *(*tnblk64_t)(iqunsafe.ppPointer(aapblk)) + *(*tnblk64_t)(iqunsafe.ppPointer(aaplen))
		goto ppallocated
	}
	if !(aamap != 0) {
		aamap = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblock_map
	...
	*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) = aagoal
cg_2:
	;
	if !(!(aalooped != 0) || *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) <= aagoal) {
		goto cg_1
	}
	aarv = Xext2fs_find_first_zero_generic_bmap(cgtls, aamap, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)), aamax_blocks-ppuint64(1), cgbp)
	if !(aarv != 0) {
...
cg_4:
	...
	if aaretval == ppint64(mvENOENT) {
		 ...
		 */
		if aaflags&ppint32(mvEXT2_NEWRANGE_FIXED_GOAL) != 0 || *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) == ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block) {
			goto ppfail
		}
		*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) = ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block)
		goto cg_2
	} else {
		if aaretval != 0 {
			goto pperrout
	...
	if aaflags&ppint32(mvEXT2_NEWRANGE_FIXED_GOAL) != 0 && *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) != aagoal {
		goto ppfail
	}

	if *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16))+aalen-ppuint64(1) < aamax_blocks-ppuint64(1) {
		ccv5 = *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) + aalen - ppuint64(1)
	} else {
		ccv5 = aamax_blocks - ppuint64(1)
	}
	aab = ccv5
	aarv1 = Xext2fs_find_first_set_generic_bmap(cgtls, aamap, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)), aab, cgbp+8)
	if !(aarv1 != 0) {
		*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 24)) = *(*tn__u64)(iqunsafe.ppPointer(cgbp + 8))
	}
	ccv3 = aarv1
	goto cg_7
cg_7:
	aaretval = ccv3
	if aaretval == ppint64(mvENOENT) {
		*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 24)) = aab + ppuint64(1)
	} else {
		if aaretval != 0 {
			goto pperrout
		}
	...
	if !(!(aaflags&iqlibc.ppInt32FromInt32(mvEXT2_NEWRANGE_MIN_LENGTH) != 0) || *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 24))-*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) >= aalen) {
		goto cg_8
	}
	/* Success! */
	*(*tnblk64_t)(iqunsafe.ppPointer(aapblk)) = *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16))
	*(*tnblk64_t)(iqunsafe.ppPointer(aaplen)) = *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 24)) - *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16))

	goto ppallocated
ppallocated:
	;
	aab = *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16))
	for {
		if !(aab < *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 24))) {
			break
		}
		Xext2fs_clear_block_uninit(cgtls, aafs, Xext2fs_group_of_blk2(cgtls, aafs, aab))
		goto cg_9
	cg_9:
		;
		aab = aab + ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_blocks_per_group)
	}
	return 0
cg_8:
	;

	if aaflags&ppint32(mvEXT2_NEWRANGE_FIXED_GOAL) != 0 {
		goto ppfail
	}
	*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) = *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 24))
	if *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 16)) >= aamax_blocks {
		if aalooped != 0 {
			goto ppfail
		}
		aalooped = ppint32(1)
...
ppfail:
	;
	aaretval = iqlibc.ppInt64FromInt64(2133571400)
	goto pperrout
pperrout:
	;
	return aaretval
...
type tn__ccgo_fp__Xext2fs_set_new_range_callback_1 = func(*iqlibc.ppTLS, ppuintptr, ppint32, ppuint64, ppuint64, ppuintptr, ppuintptr) ppint64

type tn__ccgo_fp__Xext2fs_set_new_range_callback_2 = func(*iqlibc.ppTLS, ppuintptr, ppint32, ppuint64, ppuint64, ppuintptr, ppuintptr) ppint64

func Xext2fs_set_new_range_callback(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aa__ccgo_fp_func ppuintptr, aa__ccgo_fp_old ppuintptr) {
	...
	if aa__ccgo_fp_old != 0 {
		*(*ppuintptr)(iqunsafe.ppPointer(aa__ccgo_fp_old)) = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdnew_range
	}

	(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdnew_range = aa__ccgo_fp_func
}

func Xext2fs_alloc_range(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaflags ppint32, aagoal tnblk64_t, aalen tnblk_t, aaret ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aanewr_flags ppint32
	var aaretval tnerrcode_t
	var pp_ /* plen at bp+0 */ tnblk64_t
	pp_, pp_ = aanewr_flags, aaretval
	aanewr_flags = ppint32(mvEXT2_NEWRANGE_MIN_LENGTH)

	if (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdmagic != ppint64(2133571329) {
		return iqlibc.ppInt64FromInt64(2133571329)
	}
	if aalen == ppuint32(0) || aaflags & ^iqlibc.ppInt32FromInt32(mvEXT2_ALLOCRANGE_ALL_FLAGS) != 0 {
		return iqlibc.ppInt64FromInt64(2133571399)
	}

	if aaflags&ppint32(mvEXT2_ALLOCRANGE_FIXED_GOAL) != 0 {
		aanewr_flags = aanewr_flags | ppint32(mvEXT2_NEWRANGE_FIXED_GOAL)
	}

	aaretval = Xext2fs_new_range(cgtls, aafs, aanewr_flags, aagoal, ppuint64(aalen), iqlibc.ppUintptrFromInt32(0), aaret, cgbp)
	if aaretval != 0 {
		return aaretval
	}

	if *(*tnblk64_t)(iqunsafe.ppPointer(cgbp)) < ppuint64(aalen) {
		return iqlibc.ppInt64FromInt64(2133571400)
	}

	if aaflags&ppint32(mvEXT2_ALLOCRANGE_ZERO_BLOCKS) != 0 {
		aaretval = Xext2fs_zero_blocks2(cgtls, aafs, *(*tnblk64_t)(iqunsafe.ppPointer(aaret)), iqlibc.ppInt32FromUint32(aalen), iqlibc.ppUintptrFromInt32(0), iqlibc.ppUintptrFromInt32(0))
		if aaretval != 0 {
			return aaretval
		}
	}

	Xext2fs_block_alloc_stats_range(cgtls, aafs, *(*tnblk64_t)(iqunsafe.ppPointer(aaret)), aalen, +iqlibc.ppInt32FromInt32(1))
	return aaretval
}
...
func _ext2fs_extent_goto2(*iqlibc.ppTLS, ppuintptr, ppint32, ppuint64) ppint64

func _ext2fs_extent_open2(*iqlibc.ppTLS, ppuintptr, ppuint32, ppuintptr, ppuintptr) ppint64

func _ext2fs_find_first_set_generic_bmap(*iqlibc.ppTLS, ppuintptr, ppuint64, ppuint64, ppuintptr) ppint64
...
func _ext2fs_read_block_bitmap(*iqlibc.ppTLS, ppuintptr) ppint64

func _ext2fs_test_block_bitmap_range2(*iqlibc.ppTLS, ppuintptr, ppuint64, ppuint32) ppint32
