...
func siadd_to_dirlist(cgtls *iqlibc.ppTLS, aaname ppuintptr, aalist ppuintptr) {
	...
	(*tsdir_list)(iqunsafe.ppPointer(aadp)).fdname = Xmalloc(cgtls, Xstrlen(cgtls, aaname)+ppuint64(1))
	if !((*tsdir_list)(iqunsafe.ppPointer(aadp)).fdname != 0) {
		Xfree(cgtls, aadp)
		return
	}
	Xstrcpy(cgtls, (*tsdir_list)(iqunsafe.ppPointer(aadp)).fdname, aaname)
...
func sifree_dirlist(cgtls *iqlibc.ppTLS, aalist ppuintptr) {
	...
	*(*ppuintptr)(iqunsafe.ppPointer(aalist)) = ppuintptr(0)
}

func siscan_dir(cgtls *iqlibc.ppTLS, aadirname ppuintptr, aadevice tndev_t, aalist ppuintptr, aaret_path ppuintptr) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(1200)
	defer cgtls.ppFree(1200)

	var aacp, aadir, aadp, ccv1 ppuintptr
	var aadirlen, ccv2 ppint32
	var pp_ /* path at bp+0 */ [1024]ppint8
	var pp_ /* st at bp+1024 */ tsstat
	pp_, pp_, pp_, pp_, pp_, pp_ = aacp, aadir, aadirlen, aadp, ccv1, ccv2

	aadirlen = iqlibc.ppInt32FromUint64(Xstrlen(cgtls, aadirname))
	ccv1 = Xopendir(cgtls, aadirname)
	aadir = ccv1
	if ccv1 == iqlibc.ppUintptrFromInt32(0) {
		return *(*ppint32)(iqunsafe.ppPointer(X__error(cgtls)))
	}
	aadp = Xreaddir(cgtls, aadir)
	for aadp != 0 {
		if iqlibc.ppUint64FromInt32(aadirlen)+Xstrlen(cgtls, aadp+21)+ppuint64(2) >= ppuint64(1024) {
			goto ppskip_to_next
		}
		...
		if iqlibc.ppInt32FromUint16((*(*tsstat)(iqunsafe.ppPointer(cgbp + 1024))).fdst_mode)&ppint32(mvS_IFMT) == ppint32(mvS_IFDIR) {
			siadd_to_dirlist(cgtls, cgbp, aalist)
		}
		ccv2 = iqlibc.ppBoolInt32(iqlibc.ppInt32FromUint16((*(*tsstat)(iqunsafe.ppPointer(cgbp + 1024))).fdst_mode)&ppint32(mvS_IFMT) == ppint32(mvS_IFBLK))
		goto cg_3
	cg_3:
		;
		if ccv2 != 0 && (*(*tsstat)(iqunsafe.ppPointer(cgbp + 1024))).fdst_rdev == aadevice {
			aacp = Xmalloc(cgtls, Xstrlen(cgtls, cgbp)+ppuint64(1))
			if !(aacp != 0) {
				Xclosedir(cgtls, aadir)
				return ppint32(mvENOMEM)
			}
			Xstrcpy(cgtls, aacp, cgbp)
			*(*ppuintptr)(iqunsafe.ppPointer(aaret_path)) = aacp
			goto ppsuccess
		}
		goto ppskip_to_next
	ppskip_to_next:
...
//	 */
func Xext2fs_find_block_device(cgtls *iqlibc.ppTLS, aadevice tndev_t) (cgr ppuintptr) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aacurrent ppuintptr
	var aalevel, ccv1 ppint32
	var pp_ /* list at bp+0 */ ppuintptr
	var pp_ /* new_list at bp+8 */ ppuintptr
	var pp_ /* ret_path at bp+16 */ ppuintptr
	pp_, pp_, pp_ = aacurrent, aalevel, ccv1
	...
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)) = ppuintptr(0)
	 ...
	 */
	siadd_to_dirlist(cgtls, "/devices\x00", cgbp)
	siadd_to_dirlist(cgtls, "/devfs\x00", cgbp)
	siadd_to_dirlist(cgtls, "/dev\x00", cgbp)
	...
	for *(*ppuintptr)(iqunsafe.ppPointer(cgbp)) != 0 {
		...
		*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = (*tsdir_list)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)))).fdnext
		siscan_dir(cgtls, (*tsdir_list)(iqunsafe.ppPointer(aacurrent)).fdname, aadevice, cgbp+8, cgbp+16)
		 ...
		 */
		if *(*ppuintptr)(iqunsafe.ppPointer(cgbp)) == ppuintptr(0) {
