...
func siunlink_proc(cgtls *iqlibc.ppTLS, aadirent ppuintptr, aaoffset ppint32, aablocksize ppint32, aabuf ppuintptr, aapriv_data ppuintptr) (cgr ppint32) {

	var aals, aaprev, ccv5 ppuintptr
	var ccv1, ccv3 ppint32
	pp_, pp_, pp_, pp_, pp_ = aals, aaprev, ccv1, ccv3, ccv5
	aals = aapriv_data

	aaprev = (*tslink_struct)(iqunsafe.ppPointer(aals)).fdprev
	(*tslink_struct)(iqunsafe.ppPointer(aals)).fdprev = aadirent

	if (*tslink_struct)(iqunsafe.ppPointer(aals)).fdname != 0 {
		ccv1 = iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdname_len) & ppint32(0xff)
		goto cg_2
	cg_2:
		if ccv1 != (*tslink_struct)(iqunsafe.ppPointer(aals)).fdnamelen {
			return 0
		}
		ccv1 = iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdname_len) & ppint32(0xff)
		goto cg_4
	cg_4:
		if Xstrncmp(cgtls, (*tslink_struct)(iqunsafe.ppPointer(aals)).fdname, aadirent+8, iqlibc.ppUint64FromInt32(ccv1)) != 0 {
			return 0
		}
	}
	if !((*tslink_struct)(iqunsafe.ppPointer(aals)).fdflags&iqlibc.ppInt32FromInt32(mvEXT2FS_UNLINK_FORCE) != 0) && (*tslink_struct)(iqunsafe.ppPointer(aals)).fdinode != 0 {
		if (*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdinode != (*tslink_struct)(iqunsafe.ppPointer(aals)).fdinode {
			return 0
		}
	} else {
	...
	if aaoffset != 0 {
		...
		ccv5 = aaprev + 4
		*(*tn__u16)(iqunsafe.ppPointer(ccv5)) = tn__u16(ppint32(*(*tn__u16)(iqunsafe.ppPointer(ccv5))) + iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdrec_len))
	} else {
		(*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdinode = ppuint32(0)
	}
...
func Xext2fs_unlink(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t, aaname ppuintptr, aaino tnext2_ino_t, aaflags ppint32) (cgr tnerrcode_t) {
	...
	var ccv1 ppuint64
	var ccv2 ppint64
	var pp_ /* ls at bp+0 */ tslink_struct
	...
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp))).fdname = aaname
	if aaname != 0 {
		ccv1 = Xstrlen(cgtls, aaname)
	} else {
		ccv1 = ppuint64(0)
	}
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp))).fdnamelen = iqlibc.ppInt32FromUint64(ccv1)
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp))).fdinode = aaino
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp))).fdflags = aaflags
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp))).fddone = 0
	(*(*tslink_struct)(iqunsafe.ppPointer(cgbp))).fdprev = ppuintptr(0)

	aaretval = Xext2fs_dir_iterate(cgtls, aafs, aadir, ppint32(mvDIRENT_FLAG_INCLUDE_EMPTY), ppuintptr(0), pp__ccgo_fp(siunlink_proc), cgbp)
	if aaretval != 0 {
		return aaretval
	}

	if (*(*tslink_struct)(iqunsafe.ppPointer(cgbp))).fddone != 0 {
		ccv2 = 0
	} else {
		ccv2 = ppint64(2133571366)
	}
	return ccv2
}
