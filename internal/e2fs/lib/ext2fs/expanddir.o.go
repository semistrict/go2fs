...
type tsexpand_dir_struct = struct {
	fddone      ppint32
	fdnewblocks ppint32
	fdgoal      tnblk64_t
	fderr       tnerrcode_t
...
func siexpand_dir_proc(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aablocknr ppuintptr, aablockcnt tne2_blkcnt_t, aaref_block tnblk64_t, aaref_offset ppint32, aapriv_data ppuintptr) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aaes, ccv1 ppuintptr
	var aaretval tnerrcode_t
	var pp_ /* block at bp+16 */ ppuintptr
	var pp_ /* new_blk at bp+8 */ tnblk64_t
	var pp_ /* p at bp+0 */ ppuintptr
	pp_, pp_, pp_ = aaes, aaretval, ccv1
	aaes = aapriv_data

	if *(*tnblk64_t)(iqunsafe.ppPointer(aablocknr)) != 0 {
		if aablockcnt >= 0 {
			(*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fdgoal = *(*tnblk64_t)(iqunsafe.ppPointer(aablocknr))
		}
		return 0
	}
	if aablockcnt != 0 && (*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fdgoal>>(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits == ((*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fdgoal+ppuint64(1))>>(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits {
		*(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8)) = (*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fdgoal + ppuint64(1)
	} else {

		*(*tnblk64_t)(iqunsafe.ppPointer(aaes + 8)) &= iqlibc.ppUint64FromInt32(^(iqlibc.ppInt32FromInt32(1)<<(*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdcluster_ratio_bits - iqlibc.ppInt32FromInt32(1)))
		aaretval = Xext2fs_new_block2(cgtls, aafs, (*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fdgoal, ppuintptr(0), cgbp+8)
		if aaretval != 0 {
			(*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fderr = aaretval
			return ppint32(mvBLOCK_ABORT)
		}
		(*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fdnewblocks = (*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fdnewblocks + 1
		Xext2fs_block_alloc_stats2(cgtls, aafs, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8)), +iqlibc.ppInt32FromInt32(1))
	}
	if aablockcnt > 0 {
		aaretval = Xext2fs_new_dir_block(cgtls, aafs, ppuint32(0), ppuint32(0), cgbp+16)
		if aaretval != 0 {
			(*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fderr = aaretval
			return ppint32(mvBLOCK_ABORT)
		}
		(*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fddone = ppint32(1)
		aaretval = Xext2fs_write_dir_block4(cgtls, aafs, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8)), *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)), 0, (*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fddir)
		ccv1 = cgbp + 16
	...
	cg_2:
		...
		aaretval = Xext2fs_zero_blocks2(cgtls, aafs, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8)), ppint32(1), iqlibc.ppUintptrFromInt32(0), iqlibc.ppUintptrFromInt32(0))
	}
	if aablockcnt >= 0 {
		(*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fdgoal = *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8))
	}
	if aaretval != 0 {
		(*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fderr = aaretval
		return ppint32(mvBLOCK_ABORT)
	}
	*(*tnblk64_t)(iqunsafe.ppPointer(aablocknr)) = *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 8))

	if (*tsexpand_dir_struct)(iqunsafe.ppPointer(aaes)).fddone != 0 {
...
func Xext2fs_expand_dir(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(160)
	defer cgtls.ppFree(160)

	var aaretval tnerrcode_t
	var pp_ /* es at bp+0 */ tsexpand_dir_struct
	var pp_ /* inode at bp+32 */ tsext2_inode
	pp_ = aaretval
	...
	aaretval = Xext2fs_read_inode(cgtls, aafs, aadir, cgbp+32)
	if aaretval != 0 {
		return aaretval
	}

	(*(*tsexpand_dir_struct)(iqunsafe.ppPointer(cgbp))).fddone = 0
	(*(*tsexpand_dir_struct)(iqunsafe.ppPointer(cgbp))).fderr = 0
	(*(*tsexpand_dir_struct)(iqunsafe.ppPointer(cgbp))).fdgoal = Xext2fs_find_inode_goal(cgtls, aafs, aadir, cgbp+32, ppuint64(0))
	(*(*tsexpand_dir_struct)(iqunsafe.ppPointer(cgbp))).fdnewblocks = 0
	(*(*tsexpand_dir_struct)(iqunsafe.ppPointer(cgbp))).fddir = aadir

	aaretval = Xext2fs_block_iterate3(cgtls, aafs, aadir, ppint32(mvBLOCK_FLAG_APPEND), ppuintptr(0), pp__ccgo_fp(siexpand_dir_proc), cgbp)
	if aaretval == ppint64(2133571485) {
		return Xext2fs_inline_data_expand(cgtls, aafs, aadir)
	}

	if (*(*tsexpand_dir_struct)(iqunsafe.ppPointer(cgbp))).fderr != 0 {
		return (*(*tsexpand_dir_struct)(iqunsafe.ppPointer(cgbp))).fderr
	}
	if !((*(*tsexpand_dir_struct)(iqunsafe.ppPointer(cgbp))).fddone != 0) {
		return iqlibc.ppInt64FromInt64(2133571371)
	}
	...
	aaretval = Xext2fs_inode_size_set(cgtls, aafs, cgbp+32, iqlibc.ppInt64FromUint64(ppuint64((*tsext2_inode)(iqunsafe.ppPointer(cgbp+32)).fdi_size)|ppuint64((*tsext2_inode)(iqunsafe.ppPointer(cgbp+32)).fdi_size_high)<<iqlibc.ppInt32FromInt32(32)+ppuint64((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdblocksize)))
	if aaretval != 0 {
		return aaretval
	}
	Xext2fs_iblk_add_blocks(cgtls, aafs, cgbp+32, iqlibc.ppUint64FromInt32((*(*tsexpand_dir_struct)(iqunsafe.ppPointer(cgbp))).fdnewblocks))

	aaretval = Xext2fs_write_inode(cgtls, aafs, aadir, cgbp+32)
	if aaretval != 0 {
		return aaretval
	}
