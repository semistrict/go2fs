...
type tsget_pathname_struct = struct {
	fdsearch_ino tnext2_ino_t
	fdparent     tnext2_ino_t
	fdname       ppuintptr
	fderrcode    tnerrcode_t
}

func siget_pathname_proc(cgtls *iqlibc.ppTLS, aadirent ppuintptr, aaoffset ppint32, aablocksize ppint32, aabuf ppuintptr, aapriv_data ppuintptr) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aagp ppuintptr
	var aaname_len, ccv1 ppint32
	var aaretval, ccv3 tnerrcode_t
	var pp_ /* pp at bp+0 */ ppuintptr
	pp_, pp_, pp_, pp_, pp_ = aagp, aaname_len, aaretval, ccv1, ccv3
...
cg_2:
	...
	if aaname_len == ppint32(2) && !(Xstrncmp(cgtls, aadirent+8, "..\x00", ppuint64(2)) != 0) {
		(*tsget_pathname_struct)(iqunsafe.ppPointer(aagp)).fdparent = (*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdinode
	}
	if (*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdinode == (*tsget_pathname_struct)(iqunsafe.ppPointer(aagp)).fdsearch_ino {
		*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, iqlibc.ppUint64FromInt32(aaname_len+ppint32(1)))
		...
		Xmemcpy(cgtls, aagp+8, cgbp, ppuint64(8))
		ccv3 = 0
		goto cg_4
	cg_4:
		aaretval = ccv3
		if aaretval != 0 {
			(*tsget_pathname_struct)(iqunsafe.ppPointer(aagp)).fderrcode = aaretval
			return ppint32(mvDIRENT_ABORT)
		}
		Xstrncpy(cgtls, (*tsget_pathname_struct)(iqunsafe.ppPointer(aagp)).fdname, aadirent+8, iqlibc.ppUint64FromInt32(aaname_len))
		*(*ppint8)(iqunsafe.ppPointer((*tsget_pathname_struct)(iqunsafe.ppPointer(aagp)).fdname + ppuintptr(aaname_len))) = ppint8('\000')
		return ppint32(mvDIRENT_ABORT)
	}
	return 0
}

func siext2fs_get_pathname_int(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t, aaino tnext2_ino_t, aamaxdepth ppint32, aabuf ppuintptr, aaname ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(112)
	defer cgtls.ppFree(112)

	var aaretval, ccv1, ccv10, ccv4, ccv6, ccv8 tnerrcode_t
	var ccv12, ccv14, ccv3 ppuintptr
	var pp_ /* gp at bp+16 */ tsget_pathname_struct
	var pp_ /* p at bp+8 */ ppuintptr
	var pp_ /* parent_name at bp+40 */ ppuintptr
	var pp_ /* pp at bp+0 */ ppuintptr
	var pp_ /* ret at bp+48 */ ppuintptr
	var pp_ /* tmp at bp+56 */ [32]ppint8
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aaretval, ccv1, ccv10, ccv12, ccv14, ccv3, ccv4, ccv6, ccv8
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 40)) = ppuintptr(0)

	if aadir == aaino {
		*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, ppuint64(2))
		if !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) != 0) {
			ccv1 = iqlibc.ppInt64FromInt64(2133571398)
			goto cg_2
		}
		Xmemcpy(cgtls, aaname, cgbp, ppuint64(8))
		ccv1 = 0
		goto cg_2
	cg_2:
		aaretval = ccv1
		...
		if aadir == ppuint32(mvEXT2_ROOT_INO) {
			ccv3 = "/\x00"
		} else {
			ccv3 = ".\x00"
		}
		Xstrcpy(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(aaname)), ccv3)
		return 0
	}

	if !(aadir != 0) || aamaxdepth < 0 {
		*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, ppuint64(4))
		if !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) != 0) {
			ccv1 = iqlibc.ppInt64FromInt64(2133571398)
			goto cg_5
		}
	...
	cg_5:
		...
		Xstrcpy(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(aaname)), "...\x00")
		return 0
	}

	(*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fdsearch_ino = aaino
	(*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fdparent = ppuint32(0)
	(*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fdname = ppuintptr(0)
	(*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fderrcode = 0

	aaretval = Xext2fs_dir_iterate(cgtls, aafs, aadir, 0, aabuf, pp__ccgo_fp(siget_pathname_proc), cgbp+16)
	if aaretval == ppint64(2133571402) {

		if aaino != 0 {
			X__builtin_snprintf(cgtls, cgbp+56, ppuint64(32), "<%u>/<%u>\x00", iqlibc.ppVaList(cgbp+96, aadir, aaino))
		} else {
			X__builtin_snprintf(cgtls, cgbp+56, ppuint64(32), "<%u>\x00", iqlibc.ppVaList(cgbp+96, aadir))
		}
		*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, Xstrlen(cgtls, cgbp+56)+ppuint64(1))
		if !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) != 0) {
			ccv1 = iqlibc.ppInt64FromInt64(2133571398)
			goto cg_7
		}
	...
	cg_7:
		...
		Xstrcpy(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(aaname)), cgbp+56)
		return 0
	} else {
		if aaretval != 0 {
			goto ppcleanup
	...
	if (*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fderrcode != 0 {
		aaretval = (*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fderrcode
		goto ppcleanup
	}

	aaretval = siext2fs_get_pathname_int(cgtls, aafs, (*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fdparent, aadir, aamaxdepth-ppint32(1), aabuf, cgbp+40)
	if aaretval != 0 {
		goto ppcleanup
	}
	if !(aaino != 0) {
		*(*ppuintptr)(iqunsafe.ppPointer(aaname)) = *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 40))
		return 0
	}

	if (*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fdname != 0 {
		*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, Xstrlen(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 40)))+Xstrlen(cgtls, (*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fdname)+ppuint64(2))
		if !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) != 0) {
			ccv1 = iqlibc.ppInt64FromInt64(2133571398)
			goto cg_9
		}
	...
	cg_9:
		...
		*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xmalloc(cgtls, Xstrlen(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 40)))+ppuint64(5))
		if !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) != 0) {
			ccv1 = iqlibc.ppInt64FromInt64(2133571398)
			goto cg_11
		}
	...
	*(*ppint8)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 48)))) = 0
	if *(*ppint8)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 40)) + 1)) != 0 {
		Xstrcat(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 48)), *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 40)))
	}
	Xstrcat(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 48)), "/\x00")
	if (*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fdname != 0 {
		Xstrcat(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 48)), (*(*tsget_pathname_struct)(iqunsafe.ppPointer(cgbp + 16))).fdname)
	} else {
		Xstrcat(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 48)), "???\x00")
	}
	*(*ppuintptr)(iqunsafe.ppPointer(aaname)) = *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 48))
	aaretval = 0
...
func Xext2fs_get_pathname(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t, aaino tnext2_ino_t, aaname ppuintptr) (cgr tnerrcode_t) {
...
cg_2:
	...
	aaretval = siext2fs_get_pathname_int(cgtls, aafs, aadir, aaino, ppint32(32), *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)), aaname)
	ccv3 = cgbp + 16
	Xmemcpy(cgtls, cgbp+8, ccv3, ppuint64(8))
	Xfree(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 8)))
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 8)) = ppuintptr(0)
...
func _ext2fs_dir_iterate(*iqlibc.ppTLS, ppuintptr, ppuint32, ppint32, ppuintptr, ppuintptr, ppuintptr) ppint64
