...
//	 */
func Xext2fs_check_desc(cgtls *iqlibc.ppTLS, aafs tnext2_filsys) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aab, aablk, aafirst_block, aalast_block tnblk64_t
	var aai tndgrp_t
	var aaj ppuint32
	var aaretval tnerrcode_t
	var ccv1, ccv11, ccv15, ccv2, ccv20, ccv4, ccv5, ccv9 ppint32
	var ccv13, ccv17 ppbool
	var pp_ /* bmap at bp+0 */ tnext2fs_block_bitmap
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aab, aablk, aafirst_block, aai, aaj, aalast_block, aaretval, ccv1, ccv11, ccv13, ccv15, ccv17, ccv2, ccv20, ccv4, ccv5, ccv9
	aafirst_block = ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block)
	aalast_block = Xext2fs_blocks_count(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper) - ppuint64(1)
...
cg_6:
	...
	if ccv1&(ccv4-ppint32(1)) != 0 {
		return iqlibc.ppInt64FromInt64(2133571473)
	}

	aaretval = Xext2fs_allocate_subcluster_bitmap(cgtls, aafs, "check_desc map\x00", cgbp)
	if aaretval != 0 {
		return aaretval
	}
	...
	for {
		...
		Xext2fs_reserve_super_and_bgd(cgtls, aafs, aai, *(*tnext2fs_block_bitmap)(iqunsafe.ppPointer(cgbp)))
		goto cg_7
	cg_7:
		;
		aai = aai + 1
	...
	cg_10:
		if !(ccv1 != 0) {
			aafirst_block = Xext2fs_group_first_block2(cgtls, aafs, aai)
			aalast_block = Xext2fs_group_last_block2(cgtls, aafs, aai)
		}
		...
		if ccv13 = aablk < aafirst_block || aablk > aalast_block; !ccv13 {
			ccv1 = Xext2fs_test_generic_bmap(cgtls, *(*tnext2fs_block_bitmap)(iqunsafe.ppPointer(cgbp)), aablk)
			goto cg_12
		cg_12:
		}
		if ccv13 || ccv1 != 0 {
			aaretval = iqlibc.ppInt64FromInt64(2133571352)
			goto pperrout
		}
		pp_ = Xext2fs_mark_generic_bmap(cgtls, *(*tnext2fs_block_bitmap)(iqunsafe.ppPointer(cgbp)), aablk)
		goto cg_14
	cg_14:
		;
		...
		if ccv13 || ccv1 != 0 {
			aaretval = iqlibc.ppInt64FromInt64(2133571353)
			goto pperrout
		}
		pp_ = Xext2fs_mark_generic_bmap(cgtls, *(*tnext2fs_block_bitmap)(iqunsafe.ppPointer(cgbp)), aablk)
		goto cg_18
	cg_18:
		...
		if aablk < aafirst_block || aablk+ppuint64((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdinode_blocks_per_group)-ppuint64(1) > aalast_block {
		...
		aab = aablk
		for {
			if !(aaj < (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdinode_blocks_per_group) {
				break
			}
			ccv1 = Xext2fs_test_generic_bmap(cgtls, *(*tnext2fs_block_bitmap)(iqunsafe.ppPointer(cgbp)), aab)
			goto cg_21
		cg_21:
			if ccv1 != 0 {
				aaretval = iqlibc.ppInt64FromInt64(2133571354)
			...
			pp_ = Xext2fs_mark_generic_bmap(cgtls, *(*tnext2fs_block_bitmap)(iqunsafe.ppPointer(cgbp)), aab)
			goto cg_22
		cg_22:
			;
...
pperrout:
	;
	Xext2fs_free_block_bitmap(cgtls, *(*tnext2fs_block_bitmap)(iqunsafe.ppPointer(cgbp)))
	return aaretval
}
