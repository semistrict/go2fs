...
type tsexit_data = struct {
	fdfunc1 tnext2_exit_fn
	fddata  ppuintptr
}

var siitems ppuintptr
var sinr_items tnsize_t

func sihandle_exit(cgtls *iqlibc.ppTLS) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aaed, ccv2 ppuintptr
	var pp_ /* p at bp+0 */ ppuintptr
	pp_, pp_ = aaed, ccv2

	aaed = siitems + ppuintptr(sinr_items)*16 - ppuintptr(1)*16
	for {
		if !(aaed >= siitems) {
			break
		}
		if (*tsexit_data)(iqunsafe.ppPointer(aaed)).fdfunc1 == iqlibc.ppUintptrFromInt32(0) {
			goto cg_1
		}
		(*(*func(*iqlibc.ppTLS, ppuintptr))(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tsexit_data)(iqunsafe.ppPointer(aaed)).fdfunc1})))(cgtls, (*tsexit_data)(iqunsafe.ppPointer(aaed)).fddata)
	...
	ccv2 = ppuintptr(iqunsafe.ppPointer(&siitems))
...
cg_3:
	;
	sinr_items = ppuint64(0)
}

type tn__ccgo_fp__Xext2fs_add_exit_fn_0 = func(*iqlibc.ppTLS, ppuintptr)
...
//	 */
func Xext2fs_add_exit_fn(cgtls *iqlibc.ppTLS, aa__ccgo_fp_func tnext2_exit_fn, aadata ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aaed, aafree_ed, ccv2 ppuintptr
	var aaret, ccv3 tnerrcode_t
	var aax tnsize_t
	var pp_ /* p at bp+0 */ ppuintptr
	pp_, pp_, pp_, pp_, pp_, pp_ = aaed, aafree_ed, aaret, aax, ccv2, ccv3
	aafree_ed = iqlibc.ppUintptrFromInt32(0)

	if aa__ccgo_fp_func == iqlibc.ppUintptrFromInt32(0) {
	...
	for {
		...
		if (*tsexit_data)(iqunsafe.ppPointer(aaed)).fdfunc1 == aa__ccgo_fp_func && (*tsexit_data)(iqunsafe.ppPointer(aaed)).fddata == aadata {
			return iqlibc.ppInt64FromInt64(2133571483)
		}
		if (*tsexit_data)(iqunsafe.ppPointer(aaed)).fdfunc1 == iqlibc.ppUintptrFromInt32(0) {
			aafree_ed = aaed
		}
	...
	if aafree_ed != 0 {
		(*tsexit_data)(iqunsafe.ppPointer(aafree_ed)).fdfunc1 = aa__ccgo_fp_func
		(*tsexit_data)(iqunsafe.ppPointer(aafree_ed)).fddata = aadata
		return 0
	}

	if sinr_items == ppuint64(0) {
		aaret = ppint64(Xatexit(cgtls, pp__ccgo_fp(sihandle_exit)))
		if aaret != 0 {
			return aaret
		}
	}
	...
	ccv2 = ppuintptr(iqunsafe.ppPointer(&siitems))
	Xmemcpy(cgtls, cgbp, ccv2, ppuint64(8))
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = Xrealloc(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp)), (sinr_items+ppuint64(1))*ppuint64(16))
...
cg_4:
	...
	(*(*tsexit_data)(iqunsafe.ppPointer(siitems + ppuintptr(sinr_items)*16))).fdfunc1 = aa__ccgo_fp_func
	(*(*tsexit_data)(iqunsafe.ppPointer(siitems + ppuintptr(sinr_items)*16))).fddata = aadata
	sinr_items = sinr_items + 1

	return 0
}

type tn__ccgo_fp__Xext2fs_remove_exit_fn_0 = func(*iqlibc.ppTLS, ppuintptr)

// C documentation
//
//	/* Remove a function from the exit cleanup list. */
func Xext2fs_remove_exit_fn(cgtls *iqlibc.ppTLS, aa__ccgo_fp_func tnext2_exit_fn, aadata ppuintptr) (cgr tnerrcode_t) {

	var aaed ppuintptr
	var aasz, aax tnsize_t
	pp_, pp_, pp_ = aaed, aasz, aax
	...
	for {
		...
		if (*tsexit_data)(iqunsafe.ppPointer(aaed)).fdfunc1 == aa__ccgo_fp_func && (*tsexit_data)(iqunsafe.ppPointer(aaed)).fddata == aadata {
			aasz = (sinr_items - (aax + ppuint64(1))) * ppuint64(16)
			Xmemmove(cgtls, aaed, aaed+ppuintptr(1)*16, aasz)
			Xmemset(cgtls, siitems+ppuintptr(sinr_items)*16-ppuintptr(1)*16, 0, ppuint64(16))
		}
...
func _atexit(*iqlibc.ppTLS, ppuintptr) ppint32
