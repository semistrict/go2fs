...
func silookup_proc(cgtls *iqlibc.ppTLS, aadirent ppuintptr, aaoffset ppint32, aablocksize ppint32, aabuf ppuintptr, aapriv_data ppuintptr) (cgr ppint32) {

	var aals ppuintptr
	var ccv1, ccv3 ppint32
	pp_, pp_, pp_ = aals, ccv1, ccv3
	aals = aapriv_data

	ccv1 = iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdname_len) & ppint32(0xff)
	goto cg_2
cg_2:
	if (*tslookup_struct)(iqunsafe.ppPointer(aals)).fdlen1 != ccv1 {
		return 0
	}
	ccv1 = iqlibc.ppInt32FromUint16((*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdname_len) & ppint32(0xff)
	goto cg_4
cg_4:
	if Xstrncmp(cgtls, (*tslookup_struct)(iqunsafe.ppPointer(aals)).fdname, aadirent+8, iqlibc.ppUint64FromInt32(ccv1)) != 0 {
		return 0
	}
	*(*tnext2_ino_t)(iqunsafe.ppPointer((*tslookup_struct)(iqunsafe.ppPointer(aals)).fdinode)) = (*tsext2_dir_entry)(iqunsafe.ppPointer(aadirent)).fdinode
	(*tslookup_struct)(iqunsafe.ppPointer(aals)).fdfound = (*tslookup_struct)(iqunsafe.ppPointer(aals)).fdfound + 1
	return ppint32(mvDIRENT_ABORT)
}

func Xext2fs_lookup(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aadir tnext2_ino_t, aaname ppuintptr, aanamelen ppint32, aabuf ppuintptr, aainode ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aaretval tnerrcode_t
	var ccv1 ppint64
	var pp_ /* ls at bp+0 */ tslookup_struct
	...
	(*(*tslookup_struct)(iqunsafe.ppPointer(cgbp))).fdname = aaname
	(*(*tslookup_struct)(iqunsafe.ppPointer(cgbp))).fdlen1 = aanamelen
	(*(*tslookup_struct)(iqunsafe.ppPointer(cgbp))).fdinode = aainode
	(*(*tslookup_struct)(iqunsafe.ppPointer(cgbp))).fdfound = 0

	aaretval = Xext2fs_dir_iterate(cgtls, aafs, aadir, 0, aabuf, pp__ccgo_fp(silookup_proc), cgbp)
	if aaretval != 0 {
		return aaretval
	}

	if (*(*tslookup_struct)(iqunsafe.ppPointer(cgbp))).fdfound != 0 {
		ccv1 = 0
	} else {
		ccv1 = ppint64(2133571404)
	}
	return ccv1
}
