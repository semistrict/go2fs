...
type tn__ccgo_fp__Xext2fs_read_bb_FILE2_4 = func(*iqlibc.ppTLS, ppuintptr, ppuint32, ppuintptr, ppuintptr)
...
//	 */
func Xext2fs_read_bb_FILE2(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaf ppuintptr, aabb_list ppuintptr, aapriv_data ppuintptr, aa__ccgo_fp_invalid ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(160)
	defer cgtls.ppFree(160)

	var aacount ppint32
	var aaretval tnerrcode_t
	var pp_ /* blockno at bp+0 */ ppuint64
	var pp_ /* buf at bp+8 */ [128]ppint8
	pp_, pp_ = aacount, aaretval
	...
	if !(*(*tnext2_badblocks_list)(iqunsafe.ppPointer(aabb_list)) != 0) {
		aaretval = Xext2fs_badblocks_list_create(cgtls, aabb_list, ppint32(10))
		if aaretval != 0 {
			return aaretval
		}
	}
	...
	for !(Xfeof(cgtls, aaf) != 0) {
		if Xfgets(cgtls, cgbp+8, ppint32(128), aaf) == iqlibc.ppUintptrFromInt32(0) {
			break
		}
		aacount = Xsscanf(cgtls, cgbp+8, "%llu\x00", iqlibc.ppVaList(cgbp+144, cgbp))
		if aacount <= 0 {
			continue
		}
		/* Badblocks isn't going to be updated for 64bit */
		if *(*ppuint64)(iqunsafe.ppPointer(cgbp))>>ppint32(32) != 0 {
			return ppint64(mvEOVERFLOW)
		}
		if aafs != 0 && (*(*ppuint64)(iqunsafe.ppPointer(cgbp)) < ppuint64((*tsext2_super_block)(iqunsafe.ppPointer((*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)).fds_first_data_block) || *(*ppuint64)(iqunsafe.ppPointer(cgbp)) >= Xext2fs_blocks_count(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdsuper)) {
			if aa__ccgo_fp_invalid != 0 {
				(*(*func(*iqlibc.ppTLS, tnext2_filsys, tnblk_t, ppuintptr, ppuintptr))(iqunsafe.ppPointer(&struct{ ppuintptr }{aa__ccgo_fp_invalid})))(cgtls, aafs, ppuint32(*(*ppuint64)(iqunsafe.ppPointer(cgbp))), cgbp+8, aapriv_data)
			}
			continue
		}
		aaretval = Xext2fs_badblocks_list_add(cgtls, *(*tnext2_badblocks_list)(iqunsafe.ppPointer(aabb_list)), ppuint32(*(*ppuint64)(iqunsafe.ppPointer(cgbp))))
		if aaretval != 0 {
			return aaretval
		}
	}
...
type tscompat_struct = struct {
	fdinvalid ppuintptr
}

func sicall_compat_invalid(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aablk tnblk_t, aabadstr ppuintptr, aapriv_data ppuintptr) {

	var aast ppuintptr
	pp_ = aast

	aast = aapriv_data
	if (*tscompat_struct)(iqunsafe.ppPointer(aast)).fdinvalid != 0 {
		(*(*func(*iqlibc.ppTLS, tnext2_filsys, tnblk_t))(iqunsafe.ppPointer(&struct{ ppuintptr }{(*tscompat_struct)(iqunsafe.ppPointer(aast)).fdinvalid})))(cgtls, aafs, aablk)
	}
}

type tn__ccgo_fp__Xext2fs_read_bb_FILE_3 = func(*iqlibc.ppTLS, ppuintptr, ppuint32)
...
//	 * Reads a list of bad blocks from  a FILE *
//	 */
func Xext2fs_read_bb_FILE(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaf ppuintptr, aabb_list ppuintptr, aa__ccgo_fp_invalid ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var pp_ /* st at bp+0 */ tscompat_struct

	(*(*tscompat_struct)(iqunsafe.ppPointer(cgbp))).fdinvalid = aa__ccgo_fp_invalid

	return Xext2fs_read_bb_FILE2(cgtls, aafs, aaf, aabb_list, cgbp, pp__ccgo_fp(sicall_compat_invalid))
}
