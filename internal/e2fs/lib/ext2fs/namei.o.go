...
func sifollow_link(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaroot tnext2_ino_t, aadir tnext2_ino_t, aainode tnext2_ino_t, aalink_count ppint32, aabuf ppuintptr, aares_inode ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(176)
	defer cgtls.ppFree(176)

	var aapathname, ccv11, ccv5, ccv9 ppuintptr
	var aaretval, ccv3, ccv7 tnerrcode_t
	var ccv1 ppint32
	var ccv2 ppuint64
	var pp_ /* blk at bp+160 */ tnblk64_t
	var pp_ /* buffer at bp+24 */ ppuintptr
	var pp_ /* ei at bp+32 */ tsext2_inode
	var pp_ /* p at bp+16 */ ppuintptr
	var pp_ /* pp at bp+0 */ ppuintptr
	var pp_ /* pp at bp+8 */ ppuintptr
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aapathname, aaretval, ccv1, ccv11, ccv2, ccv3, ccv5, ccv7, ccv9
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)) = ppuintptr(0)

	aaretval = Xext2fs_read_inode(cgtls, aafs, aainode, cgbp+32)
	if aaretval != 0 {
		return aaretval
	}
	if !(iqlibc.ppInt32FromUint16((*(*tsext2_inode)(iqunsafe.ppPointer(cgbp + 32))).fdi_mode)&iqlibc.ppInt32FromInt32(mvLINUX_S_IFMT) == iqlibc.ppInt32FromInt32(mvLINUX_S_IFLNK)) {
		*(*tnext2_ino_t)(iqunsafe.ppPointer(aares_inode)) = aainode
		return 0
	}
	ccv1 = aalink_count
	aalink_count = aalink_count + 1
	if ccv1 >= ppint32(mvEXT2FS_MAX_NESTED_LINKS) {
		return iqlibc.ppInt64FromInt64(2133571392)
	}

	if Xext2fs_is_fast_symlink(cgtls, cgbp+32) != 0 {
		aapathname = cgbp + 32 + 40
	} else {
		if (*(*tsext2_inode)(iqunsafe.ppPointer(cgbp + 32))).fdi_flags&ppuint32(mvEXT4_INLINE_DATA_FL) != 0 {
			ccv2 = ppuint64((*(*tsext2_inode)(iqunsafe.ppPointer(cgbp + 32))).fdi_size)
			*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 8)) = Xmalloc(cgtls, ccv2)
			if !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 8)) != 0) {
				ccv3 = iqlibc.ppInt64FromInt64(2133571398)
				goto cg_4
			}
			Xmemset(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 8)), 0, ccv2)
		...
		cg_4:
			...
			aaretval = Xext2fs_inline_data_get(cgtls, aafs, aainode, cgbp+32, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)), iqlibc.ppUintptrFromInt32(0))
			if aaretval != 0 {
				ccv5 = cgbp + 24
				Xmemcpy(cgtls, cgbp+16, ccv5, ppuint64(8))
				Xfree(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)))
				*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)) = ppuintptr(0)
				Xmemcpy(cgtls, ccv5, cgbp+16, ppuint64(8))
				pp_ = 0
				goto cg_6
			cg_6:
				;
			...
			aapathname = *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24))
		} else {
			aaretval = Xext2fs_bmap2(cgtls, aafs, aainode, cgbp+32, iqlibc.ppUintptrFromInt32(0), 0, ppuint64(0), iqlibc.ppUintptrFromInt32(0), cgbp+160)
			if aaretval != 0 {
				return aaretval
			}
		...
		cg_8:
			...
			aaretval = Xio_channel_read_blk64(cgtls, (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(aafs)).fdio, *(*tnblk64_t)(iqunsafe.ppPointer(cgbp + 160)), ppint32(1), *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 24)))
			if aaretval != 0 {
				ccv5 = cgbp + 24
				Xmemcpy(cgtls, cgbp+16, ccv5, ppuint64(8))
				Xfree(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)))
	...
	aaretval = siopen_namei(cgtls, aafs, aaroot, aadir, aapathname, ppuint64((*(*tsext2_inode)(iqunsafe.ppPointer(cgbp + 32))).fdi_size), ppint32(1), aalink_count, aabuf, aares_inode)
...
//	 */
func sidir_namei(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaroot tnext2_ino_t, cg_dir tnext2_ino_t, aapathname ppuintptr, aapathlen ppint32, aalink_count ppint32, aabuf ppuintptr, aaname ppuintptr, aanamelen ppuintptr, aares_inode ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)
	*(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp)) = cg_dir

	var aac, ccv1 ppint8
	var aalen, ccv3 ppint32
	var aaretval tnerrcode_t
	var aathisname, ccv4 ppuintptr
	var pp_ /* inode at bp+4 */ tnext2_ino_t
	pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aac, aalen, aaretval, aathisname, ccv1, ccv3, ccv4

	ccv1 = *(*ppint8)(iqunsafe.ppPointer(aapathname))
	aac = ccv1
	if ppint32(ccv1) == ppint32('/') {
		*(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp)) = aaroot
		aapathname = aapathname + 1
		aapathlen = aapathlen - 1
	}
	for ppint32(1) != 0 {
		aathisname = aapathname
		aalen = 0
		for {
			aapathlen = aapathlen - 1
			ccv3 = aapathlen
			...
			ccv4 = aapathname
			aapathname = aapathname + 1
			aac = *(*ppint8)(iqunsafe.ppPointer(ccv4))
			if ppint32(aac) == ppint32('/') {
				break
			}
		...
		aaretval = Xext2fs_lookup(cgtls, aafs, *(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp)), aathisname, aalen, aabuf, cgbp+4)
		if aaretval != 0 {
			return aaretval
		}
		aaretval = sifollow_link(cgtls, aafs, aaroot, *(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp)), *(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp + 4)), aalink_count, aabuf, cgbp)
		if aaretval != 0 {
			return aaretval
		}
	}
	*(*ppuintptr)(iqunsafe.ppPointer(aaname)) = aathisname
	*(*ppint32)(iqunsafe.ppPointer(aanamelen)) = aalen
	*(*tnext2_ino_t)(iqunsafe.ppPointer(aares_inode)) = *(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp))
	return 0
}

func siopen_namei(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaroot tnext2_ino_t, aabase tnext2_ino_t, aapathname ppuintptr, aapathlen tnsize_t, aafollow ppint32, aalink_count ppint32, aabuf ppuintptr, aares_inode ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aaretval tnerrcode_t
	var pp_ /* base_name at bp+0 */ ppuintptr
	var pp_ /* dir at bp+12 */ tnext2_ino_t
	var pp_ /* inode at bp+16 */ tnext2_ino_t
	var pp_ /* namelen at bp+8 */ ppint32
	pp_ = aaretval

	aaretval = sidir_namei(cgtls, aafs, aaroot, aabase, aapathname, iqlibc.ppInt32FromUint64(aapathlen), aalink_count, aabuf, cgbp, cgbp+8, cgbp+12)
	if aaretval != 0 {
		return aaretval
	}
	if !(*(*ppint32)(iqunsafe.ppPointer(cgbp + 8)) != 0) { /* special case: '/usr/' etc */
		*(*tnext2_ino_t)(iqunsafe.ppPointer(aares_inode)) = *(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp + 12))
		return 0
	}
	aaretval = Xext2fs_lookup(cgtls, aafs, *(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp + 12)), *(*ppuintptr)(iqunsafe.ppPointer(cgbp)), *(*ppint32)(iqunsafe.ppPointer(cgbp + 8)), aabuf, cgbp+16)
	if aaretval != 0 {
		return aaretval
	}
	if aafollow != 0 {
		aaretval = sifollow_link(cgtls, aafs, aaroot, *(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp + 12)), *(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp + 16)), aalink_count, aabuf, cgbp+16)
		if aaretval != 0 {
			return aaretval
		}
	}
	*(*tnext2_ino_t)(iqunsafe.ppPointer(aares_inode)) = *(*tnext2_ino_t)(iqunsafe.ppPointer(cgbp + 16))
	return 0
}

func Xext2fs_namei(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaroot tnext2_ino_t, aacwd tnext2_ino_t, aaname ppuintptr, aainode ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
...
cg_2:
	...
	aaretval = siopen_namei(cgtls, aafs, aaroot, aacwd, aaname, Xstrlen(cgtls, aaname), 0, 0, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)), aainode)
...
func Xext2fs_namei_follow(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaroot tnext2_ino_t, aacwd tnext2_ino_t, aaname ppuintptr, aainode ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
...
cg_2:
	...
	aaretval = siopen_namei(cgtls, aafs, aaroot, aacwd, aaname, Xstrlen(cgtls, aaname), ppint32(1), 0, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)), aainode)
...
func Xext2fs_follow_link(cgtls *iqlibc.ppTLS, aafs tnext2_filsys, aaroot tnext2_ino_t, aacwd tnext2_ino_t, aainode tnext2_ino_t, aares_inode ppuintptr) (cgr tnerrcode_t) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)
...
cg_2:
	...
	aaretval = sifollow_link(cgtls, aafs, aaroot, aacwd, aainode, 0, *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)), aares_inode)
