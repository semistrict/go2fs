...
const mvno_argument = 0
const mvoptional_argument = 2
const mvrequired_argument = 1
...
func Xparse_c_string(cgtls *iqlibc.ppTLS, aastr ppuintptr) (cgr ppint32) {

	var aach ppint8
	var aafrom, aato, ccv1, ccv3, ccv4 ppuintptr
	var aav ppint32
	pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aach, aafrom, aato, aav, ccv1, ccv3, ccv4
	...
	for {
		...
		if ppint32(*(*ppint8)(iqunsafe.ppPointer(aafrom))) == ppint32('\\') {
			aafrom = aafrom + 1
			ccv3 = aafrom
			aach = *(*ppint8)(iqunsafe.ppPointer(ccv3))
			switch ppint32(aach) {
			case ppint32('a'):
				*(*ppint8)(iqunsafe.ppPointer(aato)) = ppint8('\a')
				break
				fallthrough
			case ppint32('b'):
				*(*ppint8)(iqunsafe.ppPointer(aato)) = ppint8('\b')
				...
				fallthrough
			case ppint32('f'):
				*(*ppint8)(iqunsafe.ppPointer(aato)) = ppint8('\f')
				break
				fallthrough
			case ppint32('n'):
			...
			case ppint32('t'):
				*(*ppint8)(iqunsafe.ppPointer(aato)) = ppint8('\t')
				...
				fallthrough
			case ppint32('v'):
				*(*ppint8)(iqunsafe.ppPointer(aato)) = ppint8('\v')
				break
				fallthrough
			case ppint32('x'):
				aach = *(*ppint8)(iqunsafe.ppPointer(aafrom + iqlibc.ppUintptrFromInt32(1)))
				if ppint32(aach) >= ppint32('a') && ppint32(aach) <= ppint32('f') {
					aach = ppint8(ppint32(aach) - ppint32('a') + ppint32('A'))
				}
				if ppint32(aach) >= ppint32('0') && ppint32(aach) <= ppint32('9') {
					aav = ppint32(aach) - ppint32('0')
				} else {
					if ppint32(aach) >= ppint32('A') && ppint32(aach) <= ppint32('F') {
						aav = ppint32(aach) + ppint32(10) - ppint32('A')
					} else {
						*(*ppint8)(iqunsafe.ppPointer(aato)) = ppint8('x')
						break
					}
				}
				aafrom = aafrom + 1
				...
				if ppint32(aach) >= ppint32('0') && ppint32(aach) <= ppint32('9') {
					aav = aav*ppint32(16) + (ppint32(aach) - ppint32('0'))
				} else {
					if ppint32(aach) >= ppint32('A') && ppint32(aach) <= ppint32('F') {
						aav = aav*ppint32(16) + (ppint32(aach) + ppint32(10) - ppint32('A'))
					} else {
						*(*ppint8)(iqunsafe.ppPointer(aato)) = ppint8('x')
						aafrom = aafrom - 1
						break
				...
				aafrom = aafrom + 1
				*(*ppint8)(iqunsafe.ppPointer(aato)) = ppint8(aav)
				break
				fallthrough
			default:
				if ppint32(aach) >= ppint32('0') && ppint32(aach) <= ppint32('9') {
					...
					if ppint32(aach) >= ppint32('0') && ppint32(aach) <= ppint32('9') {
						aafrom = aafrom + 1
						aav = ppint32(8)*aav + (ppint32(aach) - ppint32('0'))
						aach = *(*ppint8)(iqunsafe.ppPointer(aafrom + iqlibc.ppUintptrFromInt32(1)))
						if ppint32(aach) >= ppint32('0') && ppint32(aach) <= ppint32('9') {
							aafrom = aafrom + 1
							aav = ppint32(8)*aav + (ppint32(aach) - ppint32('0'))
				...
				*(*ppint8)(iqunsafe.ppPointer(aato)) = aach
			}
			goto cg_2
		}
	...
	*(*ppint8)(iqunsafe.ppPointer(aato)) = ppint8('\000')
	return ppint32(ppint64(aato) - ppint64(aastr))
}

func Xprint_c_string(cgtls *iqlibc.ppTLS, aaf ppuintptr, aacp ppuintptr, aalen ppint32) {
	...
	var ccv1 ppint32
	var ccv2 ppuintptr
	pp_, pp_, pp_ = aach, ccv1, ccv2

	if aalen < 0 {
		aalen = iqlibc.ppInt32FromUint64(Xstrlen(cgtls, aacp))
	}
	...
	for {
		...
		ccv2 = aacp
		aacp = aacp + 1
		aach = iqlibc.ppUint8FromInt8(*(*ppint8)(iqunsafe.ppPointer(ccv2)))
		if iqlibc.ppInt32FromUint8(aach) == ppint32('\a') {
			Xfputs(cgtls, "\\a\x00", aaf)
		} else {
			if iqlibc.ppInt32FromUint8(aach) == ppint32('\b') {
				Xfputs(cgtls, "\\b\x00", aaf)
			} else {
				if iqlibc.ppInt32FromUint8(aach) == ppint32('\f') {
					Xfputs(cgtls, "\\f\x00", aaf)
				} else {
					if iqlibc.ppInt32FromUint8(aach) == ppint32('\n') {
						Xfputs(cgtls, "\\n\x00", aaf)
					} else {
						if iqlibc.ppInt32FromUint8(aach) == ppint32('\t') {
							Xfputs(cgtls, "\\t\x00", aaf)
						} else {
							if iqlibc.ppInt32FromUint8(aach) == ppint32('\v') {
								Xfputs(cgtls, "\\v\x00", aaf)
							} else {
								if iqlibc.ppInt32FromUint8(aach) == ppint32('\\') {
									Xfputs(cgtls, "\\\\\x00", aaf)
								} else {
									if iqlibc.ppInt32FromUint8(aach) == ppint32('\'') {
										...
										if iqlibc.ppInt32FromUint8(aach) == ppint32('"') {
											...
											if iqlibc.ppInt32FromUint8(aach) < ppint32(32) || iqlibc.ppInt32FromUint8(aach) > ppint32(126) {
												Xfprintf(cgtls, aaf, "\\%03o\x00", iqlibc.ppVaList(cgbp+8, iqlibc.ppInt32FromUint8(aach)))
											} else {
												Xfputc(cgtls, iqlibc.ppInt32FromUint8(aach), aaf)
											}
										}
									}
								}
