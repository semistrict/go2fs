...
const mvCHECK_BLOCK_DEV = 1
const mvCHECK_FS_EXIST = 4
...
func siprint_ext2_info(cgtls *iqlibc.ppTLS, aadevice ppuintptr) {
	cgbp := cgtls.ppAlloc(48)
	defer cgtls.ppFree(48)

	var aaretval tnerrcode_t
	var aasb ppuintptr
	var ccv1, ccv2, ccv4, ccv5, ccv7, ccv8 tntime_t
	var pp_ /* fs at bp+0 */ tnext2_filsys
	var pp_ /* tm at bp+8 */ tntime_t
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aaretval, aasb, ccv1, ccv2, ccv4, ccv5, ccv7, ccv8

	aaretval = Xext2fs_open2(cgtls, aadevice, ppuintptr(0), ppint32(mvEXT2_FLAG_64BITS), 0, ppuint32(0), Xunix_io_manager, cgbp)
	if aaretval != 0 {
		return
	}
	aasb = (*tsstruct_ext2_filsys)(iqunsafe.ppPointer(*(*tnext2_filsys)(iqunsafe.ppPointer(cgbp)))).fdsuper

	ccv2 = iqlibc.ppInt64FromUint8(*(*tn__u8)(iqunsafe.ppPointer(aasb + 629)))<<iqlibc.ppInt32FromInt32(32) | iqlibc.ppInt64FromUint32(*(*tn__u32)(iqunsafe.ppPointer(aasb + 44)))
	goto cg_3
cg_3:
	ccv1 = ccv2
	*(*tntime_t)(iqunsafe.ppPointer(cgbp + 8)) = ccv1
	if ccv1 != 0 {
		if *(*tn__u8)(iqunsafe.ppPointer(aasb + 136)) != 0 {
			Xprintf(cgtls, "\tlast mounted on %.*s on %s\x00", iqlibc.ppVaList(cgbp+24, iqlibc.ppInt32FromInt64(64), aasb+136, Xctime(cgtls, cgbp+8)))
		} else {
			Xprintf(cgtls, "\tlast mounted on %s\x00", iqlibc.ppVaList(cgbp+24, Xctime(cgtls, cgbp+8)))
		}
	} else {
		ccv2 = iqlibc.ppInt64FromUint8(*(*tn__u8)(iqunsafe.ppPointer(aasb + 630)))<<iqlibc.ppInt32FromInt32(32) | iqlibc.ppInt64FromUint32(*(*tn__u32)(iqunsafe.ppPointer(aasb + 264)))
		goto cg_6
	cg_6:
		ccv1 = ccv2
		*(*tntime_t)(iqunsafe.ppPointer(cgbp + 8)) = ccv1
		if ccv1 != 0 {
			Xprintf(cgtls, "\tcreated on %s\x00", iqlibc.ppVaList(cgbp+24, Xctime(cgtls, cgbp+8)))
		} else {
			ccv2 = iqlibc.ppInt64FromUint8(*(*tn__u8)(iqunsafe.ppPointer(aasb + 628)))<<iqlibc.ppInt32FromInt32(32) | iqlibc.ppInt64FromUint32(*(*tn__u32)(iqunsafe.ppPointer(aasb + 48)))
			goto cg_9
		cg_9:
			ccv1 = ccv2
			*(*tntime_t)(iqunsafe.ppPointer(cgbp + 8)) = ccv1
			if ccv1 != 0 {
				Xprintf(cgtls, "\tlast modified on %s\x00", iqlibc.ppVaList(cgbp+24, Xctime(cgtls, cgbp+8)))
			}
		}
	}
...
//	 */
func sicheck_partition_table(cgtls *iqlibc.ppTLS, aadevice ppuintptr) (cgr ppint32) {
...
//	 */
func Xcheck_plausibility(cgtls *iqlibc.ppTLS, aadevice ppuintptr, aaflags ppint32, aaret_is_dev ppuintptr) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(192)
	defer cgtls.ppFree(192)

	var aafd, aafl, aais_dev, aaret ppint32
	var aafs_label, aafs_type ppuintptr
	var pp_ /* cache at bp+144 */ tnblkid_cache
	var pp_ /* s at bp+0 */ tnext2fs_struct_stat
	pp_, pp_, pp_, pp_, pp_, pp_ = aafd, aafl, aafs_label, aafs_type, aais_dev, aaret
	aais_dev = 0
	aafl = mvO_RDONLY
	*(*tnblkid_cache)(iqunsafe.ppPointer(cgbp + 144)) = iqlibc.ppUintptrFromInt32(0)
	aafs_type = iqlibc.ppUintptrFromInt32(0)
	aafs_label = iqlibc.ppUintptrFromInt32(0)

	aafd = Xext2fs_open_file(cgtls, aadevice, aafl, ppuint16(0666))
	if aafd < 0 && *(*ppint32)(iqunsafe.ppPointer(X__error(cgtls))) == ppint32(mvENOENT) && aaflags&ppint32(mvNO_SIZE) != 0 {
		Xfprintf(cgtls, X__stderrp, "The file %s does not exist and no size was specified.\n\x00", iqlibc.ppVaList(cgbp+160, aadevice))
		Xexit(cgtls, ppint32(1))
	}
	if aafd < 0 && *(*ppint32)(iqunsafe.ppPointer(X__error(cgtls))) == ppint32(mvENOENT) && aaflags&ppint32(mvCREATE_FILE) != 0 {
		aafl = aafl | ppint32(mvO_CREAT)
		aafd = Xext2fs_open_file(cgtls, aadevice, aafl, ppuint16(0666))
		if aafd >= 0 && aaflags&ppint32(mvVERBOSE_CREATE) != 0 {
			Xprintf(cgtls, "Creating regular file %s\n\x00", iqlibc.ppVaList(cgbp+160, aadevice))
		}
	}
	if aafd < 0 {
		Xfprintf(cgtls, X__stderrp, "Could not open %s: %s\n\x00", iqlibc.ppVaList(cgbp+160, aadevice, Xerror_message(cgtls, ppint64(*(*ppint32)(iqunsafe.ppPointer(X__error(cgtls)))))))
		if *(*ppint32)(iqunsafe.ppPointer(X__error(cgtls))) == ppint32(mvENOENT) {
			Xfputs(cgtls, "\nThe device apparently does not exist; did you specify it correctly?\n\x00", X__stderrp)
		}
		Xexit(cgtls, ppint32(1))
	}

	if Xext2fs_fstat(cgtls, aafd, cgbp) < 0 {
		Xperror(cgtls, "stat\x00")
		Xexit(cgtls, ppint32(1))
	}
	Xclose(cgtls, aafd)

	if iqlibc.ppInt32FromUint16((*(*tnext2fs_struct_stat)(iqunsafe.ppPointer(cgbp))).fdst_mode)&ppint32(mvS_IFMT) == ppint32(mvS_IFBLK) {
		aais_dev = ppint32(1)
	}
	if aaret_is_dev != 0 {
		*(*ppint32)(iqunsafe.ppPointer(aaret_is_dev)) = aais_dev
	}

	if aaflags&ppint32(mvCHECK_BLOCK_DEV) != 0 && !(aais_dev != 0) {
		Xprintf(cgtls, "%s is not a block special device.\n\x00", iqlibc.ppVaList(cgbp+160, aadevice))
		return 0
	}
	 ...
	 */
	if aaflags&ppint32(mvCHECK_FS_EXIST) != 0 && Xblkid_get_cache(cgtls, cgbp+144, iqlibc.ppUintptrFromInt32(0)) >= 0 {
		aafs_type = Xblkid_get_tag_value(cgtls, *(*tnblkid_cache)(iqunsafe.ppPointer(cgbp + 144)), "TYPE\x00", aadevice)
		if aafs_type != 0 {
			aafs_label = Xblkid_get_tag_value(cgtls, *(*tnblkid_cache)(iqunsafe.ppPointer(cgbp + 144)), "LABEL\x00", aadevice)
		}
		Xblkid_put_cache(cgtls, *(*tnblkid_cache)(iqunsafe.ppPointer(cgbp + 144)))
	}

	if aafs_type != 0 {
		if aafs_label != 0 {
			Xprintf(cgtls, "%s contains a %s file system labelled '%s'\n\x00", iqlibc.ppVaList(cgbp+160, aadevice, aafs_type, aafs_label))
		} else {
			Xprintf(cgtls, "%s contains a %s file system\n\x00", iqlibc.ppVaList(cgbp+160, aadevice, aafs_type))
		}
		if Xstrncmp(cgtls, aafs_type, "ext\x00", ppuint64(3)) == 0 {
			siprint_ext2_info(cgtls, aadevice)
		}
		Xfree(cgtls, aafs_type)
		Xfree(cgtls, aafs_label)
		return 0
	}

	if aaflags&ppint32(mvCHECK_FS_EXIST) != 0 {
		aaret = sicheck_partition_table(cgtls, aadevice)
		if aaret >= 0 {
			return aaret
		}
	}
...
func _blkid_get_cache(*iqlibc.ppTLS, ppuintptr, ppuintptr) ppint32

func _blkid_get_tag_value(*iqlibc.ppTLS, ppuintptr, ppuintptr, ppuintptr) ppuintptr

func _blkid_put_cache(*iqlibc.ppTLS, ppuintptr)
...
func _ext2fs_open2(*iqlibc.ppTLS, ppuintptr, ppuintptr, ppint32, ppint32, ppuint32, ppuintptr, ppuintptr) ppint64
