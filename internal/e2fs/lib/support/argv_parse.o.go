...
const mvSTATE_QUOTED = 3
const mvSTATE_TOKEN = 2
const mvSTATE_WHITESPACE = 1
...
//	 */
func Xargv_parse(cgtls *iqlibc.ppTLS, aain_buf ppuintptr, aaret_argc ppuintptr, aaret_argv ppuintptr) (cgr ppint32) {

	var aaargc, aamax_argc, aastate, ccv10, ccv12, ccv14, ccv18, ccv20, ccv21, ccv3, ccv7, ccv9 ppint32
	var aaargv, aabuf, aacp, aanew_argv, aaoutcp, ccv13, ccv23, ccv24, ccv25, ccv26 ppuintptr
	var aach, ccv2 ppint8
	var ccv16, ccv5 tn__darwin_ct_rune_t
	var ccv17, ccv6 ppuint64
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aaargc, aaargv, aabuf, aach, aacp, aamax_argc, aanew_argv, aaoutcp, aastate, ccv10, ccv12, ccv13, ccv14, ccv16, ccv17, ccv18, ccv2, ccv20, ccv21, ccv23, ccv24, ccv25, ccv26, ccv3, ccv5, ccv6, ccv7, ccv9
	aaargc = 0
	aamax_argc = 0
	aacp = ppuintptr(0)
	aaoutcp = ppuintptr(0)
	aastate = ppint32(mvSTATE_WHITESPACE)

	aabuf = Xmalloc(cgtls, Xstrlen(cgtls, aain_buf)+ppuint64(1))
	if !(aabuf != 0) {
		return -ppint32(1)
	}
	...
	aacp = aain_buf
	for {
		ccv2 = *(*ppint8)(iqunsafe.ppPointer(aacp))
		aach = ccv2
		if !(ccv2 != 0) {
			break
		}
		if aastate == ppint32(mvSTATE_WHITESPACE) {
			ccv5 = ppint32(aach)
			ccv6 = ppuint64(0x00004000)
		...
		cg_4:
			...
			aastate = ppint32(mvSTATE_TOKEN)
			if aaargc >= aamax_argc {
				aamax_argc = aamax_argc + ppint32(3)
				aanew_argv = Xrealloc(cgtls, aaargv, iqlibc.ppUint64FromInt32(aamax_argc+iqlibc.ppInt32FromInt32(1))*ppuint64(8))
				if !(aanew_argv != 0) {
					Xfree(cgtls, aaargv)
					Xfree(cgtls, aabuf)
					return -ppint32(1)
			...
			ccv3 = aaargc
			aaargc = aaargc + 1
			*(*ppuintptr)(iqunsafe.ppPointer(aaargv + ppuintptr(ccv3)*8)) = aaoutcp
		}
		if aastate == ppint32(mvSTATE_QUOTED) {
			if ppint32(aach) == ppint32('"') {
				aastate = ppint32(mvSTATE_TOKEN)
			} else {
				ccv13 = aaoutcp
				aaoutcp = aaoutcp + 1
				*(*ppint8)(iqunsafe.ppPointer(ccv13)) = aach
			}
			goto cg_1
		}
	...
	cg_15:
		if ccv3 != 0 {
			...
			ccv13 = aaoutcp
			aaoutcp = aaoutcp + 1
			*(*ppint8)(iqunsafe.ppPointer(ccv13)) = 0
			aastate = ppint32(mvSTATE_WHITESPACE)
			goto cg_1
		}
		if ppint32(aach) == ppint32('"') {
			aastate = ppint32(mvSTATE_QUOTED)
			goto cg_1
		}
		if ppint32(aach) == ppint32('\\') {
			aacp = aacp + 1
			ccv13 = aacp
			aach = *(*ppint8)(iqunsafe.ppPointer(ccv13))
			switch ppint32(aach) {
			case ppint32('\000'):
				aach = ppint8('\\')
				aacp = aacp - 1
	...
	if aastate != ppint32(mvSTATE_WHITESPACE) {
		ccv13 = aaoutcp
		aaoutcp = aaoutcp + 1
		*(*ppint8)(iqunsafe.ppPointer(ccv13)) = ppint8('\000')
	}
	if aaargv == ppuintptr(0) {
		aaargv = Xmalloc(cgtls, ppuint64(8))
		Xfree(cgtls, aabuf)
		if !(aaargv != 0) {
			return -ppint32(1)
		}
	}
	*(*ppuintptr)(iqunsafe.ppPointer(aaargv + ppuintptr(aaargc)*8)) = ppuintptr(0)
	if aaret_argc != 0 {
		*(*ppint32)(iqunsafe.ppPointer(aaret_argc)) = aaargc
	}
	if aaret_argv != 0 {
		*(*ppuintptr)(iqunsafe.ppPointer(aaret_argv)) = aaargv
	}
	return 0
}

func Xargv_free(cgtls *iqlibc.ppTLS, aaargv ppuintptr) {

	Xfree(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(aaargv)))
	Xfree(cgtls, aaargv)
}
