...
func Xuuid_pack(cgtls *iqlibc.ppTLS, aauu ppuintptr, aaptr ppuintptr) {

	var aaout ppuintptr
	var aatmp tnuint32_t
	pp_, pp_ = aaout, aatmp
	aaout = aaptr

	aatmp = (*tsuuid)(iqunsafe.ppPointer(aauu)).fdtime_low
	*(*ppuint8)(iqunsafe.ppPointer(aaout + 3)) = ppuint8(aatmp)
	aatmp = aatmp >> ppuint32(8)
	*(*ppuint8)(iqunsafe.ppPointer(aaout + 2)) = ppuint8(aatmp)
	aatmp = aatmp >> ppuint32(8)
	*(*ppuint8)(iqunsafe.ppPointer(aaout + 1)) = ppuint8(aatmp)
	aatmp = aatmp >> ppuint32(8)
	*(*ppuint8)(iqunsafe.ppPointer(aaout)) = ppuint8(aatmp)

	aatmp = ppuint32((*tsuuid)(iqunsafe.ppPointer(aauu)).fdtime_mid)
	*(*ppuint8)(iqunsafe.ppPointer(aaout + 5)) = ppuint8(aatmp)
	aatmp = aatmp >> ppuint32(8)
	*(*ppuint8)(iqunsafe.ppPointer(aaout + 4)) = ppuint8(aatmp)

	aatmp = ppuint32((*tsuuid)(iqunsafe.ppPointer(aauu)).fdtime_hi_and_version)
	*(*ppuint8)(iqunsafe.ppPointer(aaout + 7)) = ppuint8(aatmp)
	aatmp = aatmp >> ppuint32(8)
	*(*ppuint8)(iqunsafe.ppPointer(aaout + 6)) = ppuint8(aatmp)

	aatmp = ppuint32((*tsuuid)(iqunsafe.ppPointer(aauu)).fdclock_seq)
	*(*ppuint8)(iqunsafe.ppPointer(aaout + 9)) = ppuint8(aatmp)
	aatmp = aatmp >> ppuint32(8)
	*(*ppuint8)(iqunsafe.ppPointer(aaout + 8)) = ppuint8(aatmp)

	Xmemcpy(cgtls, aaout+ppuintptr(10), aauu+10, ppuint64(6))
}
