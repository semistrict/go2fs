...
func Xinit_error_table(cgtls *iqlibc.ppTLS, aamsgs ppuintptr, aabase ppint64, aacount ppint32) (cgr ppint32) {

	var aanew_et ppuintptr
	pp_ = aanew_et

	if !(aabase != 0) || !(aacount != 0) || !(aamsgs != 0) {
		return 0
	}

	aanew_et = Xmalloc(cgtls, ppuint64(40))
	if !(aanew_et != 0) {
		return ppint32(mvENOMEM)
	} /* oops */
	(*tsfoobar)(iqunsafe.ppPointer(aanew_et)).fdetl.fdtable = aanew_et + 16
	(*tsfoobar)(iqunsafe.ppPointer(aanew_et)).fdet.fdmsgs = aamsgs
	(*tsfoobar)(iqunsafe.ppPointer(aanew_et)).fdet.fdbase = aabase
	(*tsfoobar)(iqunsafe.ppPointer(aanew_et)).fdet.fdn_msgs = aacount

	(*tsfoobar)(iqunsafe.ppPointer(aanew_et)).fdetl.fdnext = X_et_dynamic_list
	X_et_dynamic_list = aanew_et
	return 0
}

var __et_dynamic_list ppuintptr
