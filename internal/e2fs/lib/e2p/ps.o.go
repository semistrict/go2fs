...
func Xprint_fs_state(cgtls *iqlibc.ppTLS, aaf ppuintptr, aastate ppuint16) {

	if iqlibc.ppInt32FromUint16(aastate)&ppint32(mvEXT2_VALID_FS) != 0 {
		Xfprintf(cgtls, aaf, " clean\x00", 0)
	} else {
		Xfprintf(cgtls, aaf, " not clean\x00", 0)
	}
	if iqlibc.ppInt32FromUint16(aastate)&ppint32(mvEXT2_ERROR_FS) != 0 {
		Xfprintf(cgtls, aaf, " with errors\x00", 0)
	}
}
