...
func Xparse_num_blocks2(cgtls *iqlibc.ppTLS, aaarg ppuintptr, aalog_block_size ppint32) (cgr ppuint64) {
	...
	aanum = Xstrtoull(cgtls, aaarg, cgbp, 0)

	if *(*ppint8)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)))) != 0 && *(*ppint8)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) + 1)) != 0 {
		return ppuint64(0)
	}

	switch ppint32(*(*ppint8)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp))))) { /* Using fall-through logic */
	case ppint32('T'):
		fallthrough
	case ppint32('t'):
		aanum = aanum << ppuint64(10)
		/* fallthrough */
		fallthrough
	case ppint32('G'):
		fallthrough
	...
	case ppint32('K'):
		fallthrough
	case ppint32('k'):
		if aalog_block_size < 0 {
			aanum = aanum << ppuint64(10)
		} else {
			aanum = aanum >> iqlibc.ppUint64FromInt32(aalog_block_size)
		}
		break
		fallthrough
	case ppint32('s'):
		if aalog_block_size < 0 {
			aanum = aanum << ppuint64(9)
		} else {
			aanum = aanum >> iqlibc.ppUint64FromInt32(iqlibc.ppInt32FromInt32(1)+aalog_block_size)
		}
		break
		fallthrough
	case ppint32('\000'):
...
func Xparse_num_blocks(cgtls *iqlibc.ppTLS, aaarg ppuintptr, aalog_block_size ppint32) (cgr ppuint64) {

	return ppuint64(Xparse_num_blocks2(cgtls, aaarg, aalog_block_size))
}
