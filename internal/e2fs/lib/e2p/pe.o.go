...
func Xprint_fs_errors(cgtls *iqlibc.ppTLS, aaf ppuintptr, aaerrors ppuint16) {

	switch iqlibc.ppInt32FromUint16(aaerrors) {

	case ppint32(mvEXT2_ERRORS_CONTINUE):
		Xfprintf(cgtls, aaf, "Continue\x00", 0)
		break
		fallthrough
	case ppint32(mvEXT2_ERRORS_RO):
		Xfprintf(cgtls, aaf, "Remount read-only\x00", 0)
		break
		fallthrough
	case ppint32(mvEXT2_ERRORS_PANIC):
		Xfprintf(cgtls, aaf, "Panic\x00", 0)
		break
		fallthrough
	default:
		Xfprintf(cgtls, aaf, "Unknown (continue)\x00", 0)
	}
}
