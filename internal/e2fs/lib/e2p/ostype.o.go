...
//	 */
func Xe2p_os2string(cgtls *iqlibc.ppTLS, aaos_type ppint32) (cgr ppuintptr) {

	var aaos, aaret ppuintptr
	pp_, pp_ = aaos, aaret

	if aaos_type >= 0 && aaos_type <= ppint32(mvEXT2_OS_LITES) {
		aaos = sios_tab[aaos_type]
	} else {
		aaos = "(unknown os)\x00"
	}

	aaret = Xmalloc(cgtls, Xstrlen(cgtls, aaos)+ppuint64(1))
	if aaret != 0 {
		Xstrcpy(cgtls, aaret, aaos)
	}
	return aaret
}
...
//	 * Convert an os_type to a string
//	 */
func Xe2p_string2os(cgtls *iqlibc.ppTLS, aastr ppuintptr) (cgr ppint32) {
	...
	aacpp = ppuintptr(iqunsafe.ppPointer(&sios_tab))
	for {
		if !(*(*ppuintptr)(iqunsafe.ppPointer(aacpp)) != 0) {
			break
		}
		if !(Xstrcasecmp(cgtls, aastr, *(*ppuintptr)(iqunsafe.ppPointer(aacpp))) != 0) {
			return aai
		}
