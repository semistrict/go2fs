...
var siext4_encoding_map = [2]struct {
	fdname           ppuintptr
	fdencoding_magic tn__u16
	fddefault_flags  tn__u16
}{
	0: {
		fdname:           "utf8-12.1\x00",
		fdencoding_magic: ppuint16(mvEXT4_ENC_UTF8_12_1),
	},
	1: {
		fdname:           "utf8\x00",
		fdencoding_magic: ppuint16(mvEXT4_ENC_UTF8_12_1),
...
type tsenc_flags = struct {
	fdflag  tn__u16
	fdparam ppuintptr
}

var siencoding_flags = [1]tsenc_flags{
	0: {
		fdflag:  iqlibc.ppUint16FromInt32(iqlibc.ppInt32FromInt32(1) << iqlibc.ppInt32FromInt32(0)),
		fdparam: "strict\x00",
	},
}
...
func Xe2p_str2encoding(cgtls *iqlibc.ppTLS, aastring ppuintptr) (cgr ppint32) {
	...
	aai = ppuint32(0)
	for {
		if !(ppuint64(aai) < iqlibc.ppUint64FromInt64(32)/iqlibc.ppUint64FromInt64(16)) {
			break
		}
		if !(Xstrcmp(cgtls, aastring, siext4_encoding_map[aai].fdname) != 0) {
			return iqlibc.ppInt32FromUint16(siext4_encoding_map[aai].fdencoding_magic)
		}
		goto cg_1
	cg_1:
		;
...
func Xe2p_encoding2str(cgtls *iqlibc.ppTLS, aaencoding ppint32) (cgr ppuintptr) {
	...
	for {
		...
		if iqlibc.ppInt32FromUint16(siext4_encoding_map[aai].fdencoding_magic) == aaencoding {
			return siext4_encoding_map[aai].fdname
		}
		goto cg_1
	cg_1:
		;
	...
	Xsprintf(cgtls, ppuintptr(iqunsafe.ppPointer(&snbuf)), "UNKNOWN_ENCODING_%d\x00", iqlibc.ppVaList(cgbp+8, aaencoding))
	return ppuintptr(iqunsafe.ppPointer(&snbuf))
}

func Xe2p_get_encoding_flags(cgtls *iqlibc.ppTLS, aaencoding ppint32) (cgr ppint32) {
	...
	for {
		...
		if iqlibc.ppInt32FromUint16(siext4_encoding_map[aai].fdencoding_magic) == aaencoding {
			return iqlibc.ppInt32FromUint16(siext4_encoding_map[aai].fddefault_flags)
		}
		goto cg_1
	cg_1:
		;
...
func Xe2p_str2encoding_flags(cgtls *iqlibc.ppTLS, aaencoding ppint32, aaparam ppuintptr, aaflags ppuintptr) (cgr ppint32) {

	var aaf, aafl, ccv2, ccv3 ppuintptr
	var aai, aaneg ppuint32
	pp_, pp_, pp_, pp_, pp_, pp_ = aaf, aafl, aai, aaneg, ccv2, ccv3
	aaf = Xstrtok(cgtls, aaparam, "-\x00")
	aaneg = ppuint32(0)

	if aaencoding != ppint32(mvEXT4_ENC_UTF8_12_1) {
		return -ppint32(mvEINVAL)
	}
	for aaf != 0 {
		aaneg = ppuint32(0)
		if !(Xstrncmp(cgtls, "no\x00", aaf, ppuint64(2)) != 0) {
			aaneg = ppuint32(1)
			aaf = aaf + ppuintptr(2)
		}
		...
		aai = ppuint32(0)
		for {
			if !(ppuint64(aai) < iqlibc.ppUint64FromInt64(16)/iqlibc.ppUint64FromInt64(16)) {
				break
			}
			aafl = ppuintptr(iqunsafe.ppPointer(&siencoding_flags)) + ppuintptr(aai)*16
			if !(Xstrcmp(cgtls, (*tsenc_flags)(iqunsafe.ppPointer(aafl)).fdparam, aaf) != 0) {
				if aaneg != 0 {

					ccv2 = aaflags
					*(*tn__u16)(iqunsafe.ppPointer(ccv2)) = tn__u16(ppint32(*(*tn__u16)(iqunsafe.ppPointer(ccv2))) & ^iqlibc.ppInt32FromUint16((*tsenc_flags)(iqunsafe.ppPointer(aafl)).fdflag))
				} else {
	...
	ppnext_flag:
		;
		aaf = Xstrtok(cgtls, iqlibc.ppUintptrFromInt32(0), "-\x00")
	}
	return 0
}
...
func _strtok(*iqlibc.ppTLS, ppuintptr, ppuintptr) ppuintptr
