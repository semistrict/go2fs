...
func Xgetflags(cgtls *iqlibc.ppTLS, aafd ppint32, aaflags ppuintptr) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(144)
	defer cgtls.ppFree(144)

	var pp_ /* buf at bp+0 */ tsstat

	if Xfstat(cgtls, aafd, cgbp) == -ppint32(1) {
		return -ppint32(1)
	}

	*(*ppuint64)(iqunsafe.ppPointer(aaflags)) = ppuint64(0)
	if (*(*tsstat)(iqunsafe.ppPointer(cgbp))).fdst_flags&ppuint32(mvUF_IMMUTABLE) != 0 {

		*(*ppuint64)(iqunsafe.ppPointer(aaflags)) |= ppuint64(mvEXT2_IMMUTABLE_FL)
	}
	if (*(*tsstat)(iqunsafe.ppPointer(cgbp))).fdst_flags&ppuint32(mvUF_APPEND) != 0 {

		*(*ppuint64)(iqunsafe.ppPointer(aaflags)) |= ppuint64(mvEXT2_APPEND_FL)
	}
	if (*(*tsstat)(iqunsafe.ppPointer(cgbp))).fdst_flags&ppuint32(mvUF_NODUMP) != 0 {

		*(*ppuint64)(iqunsafe.ppPointer(aaflags)) |= ppuint64(mvEXT2_NODUMP_FL)
	}
