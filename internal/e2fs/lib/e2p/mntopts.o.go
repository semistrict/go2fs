...
const mvEXT2_DEFM_ACL = 8
const mvEXT2_DEFM_BSDGROUPS = 2
const mvEXT2_DEFM_DEBUG = 1
const mvEXT2_DEFM_UID16 = 16
const mvEXT2_DEFM_XATTR_USER = 4
...
const mvEXT3_DEFM_JMODE_DATA = 32
const mvEXT3_DEFM_JMODE_ORDERED = 64
const mvEXT3_DEFM_JMODE_WBACK = 96
...
const mvEXT4_DEFM_BLOCK_VALIDITY = 512
const mvEXT4_DEFM_DISCARD = 1024
const mvEXT4_DEFM_NOBARRIER = 256
const mvEXT4_DEFM_NODELALLOC = 2048
...
type tsmntopt = struct {
	fdmask    ppuint32
	fdstring1 ppuintptr
}

var simntopt_list = [13]tsmntopt{
	0: {
		fdmask:    ppuint32(mvEXT2_DEFM_DEBUG),
		fdstring1: "debug\x00",
	},
	1: {
		fdmask:    ppuint32(mvEXT2_DEFM_BSDGROUPS),
		fdstring1: "bsdgroups\x00",
	},
	2: {
		fdmask:    ppuint32(mvEXT2_DEFM_XATTR_USER),
		fdstring1: "user_xattr\x00",
	},
	3: {
		fdmask:    ppuint32(mvEXT2_DEFM_ACL),
		fdstring1: "acl\x00",
	},
	4: {
		fdmask:    ppuint32(mvEXT2_DEFM_UID16),
		fdstring1: "uid16\x00",
	},
	5: {
		fdmask:    ppuint32(mvEXT3_DEFM_JMODE_DATA),
		fdstring1: "journal_data\x00",
	},
	6: {
		fdmask:    ppuint32(mvEXT3_DEFM_JMODE_ORDERED),
		fdstring1: "journal_data_ordered\x00",
	},
	7: {
		fdmask:    ppuint32(mvEXT3_DEFM_JMODE_WBACK),
		fdstring1: "journal_data_writeback\x00",
	},
	8: {
		fdmask:    ppuint32(mvEXT4_DEFM_NOBARRIER),
		fdstring1: "nobarrier\x00",
	},
	9: {
		fdmask:    ppuint32(mvEXT4_DEFM_BLOCK_VALIDITY),
		fdstring1: "block_validity\x00",
	},
	10: {
		fdmask:    ppuint32(mvEXT4_DEFM_DISCARD),
		fdstring1: "discard\x00",
	},
	11: {
		fdmask:    ppuint32(mvEXT4_DEFM_NODELALLOC),
		fdstring1: "nodelalloc\x00",
	},
	12: {},
}

func Xe2p_mntopt2string(cgtls *iqlibc.ppTLS, aamask ppuint32) (cgr ppuintptr) {
	...
	aaf = ppuintptr(iqunsafe.ppPointer(&simntopt_list))
	for {
		if !((*tsmntopt)(iqunsafe.ppPointer(aaf)).fdstring1 != 0) {
			break
		}
		if aamask == (*tsmntopt)(iqunsafe.ppPointer(aaf)).fdmask {
			return (*tsmntopt)(iqunsafe.ppPointer(aaf)).fdstring1
		}
	...
	Xsprintf(cgtls, ppuintptr(iqunsafe.ppPointer(&snbuf)), "MNTOPT_%d\x00", iqlibc.ppVaList(cgbp+8, aafnum))
	return ppuintptr(iqunsafe.ppPointer(&snbuf))
}

func Xe2p_string2mntopt(cgtls *iqlibc.ppTLS, aastring ppuintptr, aamask ppuintptr) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)
	...
	for {
		...
		if !(Xstrcasecmp(cgtls, aastring, (*tsmntopt)(iqunsafe.ppPointer(aaf)).fdstring1) != 0) {
			*(*ppuint32)(iqunsafe.ppPointer(aamask)) = (*tsmntopt)(iqunsafe.ppPointer(aaf)).fdmask
			return 0
		}
	...
	if Xstrncasecmp(cgtls, aastring, "MNTOPT_\x00", ppuint64(7)) != 0 {
		return ppint32(1)
	}

	if ppint32(*(*ppint8)(iqunsafe.ppPointer(aastring + 8))) == 0 {
		return ppint32(1)
	}
	aanum = ppint32(Xstrtol(cgtls, aastring+ppuintptr(8), cgbp, ppint32(10)))
...
//	 */
func Xe2p_edit_mntopts(cgtls *iqlibc.ppTLS, aastr ppuintptr, aamntopts ppuintptr, aaok tn__u32) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aabuf, aacp, aanext, ccv1 ppuintptr
	var aaneg, aarc ppint32
	var pp_ /* mask at bp+0 */ ppuint32
	pp_, pp_, pp_, pp_, pp_, pp_ = aabuf, aacp, aaneg, aanext, aarc, ccv1
	aarc = 0
	...
	aacp = aabuf
	for aacp != 0 && *(*ppint8)(iqunsafe.ppPointer(aacp)) != 0 {
		aaneg = 0
		...
		if Xe2p_string2mntopt(cgtls, aacp, cgbp) != 0 {
			aarc = ppint32(1)
			break
		}
		if aaok != 0 && !(aaok&*(*ppuint32)(iqunsafe.ppPointer(cgbp)) != 0) {
			aarc = ppint32(1)
			break
		}
		if *(*ppuint32)(iqunsafe.ppPointer(cgbp))&ppuint32(mvEXT3_DEFM_JMODE) != 0 {

			*(*tn__u32)(iqunsafe.ppPointer(aamntopts)) &= iqlibc.ppUint32FromInt32(^iqlibc.ppInt32FromInt32(mvEXT3_DEFM_JMODE))
		}
		if aaneg != 0 {

			*(*tn__u32)(iqunsafe.ppPointer(aamntopts)) &= ^*(*ppuint32)(iqunsafe.ppPointer(cgbp))
		} else {

			*(*tn__u32)(iqunsafe.ppPointer(aamntopts)) |= *(*ppuint32)(iqunsafe.ppPointer(cgbp))
		}
		if aanext != 0 {
			ccv1 = aanext + ppuintptr(1)
		} else {
			ccv1 = ppuintptr(0)
		}
		aacp = ccv1
