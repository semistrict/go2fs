...
var sihash_list = [4]tshash{
	0: {
		fdstring1: "legacy\x00",
	},
	1: {
		fdnum:     ppint32(mvEXT2_HASH_HALF_MD4),
		fdstring1: "half_md4\x00",
	},
	2: {
		fdnum:     ppint32(mvEXT2_HASH_TEA),
		fdstring1: "tea\x00",
	},
	3: {},
}

func Xe2p_hash2string(cgtls *iqlibc.ppTLS, aanum ppint32) (cgr ppuintptr) {
	...
	aap = ppuintptr(iqunsafe.ppPointer(&sihash_list))
	for {
		if !((*tshash)(iqunsafe.ppPointer(aap)).fdstring1 != 0) {
			break
		}
		if aanum == (*tshash)(iqunsafe.ppPointer(aap)).fdnum {
			return (*tshash)(iqunsafe.ppPointer(aap)).fdstring1
		}
	...
	Xsprintf(cgtls, ppuintptr(iqunsafe.ppPointer(&snbuf)), "HASHALG_%d\x00", iqlibc.ppVaList(cgbp+8, aanum))
...
//	 */
func Xe2p_string2hash(cgtls *iqlibc.ppTLS, aastring ppuintptr) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)
	...
	for {
		...
		if !(Xstrcasecmp(cgtls, aastring, (*tshash)(iqunsafe.ppPointer(aap)).fdstring1) != 0) {
			return (*tshash)(iqunsafe.ppPointer(aap)).fdnum
		}
	...
	if Xstrncasecmp(cgtls, aastring, "HASHALG_\x00", ppuint64(8)) != 0 {
		return -ppint32(1)
	}
