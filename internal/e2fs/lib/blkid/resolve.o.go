...
//	 */
func Xblkid_get_tag_value(cgtls *iqlibc.ppTLS, aacache tnblkid_cache, aatagname ppuintptr, aadevname ppuintptr) (cgr ppuintptr) {
	cgbp := cgtls.ppAlloc(16)
	defer cgtls.ppFree(16)

	var aadev, ccv1 tnblkid_dev
	var aafound, ccv2 tnblkid_tag
	var aaret ppuintptr
	var ccv3 ppbool
	var pp_ /* c at bp+0 */ tnblkid_cache
	pp_, pp_, pp_, pp_, pp_, pp_ = aadev, aafound, aaret, ccv1, ccv2, ccv3
	*(*tnblkid_cache)(iqunsafe.ppPointer(cgbp)) = aacache
	aaret = iqlibc.ppUintptrFromInt32(0)
	...
	if !(aacache != 0) {
		if Xblkid_get_cache(cgtls, cgbp, iqlibc.ppUintptrFromInt32(0)) < 0 {
			return iqlibc.ppUintptrFromInt32(0)
		}
	}

	ccv1 = Xblkid_get_dev(cgtls, *(*tnblkid_cache)(iqunsafe.ppPointer(cgbp)), aadevname, iqlibc.ppInt32FromInt32(mvBLKID_DEV_CREATE)|iqlibc.ppInt32FromInt32(mvBLKID_DEV_VERIFY))
	aadev = ccv1
	if ccv3 = ccv1 != 0; ccv3 {
		ccv2 = Xblkid_find_tag_dev(cgtls, aadev, aatagname)
		aafound = ccv2
	}
	if ccv3 && ccv2 != 0 {
		aaret = Xblkid_strdup(cgtls, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aafound)).fdbit_val)
	}

	if !(aacache != 0) {
		Xblkid_put_cache(cgtls, *(*tnblkid_cache)(iqunsafe.ppPointer(cgbp)))
	}
...
//	 */
func Xblkid_get_devname(cgtls *iqlibc.ppTLS, aacache tnblkid_cache, aatoken ppuintptr, aavalue ppuintptr) (cgr ppuintptr) {
	...
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)) = ppuintptr(0)
	...
	if !(aavalue != 0) {
		if !(Xstrchr(cgtls, aatoken, ppint32('=')) != 0) {
			aaret = Xblkid_strdup(cgtls, aatoken)
			goto ppout
		}
		Xblkid_parse_tag_string(cgtls, aatoken, cgbp+8, cgbp+16)
		if !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 8)) != 0) || !(*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)) != 0) {
			goto ppout
		}
		aatoken = *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 8))
		aavalue = *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16))
	}

	aadev = Xblkid_find_dev_with_tag(cgtls, *(*tnblkid_cache)(iqunsafe.ppPointer(cgbp)), aatoken, aavalue)
	if !(aadev != 0) {
		goto ppout
	}

	aaret = Xblkid_strdup(cgtls, Xblkid_dev_devname(cgtls, aadev))
...
func _blkid_dev_devname(*iqlibc.ppTLS, ppuintptr) ppuintptr

func _blkid_find_dev_with_tag(*iqlibc.ppTLS, ppuintptr, ppuintptr, ppuintptr) ppuintptr

func _blkid_find_tag_dev(*iqlibc.ppTLS, ppuintptr, ppuintptr) ppuintptr
...
func _blkid_parse_tag_string(*iqlibc.ppTLS, ppuintptr, ppuintptr, ppuintptr) ppint32
