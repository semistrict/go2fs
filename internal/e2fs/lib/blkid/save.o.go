...
func sisave_dev(cgtls *iqlibc.ppTLS, aadev tnblkid_dev, aafile ppuintptr) (cgr ppint32) {
	...
	if !(aadev != 0) || ppint32(*(*ppint8)(iqunsafe.ppPointer((*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_name))) != ppint32('/') {
		return 0
	}

	Xfprintf(cgtls, aafile, "<device DEVNO=\"0x%04lx\" TIME=\"%ld\"\x00", iqlibc.ppVaList(cgbp+8, iqlibc.ppUint64FromInt32((*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_devno), (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_time))
	if (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_pri != 0 {
		Xfprintf(cgtls, aafile, " PRI=\"%d\"\x00", iqlibc.ppVaList(cgbp+8, (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_pri))
	}
	...
	for {
		...
		aatag = aap - ppuintptr(ppuint64(iqlibc.ppUintptrFromInt32(0)))
		Xfprintf(cgtls, aafile, " %s=\"%s\"\x00", iqlibc.ppVaList(cgbp+8, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatag)).fdbit_name, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatag)).fdbit_val))
	...
	Xfprintf(cgtls, aafile, ">%s</device>\n\x00", iqlibc.ppVaList(cgbp+8, (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_name))
...
//	 */
func Xblkid_flush_cache(cgtls *iqlibc.ppTLS, aacache tnblkid_cache) (cgr ppint32) {
	cgbp := cgtls.ppAlloc(160)
	defer cgtls.ppFree(160)

	var aabackup, aafile, aafilename, aaopened, aap, aatmp, ccv1, ccv4 ppuintptr
	var aadev tnblkid_dev
	var aafd, aaret, ccv2, ccv5, ccv7 ppint32
	var aasave_umask tnmode_t
	var pp_ /* st at bp+0 */ tsstat
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aabackup, aadev, aafd, aafile, aafilename, aaopened, aap, aaret, aasave_umask, aatmp, ccv1, ccv2, ccv4, ccv5, ccv7
	aatmp = iqlibc.ppUintptrFromInt32(0)
	aaopened = iqlibc.ppUintptrFromInt32(0)
	aafile = iqlibc.ppUintptrFromInt32(0)
	aaret = 0
	...
	ccv1 = aacache
	ccv2 = iqlibc.ppBoolInt32((*tslist_head)(iqunsafe.ppPointer(ccv1)).fdnext == ccv1)
	goto cg_3
cg_3:
	;
	if ccv2 != 0 || !((*tsblkid_struct_cache)(iqunsafe.ppPointer(aacache)).fdbic_flags&iqlibc.ppUint32FromInt32(mvBLKID_BIC_FL_CHANGED) != 0) {

		return 0
	}

	if (*tsblkid_struct_cache)(iqunsafe.ppPointer(aacache)).fdbic_filename != 0 {
		ccv1 = (*tsblkid_struct_cache)(iqunsafe.ppPointer(aacache)).fdbic_filename
	} else {
		ccv1 = "/etc/blkid.tab\x00"
	}
	aafilename = ccv1

	/* If we can't write to the cache file, then don't even try */
	ccv2 = Xstat(cgtls, aafilename, cgbp)
	aaret = ccv2
	if ccv2 < 0 && *(*ppint32)(iqunsafe.ppPointer(X__error(cgtls))) != ppint32(mvENOENT) || aaret == 0 && Xaccess(cgtls, aafilename, iqlibc.ppInt32FromInt32(1)<<iqlibc.ppInt32FromInt32(1)) < 0 {
	 ...
	 */
	if aaret == 0 && iqlibc.ppInt32FromUint16((*(*tsstat)(iqunsafe.ppPointer(cgbp))).fdst_mode)&ppint32(mvS_IFMT) == ppint32(mvS_IFREG) {
		aatmp = Xmalloc(cgtls, Xstrlen(cgtls, aafilename)+ppuint64(8))
		if aatmp != 0 {
			aasave_umask = Xumask(cgtls, ppuint16(022))
			Xsprintf(cgtls, aatmp, "%s-XXXXXX\x00", iqlibc.ppVaList(cgbp+152, aafilename))
			aafd = Xmkstemp(cgtls, aatmp)
			Xumask(cgtls, aasave_umask)
			if aafd >= 0 {
				aafile = Xfdopen(cgtls, aafd, "w\x00")
				aaopened = aatmp
			}
			Xfchmod(cgtls, aafd, ppuint16(0644))
		}
	}

	if !(aafile != 0) {
		aafile = Xfopen(cgtls, aafilename, "w\x00")
		aaopened = aafilename
	}

	if !(aafile != 0) {
		aaret = *(*ppint32)(iqunsafe.ppPointer(X__error(cgtls)))
		goto pperrout
	}
	...
	for {
		...
		aadev = aap - ppuintptr(ppuint64(iqlibc.ppUintptrFromInt32(0)))
		if !((*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_type != 0) {
			goto cg_6
		}
		ccv2 = sisave_dev(cgtls, aadev, aafile)
		aaret = ccv2
		if ccv2 < 0 {
			break
		}
	...
	if aaopened != aafilename {
		if aaret < 0 {
			Xunlink(cgtls, aaopened)

		} else {

			aabackup = Xmalloc(cgtls, Xstrlen(cgtls, aafilename)+ppuint64(5))
			if aabackup != 0 {
				Xsprintf(cgtls, aabackup, "%s.old\x00", iqlibc.ppVaList(cgbp+152, aafilename))
				Xunlink(cgtls, aabackup)
				Xlink(cgtls, aafilename, aabackup)
				Xfree(cgtls, aabackup)
			}
			if Xrename(cgtls, aaopened, aafilename) < 0 {
				Xunlink(cgtls, aaopened)
			}
...
func _fchmod(*iqlibc.ppTLS, ppint32, ppuint16) ppint32
...
func _link(*iqlibc.ppTLS, ppuintptr, ppuintptr) ppint32

func _malloc(*iqlibc.ppTLS, ppuint64) ppuintptr

func _mkstemp(*iqlibc.ppTLS, ppuintptr) ppint32

func _rename(*iqlibc.ppTLS, ppuintptr, ppuintptr) ppint32
