...
const mvTAG_ITERATE_MAGIC = 27600972
...
type tsblkid_struct_tag_iterate = struct {
	fdmagic ppint32
	fddev   tnblkid_dev
	fdp     ppuintptr
}
...
func siblkid_new_tag(cgtls *iqlibc.ppTLS) (cgr tnblkid_tag) {

	var aatag, ccv1 tnblkid_tag
	pp_, pp_ = aatag, ccv1

	ccv1 = Xcalloc(cgtls, ppuint64(1), ppuint64(56))
	aatag = ccv1
	if !(ccv1 != 0) {
		return iqlibc.ppUintptrFromInt32(0)
	}

	(*tslist_head)(iqunsafe.ppPointer(aatag)).fdnext = aatag
	(*tslist_head)(iqunsafe.ppPointer(aatag)).fdprev = aatag
	(*tslist_head)(iqunsafe.ppPointer(aatag + 16)).fdnext = aatag + 16
	(*tslist_head)(iqunsafe.ppPointer(aatag + 16)).fdprev = aatag + 16

	return aatag
}

func Xblkid_free_tag(cgtls *iqlibc.ppTLS, aatag tnblkid_tag) {

	var ccv1, ccv2, ccv3, ccv4, ccv5, ccv6 ppuintptr
	pp_, pp_, pp_, pp_, pp_, pp_ = ccv1, ccv2, ccv3, ccv4, ccv5, ccv6
	if !(aatag != 0) {
		return
	}
	...
	(*tslist_head)(iqunsafe.ppPointer(ccv2)).fdnext = ccv3 /* list of tags for this device */
	ccv1 = aatag + 16
	ccv2 = (*tslist_head)(iqunsafe.ppPointer(ccv1)).fdprev
	ccv3 = (*tslist_head)(iqunsafe.ppPointer(ccv1)).fdnext
	(*tslist_head)(iqunsafe.ppPointer(ccv3)).fdprev = ccv2
	(*tslist_head)(iqunsafe.ppPointer(ccv2)).fdnext = ccv3 /* list of tags with this type */

	Xfree(cgtls, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatag)).fdbit_name)
	Xfree(cgtls, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatag)).fdbit_val)
...
//	 */
func Xblkid_find_tag_dev(cgtls *iqlibc.ppTLS, aadev tnblkid_dev, aatype ppuintptr) (cgr tnblkid_tag) {

	var aap ppuintptr
	var aatmp tnblkid_tag
	pp_, pp_ = aap, aatmp
	...
	aap = (*tslist_head)(iqunsafe.ppPointer(aadev + 16)).fdnext
	for {
		if !(aap != aadev+16) {
			break
		}
		...
		if !(Xstrcmp(cgtls, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatmp)).fdbit_name, aatype) != 0) {
			return aatmp
		}
...
func Xblkid_dev_has_tag(cgtls *iqlibc.ppTLS, aadev tnblkid_dev, aatype ppuintptr, aavalue ppuintptr) (cgr ppint32) {
	...
	aatag = Xblkid_find_tag_dev(cgtls, aadev, aatype)
	if !(aavalue != 0) {
		return iqlibc.ppBoolInt32(aatag != iqlibc.ppUintptrFromInt32(0))
	}
	if !(aatag != 0) || Xstrcmp(cgtls, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatag)).fdbit_val, aavalue) != 0 {
		return 0
	}
	return ppint32(1)
}
...
//	 */
func siblkid_find_head_cache(cgtls *iqlibc.ppTLS, aacache tnblkid_cache, aatype ppuintptr) (cgr tnblkid_tag) {

	var aahead, aatmp tnblkid_tag
	var aap ppuintptr
	pp_, pp_, pp_ = aahead, aap, aatmp
	aahead = iqlibc.ppUintptrFromInt32(0)

	if !(aacache != 0) || !(aatype != 0) {
		return iqlibc.ppUintptrFromInt32(0)
	}

	aap = (*tslist_head)(iqunsafe.ppPointer(aacache + 16)).fdnext
	for {
		if !(aap != aacache+16) {
			break
		}
...
//	 */
func Xblkid_set_tag(cgtls *iqlibc.ppTLS, aadev tnblkid_dev, aaname ppuintptr, aavalue ppuintptr, aavlength ppint32) (cgr ppint32) {

	var aadev_var, aaval, ccv1, ccv10, ccv11, ccv12, ccv13, ccv14, ccv3, ccv4, ccv5, ccv6, ccv7, ccv8, ccv9 ppuintptr
	var aahead1, aat, ccv2 tnblkid_tag
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aadev_var, aahead1, aat, aaval, ccv1, ccv10, ccv11, ccv12, ccv13, ccv14, ccv2, ccv3, ccv4, ccv5, ccv6, ccv7, ccv8, ccv9
	aat = ppuintptr(0)
	aahead1 = ppuintptr(0)
	aaval = ppuintptr(0)
	aadev_var = ppuintptr(0)

	if !(aadev != 0) || !(aaname != 0) {
		return -ppint32(mvBLKID_ERR_PARAM)
	}

	ccv1 = Xblkid_strndup(cgtls, aavalue, aavlength)
	aaval = ccv1
	if !(ccv1 != 0) && aavalue != 0 {
	 ...
	 */
	if !(Xstrcmp(cgtls, aaname, "TYPE\x00") != 0) {
		aadev_var = aadev + 48
	} else {
		if !(Xstrcmp(cgtls, aaname, "LABEL\x00") != 0) {
			aadev_var = aadev + 80
		} else {
			if !(Xstrcmp(cgtls, aaname, "UUID\x00") != 0) {
				aadev_var = aadev + 88
			}
		}
	}

	aat = Xblkid_find_tag_dev(cgtls, aadev, aaname)
	if !(aavalue != 0) {
		if aat != 0 {
			Xblkid_free_tag(cgtls, aat)
		}
	} else {
		if aat != 0 {
			if !(Xstrcmp(cgtls, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aat)).fdbit_val, aaval) != 0) {
				/* Same thing, exit */
				Xfree(cgtls, aaval)
				return 0
			}
			Xfree(cgtls, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aat)).fdbit_val)
			(*tsblkid_struct_tag)(iqunsafe.ppPointer(aat)).fdbit_val = aaval
		} else {
			/* Existing tag not present, add to device */
			ccv2 = siblkid_new_tag(cgtls)
			aat = ccv2
			if !(ccv2 != 0) {
				goto pperrout
			}
			(*tsblkid_struct_tag)(iqunsafe.ppPointer(aat)).fdbit_name = Xblkid_strdup(cgtls, aaname)
			(*tsblkid_struct_tag)(iqunsafe.ppPointer(aat)).fdbit_val = aaval
			(*tsblkid_struct_tag)(iqunsafe.ppPointer(aat)).fdbit_dev = aadev

			ccv1 = aadev + 16
			ccv3 = aat
			ccv4 = (*tslist_head)(iqunsafe.ppPointer(ccv1)).fdprev
			ccv5 = ccv1
			...
			if (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_cache != 0 {
				aahead1 = siblkid_find_head_cache(cgtls, (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_cache, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aat)).fdbit_name)
				if !(aahead1 != 0) {
					aahead1 = siblkid_new_tag(cgtls)
					if !(aahead1 != 0) {
						goto pperrout
					}

					(*tsblkid_struct_tag)(iqunsafe.ppPointer(aahead1)).fdbit_name = Xblkid_strdup(cgtls, aaname)
					if !((*tsblkid_struct_tag)(iqunsafe.ppPointer(aahead1)).fdbit_name != 0) {
						goto pperrout
					}
					ccv1 = (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_cache + 16
					ccv3 = aahead1
					ccv4 = (*tslist_head)(iqunsafe.ppPointer(ccv1)).fdprev
					ccv5 = ccv1
					(*tslist_head)(iqunsafe.ppPointer(ccv5)).fdprev = ccv3
	...
	if aadev_var != 0 {
		*(*ppuintptr)(iqunsafe.ppPointer(aadev_var)) = aaval
	}

	if (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_cache != 0 {

		*(*ppuint32)(iqunsafe.ppPointer((*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_cache + 48)) |= ppuint32(mvBLKID_BIC_FL_CHANGED)
	}
	return 0
...
//	 */
func Xblkid_parse_tag_string(cgtls *iqlibc.ppTLS, aatoken ppuintptr, aaret_type ppuintptr, aaret_val ppuintptr) (cgr ppint32) {

	var aac ppint8
	var aacp, aaname, aavalue, ccv1, ccv3, ccv4, ccv5 ppuintptr
	var ccv2 ppbool
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aac, aacp, aaname, aavalue, ccv1, ccv2, ccv3, ccv4, ccv5

	if ccv2 = !(aatoken != 0); !ccv2 {
		ccv1 = Xstrchr(cgtls, aatoken, ppint32('='))
		aacp = ccv1
	}
	if ccv2 || !(ccv1 != 0) {
		return -ppint32(1)
	...
	aaname = Xblkid_strdup(cgtls, aatoken)
	if !(aaname != 0) {
		return -ppint32(1)
	}
	aavalue = aaname + ppuintptr(ppint64(aacp)-ppint64(aatoken))
	ccv1 = aavalue
	aavalue = aavalue + 1
	*(*ppint8)(iqunsafe.ppPointer(ccv1)) = ppint8('\000')
	if ppint32(*(*ppint8)(iqunsafe.ppPointer(aavalue))) == ppint32('"') || ppint32(*(*ppint8)(iqunsafe.ppPointer(aavalue))) == ppint32('\'') {
		ccv1 = aavalue
		aavalue = aavalue + 1
		aac = *(*ppint8)(iqunsafe.ppPointer(ccv1))
		ccv3 = Xstrrchr(cgtls, aavalue, ppint32(aac))
		aacp = ccv3
		if !(ccv3 != 0) {
			goto pperrout
	...
	aavalue = Xblkid_strdup(cgtls, aavalue)
	if !(aavalue != 0) {
		goto pperrout
	}

	*(*ppuintptr)(iqunsafe.ppPointer(aaret_type)) = aaname
	*(*ppuintptr)(iqunsafe.ppPointer(aaret_val)) = aavalue
...
func Xblkid_tag_iterate_begin(cgtls *iqlibc.ppTLS, aadev tnblkid_dev) (cgr tnblkid_tag_iterate) {

	var aaiter tnblkid_tag_iterate
	pp_ = aaiter

	aaiter = Xmalloc(cgtls, ppuint64(24))
	if aaiter != 0 {
		(*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fdmagic = ppint32(mvTAG_ITERATE_MAGIC)
		(*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fddev = aadev
		(*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fdp = (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_tags.fdnext
	}
	return aaiter
}
...
//	 */
func Xblkid_tag_next(cgtls *iqlibc.ppTLS, aaiter tnblkid_tag_iterate, aatype ppuintptr, aavalue ppuintptr) (cgr ppint32) {

	var aatag tnblkid_tag
	pp_ = aatag

	*(*ppuintptr)(iqunsafe.ppPointer(aatype)) = ppuintptr(0)
	*(*ppuintptr)(iqunsafe.ppPointer(aavalue)) = ppuintptr(0)
	if !(aaiter != 0) || (*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fdmagic != ppint32(mvTAG_ITERATE_MAGIC) || (*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fdp == (*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fddev+16 {
		return -ppint32(1)
	}
	aatag = (*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fdp - ppuintptr(ppuint64(iqlibc.ppUintptrFromInt32(0)))
	*(*ppuintptr)(iqunsafe.ppPointer(aatype)) = (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatag)).fdbit_name
	*(*ppuintptr)(iqunsafe.ppPointer(aavalue)) = (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatag)).fdbit_val
	(*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fdp = (*tslist_head)(iqunsafe.ppPointer((*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fdp)).fdnext
	return 0
}

func Xblkid_tag_iterate_end(cgtls *iqlibc.ppTLS, aaiter tnblkid_tag_iterate) {

	if !(aaiter != 0) || (*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fdmagic != ppint32(mvTAG_ITERATE_MAGIC) {
		return
	}
	(*tsblkid_struct_tag_iterate)(iqunsafe.ppPointer(aaiter)).fdmagic = 0
	Xfree(cgtls, aaiter)
}
...
//	 */
func Xblkid_find_dev_with_tag(cgtls *iqlibc.ppTLS, aacache tnblkid_cache, aatype ppuintptr, aavalue ppuintptr) (cgr tnblkid_dev) {

	var aadev tnblkid_dev
	var aahead, aatmp tnblkid_tag
	var aap ppuintptr
	var aapri, aaprobe_new ppint32
	pp_, pp_, pp_, pp_, pp_, pp_ = aadev, aahead, aap, aapri, aaprobe_new, aatmp
	aaprobe_new = 0

	if !(aacache != 0) || !(aatype != 0) || !(aavalue != 0) {
		return iqlibc.ppUintptrFromInt32(0)
	}
...
pptry_again:
	...
	aapri = -ppint32(1)
	aadev = ppuintptr(0)
	aahead = siblkid_find_head_cache(cgtls, aacache, aatype)

	if aahead != 0 {
		aap = (*tslist_head)(iqunsafe.ppPointer(aahead + 16)).fdnext
		for {
			if !(aap != aahead+16) {
				break
			}
			aatmp = aap - ppuintptr(ppuint64(iqlibc.ppUintptrFromInt32(0)+16))

			if !(Xstrcmp(cgtls, (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatmp)).fdbit_val, aavalue) != 0) && (*tsblkid_struct_dev)(iqunsafe.ppPointer((*tsblkid_struct_tag)(iqunsafe.ppPointer(aatmp)).fdbit_dev)).fdbid_pri > aapri && !(Xaccess(cgtls, (*tsblkid_struct_dev)(iqunsafe.ppPointer((*tsblkid_struct_tag)(iqunsafe.ppPointer(aatmp)).fdbit_dev)).fdbid_name, mvF_OK) != 0) {
				aadev = (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatmp)).fdbit_dev
				aapri = (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_pri
			}
	...
	if aadev != 0 && !((*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_flags&iqlibc.ppUint32FromInt32(mvBLKID_BID_FL_VERIFIED) != 0) {
		...
		if !(aadev != 0) || aadev != 0 && (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_flags&ppuint32(mvBLKID_BID_FL_VERIFIED) != 0 {
			goto pptry_again
		}
	}

	if !(aadev != 0) && !(aaprobe_new != 0) {
		if Xblkid_probe_all_new(cgtls, aacache) < 0 {
			return iqlibc.ppUintptrFromInt32(0)
		}
		aaprobe_new = aaprobe_new + 1
		goto pptry_again
	}

	if !(aadev != 0) && !((*tsblkid_struct_cache)(iqunsafe.ppPointer(aacache)).fdbic_flags&iqlibc.ppUint32FromInt32(mvBLKID_BIC_FL_PROBED) != 0) {
		if Xblkid_probe_all(cgtls, aacache) < 0 {
			return iqlibc.ppUintptrFromInt32(0)
		}
		goto pptry_again
	}
...
func _blkid_probe_all(*iqlibc.ppTLS, ppuintptr) ppint32

func _blkid_probe_all_new(*iqlibc.ppTLS, ppuintptr) ppint32
