...
func Xblkid_get_cache(cgtls *iqlibc.ppTLS, aaret_cache ppuintptr, aafilename ppuintptr) (cgr ppint32) {

	var aacache, ccv1 tnblkid_cache
	pp_, pp_ = aacache, ccv1

	ccv1 = Xcalloc(cgtls, ppuint64(1), ppuint64(64))
	aacache = ccv1
	if !(ccv1 != 0) {
		return -ppint32(mvBLKID_ERR_MEM)
	}

	(*tslist_head)(iqunsafe.ppPointer(aacache)).fdnext = aacache
	(*tslist_head)(iqunsafe.ppPointer(aacache)).fdprev = aacache
	(*tslist_head)(iqunsafe.ppPointer(aacache + 16)).fdnext = aacache + 16
	(*tslist_head)(iqunsafe.ppPointer(aacache + 16)).fdprev = aacache + 16

	if aafilename != 0 && !(Xstrlen(cgtls, aafilename) != 0) {
		aafilename = ppuintptr(0)
	}
	if !(aafilename != 0) {
		aafilename = sisafe_getenv(cgtls, "BLKID_FILE\x00")
	}
	if !(aafilename != 0) {
		aafilename = "/etc/blkid.tab\x00"
	}
	(*tsblkid_struct_cache)(iqunsafe.ppPointer(aacache)).fdbic_filename = Xblkid_strdup(cgtls, aafilename)

	Xblkid_read_cache(cgtls, aacache)

	*(*tnblkid_cache)(iqunsafe.ppPointer(aaret_cache)) = aacache
	return 0
}

func Xblkid_put_cache(cgtls *iqlibc.ppTLS, aacache tnblkid_cache) {

	var aabad, aatag tnblkid_tag
	var aadev tnblkid_dev
	var ccv1, ccv4, ccv7 ppuintptr
	var ccv2, ccv5, ccv8 ppint32
	pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_, pp_ = aabad, aadev, aatag, ccv1, ccv2, ccv4, ccv5, ccv7, ccv8
	if !(aacache != 0) {
		return
	}
	...
	cg_3:
		...
		aadev = (*tsblkid_struct_cache)(iqunsafe.ppPointer(aacache)).fdbic_devs.fdnext - ppuintptr(ppuint64(iqlibc.ppUintptrFromInt32(0)))
	...
	cg_6:
		...
		aatag = (*tsblkid_struct_cache)(iqunsafe.ppPointer(aacache)).fdbic_tags.fdnext - ppuintptr(ppuint64(iqlibc.ppUintptrFromInt32(0)))

		for {
			ccv4 = aatag + 16
			ccv5 = iqlibc.ppBoolInt32((*tslist_head)(iqunsafe.ppPointer(ccv4)).fdnext == ccv4)
			goto cg_9
		cg_9:
			if !!(ccv5 != 0) {
				break
			}
			aabad = (*tsblkid_struct_tag)(iqunsafe.ppPointer(aatag)).fdbit_names.fdnext - ppuintptr(ppuint64(iqlibc.ppUintptrFromInt32(0)+16))

			Xblkid_free_tag(cgtls, aabad)

		}
		Xblkid_free_tag(cgtls, aatag)

	}
	Xfree(cgtls, (*tsblkid_struct_cache)(iqunsafe.ppPointer(aacache)).fdbic_filename)

	Xfree(cgtls, aacache)
}

func Xblkid_gc_cache(cgtls *iqlibc.ppTLS, aacache tnblkid_cache) {
	cgbp := cgtls.ppAlloc(144)
	defer cgtls.ppFree(144)

	var aadev tnblkid_dev
	var aap, aapnext ppuintptr
	var pp_ /* st at bp+0 */ tsstat
	pp_, pp_, pp_ = aadev, aap, aapnext
	...
	for {
		...
		if Xstat(cgtls, (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_name, cgbp) < 0 {
