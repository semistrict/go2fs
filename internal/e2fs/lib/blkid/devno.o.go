...
func Xblkid_strndup(cgtls *iqlibc.ppTLS, aas ppuintptr, aalength ppint32) (cgr ppuintptr) {
	...
	if !(aalength != 0) {
		aalength = iqlibc.ppInt32FromUint64(Xstrlen(cgtls, aas))
	}

	aaret = Xmalloc(cgtls, iqlibc.ppUint64FromInt32(aalength+ppint32(1)))
	if aaret != 0 {
		Xstrncpy(cgtls, aaret, aas, iqlibc.ppUint64FromInt32(aalength))
		*(*ppint8)(iqunsafe.ppPointer(aaret + ppuintptr(aalength))) = ppint8('\000')
	}
	return aaret
}

func Xblkid_strdup(cgtls *iqlibc.ppTLS, aas ppuintptr) (cgr ppuintptr) {

	return Xblkid_strndup(cgtls, aas, 0)
}
...
//	 */
func siadd_to_dirlist(cgtls *iqlibc.ppTLS, aaname ppuintptr, aalist ppuintptr) {

	var aadp ppuintptr
	pp_ = aadp

	aadp = Xmalloc(cgtls, ppuint64(16))
	if !(aadp != 0) {
		return
	}
	(*tsdir_list)(iqunsafe.ppPointer(aadp)).fdname = Xblkid_strdup(cgtls, aaname)
	if !((*tsdir_list)(iqunsafe.ppPointer(aadp)).fdname != 0) {
		Xfree(cgtls, aadp)
		return
	}
	(*tsdir_list)(iqunsafe.ppPointer(aadp)).fdnext = *(*ppuintptr)(iqunsafe.ppPointer(aalist))
	*(*ppuintptr)(iqunsafe.ppPointer(aalist)) = aadp
}
...
//	 */
func sifree_dirlist(cgtls *iqlibc.ppTLS, aalist ppuintptr) {

	var aadp, aanext ppuintptr
	pp_, pp_ = aadp, aanext

	aadp = *(*ppuintptr)(iqunsafe.ppPointer(aalist))
	for {
		if !(aadp != 0) {
			break
		}
		aanext = (*tsdir_list)(iqunsafe.ppPointer(aadp)).fdnext
		Xfree(cgtls, (*tsdir_list)(iqunsafe.ppPointer(aadp)).fdname)
		Xfree(cgtls, aadp)
	...
	*(*ppuintptr)(iqunsafe.ppPointer(aalist)) = iqlibc.ppUintptrFromInt32(0)
}

func Xblkid__scan_dir(cgtls *iqlibc.ppTLS, aadirname ppuintptr, aadevno tndev_t, aalist ppuintptr, aadevname ppuintptr) {
	cgbp := cgtls.ppAlloc(1200)
	defer cgtls.ppFree(1200)

	var aadir, aadp, ccv1, ccv2 ppuintptr
	var aadirlen, ccv3 ppint32
	var pp_ /* path at bp+0 */ [1024]ppint8
	var pp_ /* st at bp+1024 */ tsstat
	pp_, pp_, pp_, pp_, pp_, pp_ = aadir, aadirlen, aadp, ccv1, ccv2, ccv3

	ccv1 = Xopendir(cgtls, aadirname)
	aadir = ccv1
	if ccv1 == iqlibc.ppUintptrFromInt32(0) {
		return
	}
	aadirlen = iqlibc.ppInt32FromUint64(Xstrlen(cgtls, aadirname) + ppuint64(2))
	for {
		...
		if iqlibc.ppUint64FromInt32(aadirlen)+Xstrlen(cgtls, aadp+21) >= ppuint64(1024) {
			continue
		}

		if ppint32(*(*ppint8)(iqunsafe.ppPointer(aadp + 21))) == ppint32('.') && (ppint32(*(*ppint8)(iqunsafe.ppPointer(aadp + 21 + 1))) == 0 || ppint32(*(*ppint8)(iqunsafe.ppPointer(aadp + 21 + 1))) == ppint32('.') && ppint32(*(*ppint8)(iqunsafe.ppPointer(aadp + 21 + 2))) == 0) {
			continue
		}

		Xsprintf(cgtls, cgbp, "%s/%s\x00", iqlibc.ppVaList(cgbp+1176, aadirname, aadp+21))
		if Xstat(cgtls, cgbp, cgbp+1024) < 0 {
			continue
		}

		ccv3 = iqlibc.ppBoolInt32(iqlibc.ppInt32FromUint16((*(*tsstat)(iqunsafe.ppPointer(cgbp + 1024))).fdst_mode)&ppint32(mvS_IFMT) == ppint32(mvS_IFBLK))
		goto cg_4
	cg_4:
		;
		if ccv3 != 0 && (*(*tsstat)(iqunsafe.ppPointer(cgbp + 1024))).fdst_rdev == aadevno {
			*(*ppuintptr)(iqunsafe.ppPointer(aadevname)) = Xblkid_strdup(cgtls, cgbp)

			break
		}
		if aalist != 0 && iqlibc.ppInt32FromUint16((*(*tsstat)(iqunsafe.ppPointer(cgbp + 1024))).fdst_mode)&ppint32(mvS_IFMT) == ppint32(mvS_IFDIR) && !(Xlstat(cgtls, cgbp, cgbp+1024) != 0) && iqlibc.ppInt32FromUint16((*(*tsstat)(iqunsafe.ppPointer(cgbp + 1024))).fdst_mode)&ppint32(mvS_IFMT) == ppint32(mvS_IFDIR) {
			siadd_to_dirlist(cgtls, cgbp, aalist)
		}
...
//	 */
func Xblkid_devno_to_devname(cgtls *iqlibc.ppTLS, aadevno tndev_t) (cgr ppuintptr) {
	cgbp := cgtls.ppAlloc(32)
	defer cgtls.ppFree(32)

	var aacurrent, aadir ppuintptr
	var pp_ /* devname at bp+16 */ ppuintptr
	var pp_ /* list at bp+0 */ ppuintptr
	var pp_ /* new_list at bp+8 */ ppuintptr
	pp_, pp_ = aacurrent, aadir
	...
	*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)) = iqlibc.ppUintptrFromInt32(0)

	/*
	 * Add the starting directories to search in reverse order of
	 * importance, since we are using a stack...
	 */
	aadir = ppuintptr(iqunsafe.ppPointer(&sidevdirs))
	for {
		if !(*(*ppuintptr)(iqunsafe.ppPointer(aadir)) != 0) {
			break
		}
		siadd_to_dirlist(cgtls, *(*ppuintptr)(iqunsafe.ppPointer(aadir)), cgbp)
		goto cg_1
	cg_1:
		;
		aadir += 8
	...
	for *(*ppuintptr)(iqunsafe.ppPointer(cgbp)) != 0 {
		aacurrent = *(*ppuintptr)(iqunsafe.ppPointer(cgbp))

		*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = (*tsdir_list)(iqunsafe.ppPointer(*(*ppuintptr)(iqunsafe.ppPointer(cgbp)))).fdnext

		Xblkid__scan_dir(cgtls, (*tsdir_list)(iqunsafe.ppPointer(aacurrent)).fdname, aadevno, cgbp+8, cgbp+16)
		Xfree(cgtls, (*tsdir_list)(iqunsafe.ppPointer(aacurrent)).fdname)
		Xfree(cgtls, aacurrent)
		if *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16)) != 0 {
			break
		}
		/*
		 ...
		 */
		if *(*ppuintptr)(iqunsafe.ppPointer(cgbp)) == iqlibc.ppUintptrFromInt32(0) {
			*(*ppuintptr)(iqunsafe.ppPointer(cgbp)) = *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 8))
			*(*ppuintptr)(iqunsafe.ppPointer(cgbp + 8)) = iqlibc.ppUintptrFromInt32(0)
		}
	}
	sifree_dirlist(cgtls, cgbp)
	sifree_dirlist(cgtls, cgbp+8)
	...
	return *(*ppuintptr)(iqunsafe.ppPointer(cgbp + 16))
}
...
func _lstat(*iqlibc.ppTLS, ppuintptr, ppuintptr) ppint32
