...
const mvDEV_ITERATE_MAGIC = 27600972
...
type tsblkid_struct_dev_iterate = struct {
	fdmagic        ppint32
	fdcache        tnblkid_cache
	fdsearch_type  ppuintptr
	fdsearch_value ppuintptr
	fdp            ppuintptr
}
...
func Xblkid_new_dev(cgtls *iqlibc.ppTLS) (cgr tnblkid_dev) {

	var aadev, ccv1 tnblkid_dev
	pp_, pp_ = aadev, ccv1

	ccv1 = Xcalloc(cgtls, ppuint64(1), ppuint64(96))
	aadev = ccv1
	if !(ccv1 != 0) {
		return iqlibc.ppUintptrFromInt32(0)
	}

	(*tslist_head)(iqunsafe.ppPointer(aadev)).fdnext = aadev
	(*tslist_head)(iqunsafe.ppPointer(aadev)).fdprev = aadev
	(*tslist_head)(iqunsafe.ppPointer(aadev + 16)).fdnext = aadev + 16
	(*tslist_head)(iqunsafe.ppPointer(aadev + 16)).fdprev = aadev + 16

	return aadev
}

func Xblkid_free_dev(cgtls *iqlibc.ppTLS, aadev tnblkid_dev) {

	var aatag tnblkid_tag
	var ccv1, ccv2, ccv3, ccv4 ppuintptr
	var ccv5 ppint32
	pp_, pp_, pp_, pp_, pp_, pp_ = aatag, ccv1, ccv2, ccv3, ccv4, ccv5
	if !(aadev != 0) {
		return
	}

	ccv1 = aadev
	ccv2 = (*tslist_head)(iqunsafe.ppPointer(ccv1)).fdprev
	ccv3 = (*tslist_head)(iqunsafe.ppPointer(ccv1)).fdnext
	(*tslist_head)(iqunsafe.ppPointer(ccv3)).fdprev = ccv2
	(*tslist_head)(iqunsafe.ppPointer(ccv2)).fdnext = ccv3
	for {
		ccv1 = aadev + 16
		ccv5 = iqlibc.ppBoolInt32((*tslist_head)(iqunsafe.ppPointer(ccv1)).fdnext == ccv1)
		goto cg_6
	cg_6:
		if !!(ccv5 != 0) {
			break
		}
		aatag = (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_tags.fdnext - ppuintptr(ppuint64(iqlibc.ppUintptrFromInt32(0)))
		Xblkid_free_tag(cgtls, aatag)

	}
	Xfree(cgtls, (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_name)
	Xfree(cgtls, aadev)
}
...
//	 */
func Xblkid_dev_devname(cgtls *iqlibc.ppTLS, aadev tnblkid_dev) (cgr ppuintptr) {

	return (*tsblkid_struct_dev)(iqunsafe.ppPointer(aadev)).fdbid_name
}

func Xblkid_dev_iterate_begin(cgtls *iqlibc.ppTLS, aacache tnblkid_cache) (cgr tnblkid_dev_iterate) {

	var aaiter tnblkid_dev_iterate
	pp_ = aaiter

	aaiter = Xmalloc(cgtls, ppuint64(40))
	if aaiter != 0 {
		(*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdmagic = ppint32(mvDEV_ITERATE_MAGIC)
		(*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdcache = aacache
		(*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdp = (*tsblkid_struct_cache)(iqunsafe.ppPointer(aacache)).fdbic_devs.fdnext
		(*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdsearch_type = ppuintptr(0)
		(*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdsearch_value = ppuintptr(0)
	}
	return aaiter
}

func Xblkid_dev_set_search(cgtls *iqlibc.ppTLS, aaiter tnblkid_dev_iterate, aasearch_type ppuintptr, aasearch_value ppuintptr) (cgr ppint32) {

	var aanew_type, aanew_value ppuintptr
	pp_, pp_ = aanew_type, aanew_value

	if !(aaiter != 0) || (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdmagic != ppint32(mvDEV_ITERATE_MAGIC) || !(aasearch_type != 0) || !(aasearch_value != 0) {
		return -ppint32(1)
	}
	aanew_type = Xmalloc(cgtls, Xstrlen(cgtls, aasearch_type)+ppuint64(1))
	aanew_value = Xmalloc(cgtls, Xstrlen(cgtls, aasearch_value)+ppuint64(1))
	if !(aanew_type != 0) || !(aanew_value != 0) {
		Xfree(cgtls, aanew_type)
		Xfree(cgtls, aanew_value)
		return -ppint32(1)
	}
	Xstrcpy(cgtls, aanew_type, aasearch_type)
	Xstrcpy(cgtls, aanew_value, aasearch_value)
	Xfree(cgtls, (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdsearch_type)
	Xfree(cgtls, (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdsearch_value)
	(*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdsearch_type = aanew_type
	(*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdsearch_value = aanew_value
	return 0
}
...
//	 */
func Xblkid_dev_next(cgtls *iqlibc.ppTLS, aaiter tnblkid_dev_iterate, aaret_dev ppuintptr) (cgr ppint32) {

	var aadev tnblkid_dev
	pp_ = aadev

	*(*tnblkid_dev)(iqunsafe.ppPointer(aaret_dev)) = ppuintptr(0)
	if !(aaiter != 0) || (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdmagic != ppint32(mvDEV_ITERATE_MAGIC) {
		return -ppint32(1)
	}
	for (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdp != (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdcache {
		aadev = (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdp - ppuintptr(ppuint64(iqlibc.ppUintptrFromInt32(0)))
		(*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdp = (*tslist_head)(iqunsafe.ppPointer((*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdp)).fdnext
		if (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdsearch_type != 0 && !(Xblkid_dev_has_tag(cgtls, aadev, (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdsearch_type, (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdsearch_value) != 0) {
			continue
		}
		*(*tnblkid_dev)(iqunsafe.ppPointer(aaret_dev)) = aadev
		return 0
	}
	return -ppint32(1)
}

func Xblkid_dev_iterate_end(cgtls *iqlibc.ppTLS, aaiter tnblkid_dev_iterate) {

	if !(aaiter != 0) || (*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdmagic != ppint32(mvDEV_ITERATE_MAGIC) {
		return
	}
	(*tsblkid_struct_dev_iterate)(iqunsafe.ppPointer(aaiter)).fdmagic = 0
	Xfree(cgtls, aaiter)
}

func _blkid_dev_has_tag(*iqlibc.ppTLS, ppuintptr, ppuintptr, ppuintptr) ppint32

func _blkid_free_tag(*iqlibc.ppTLS, ppuintptr)
